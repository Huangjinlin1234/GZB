...
export const vueMixinButton = {
  methods: {
    getFactory () {
      let _this = this;
      let cur = _this;
      while (cur.$vnode && cur.$vnode.tag && cur.$vnode.tag.indexOf('TemplateFactory') === -1) {
        cur = cur.$parent;
      }
      cur.contextData = cur.pageParams;
      return cur;
    },
    buttonLogs: function (btnObj, buttonName) {
      ...
      if (!tab) { // toto confirm q确认框无法获取当前页面路由
        return;
      }
    ...
    checkCtrl: function (ctrlCode, menuId) {
      ...
      return !checkCtrl(ctrlCode, menuId);
    }
  }
};
// 使用方法，在具体的页面中增加：
