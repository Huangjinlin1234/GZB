...
export function request (option) {
  ...
  if (options.async == false) {
    ...
    utilVue.$request(options).then((response) => {
...
// 弹出窗口.
export function openDialogAndCloseMe (_title, _js, _width, _height, _pars, _callBack, _isCloseMe, _isHaveCloseBtn, _jsMode) {
...
// 前端弹出窗口
export function showTextOrHtmlMsgBox (_title, _text, _isHtml, _width, _height, _btns, _callBackFunc, _isAutoClose, _type) {
  // 先取得菜单根窗口
...
export function importJsOrderBy (_allJSFiles, _index, _callBack, _idMark) {
  ...
  var strOneJsFile = _allJSFiles[_index];
  var script = document.createElement('script');
  if (_idMark != undefined) { // 如果定义了id标记,则设置id,个id用于判断重复
...
export function replaceStrByMacroKeys (_sql, jsoCardData) {
  ...
  var jsoUser = this.getLoginUserInfo(); //
  var strWhereSQL = _sql;
  var aryItems = this.parseStrMacroKeys(strWhereSQL, '${', '}');
  var strItemValue = null;
  if (aryItems != null && aryItems.length > 0) {
    for (var i = 0; i < aryItems.length; i++) {
      var strItemKey = aryItems[i]; //
      if (strItemKey == 'LoginUserId') {
        ...
        strWhereSQL = strWhereSQL.replace('${' + strItemKey + '}', strItemValue);
      } else if (strItemKey == 'LoginOrgCode') { // 机构编码
        ...
        strWhereSQL = strWhereSQL.replace('${' + strItemKey + '}', strItemValue);
      } else if (strItemKey == 'LoginInstuCode') { // 机构编码
        ...
        if (jsoCardData != undefined && jsoCardData != null) {
          var isHaveCol = jsoCardData.hasOwnProperty(strItemKey.toLocaleLowerCase()); // 是否有该字段
          var strCardItemValue = jsoCardData[strItemKey.toLocaleLowerCase()]; // 卡片中的值
          if (isHaveCol) { // 如果模板中有该字段才进行实际替换
            strWhereSQL = strWhereSQL.replace('${' + strItemKey + '}', strCardItemValue);
          }
        }
      }
    }
...
export function getLoginUserInfo () {
  var userInfo = utils.sessionStore.get('YUFP-SESSION-USER');
...
export function getSEQFromServer (seqId) {
  ...
  this.request({
    ...
    success: function (response) {
      if (response.code == 0) {
        tempNewSeq = Array.isArray(response.data) ? response.data[0] : response.data;
      }
    }
  });
  return tempNewSeq;
...
export function getSEQWithParamFromServer (seqId, param) {
  ...
  this.request({
    type: 'POST',
    url: backend.seqService + '/api/sequenceconfig/getSequenceWithTemplateWithCount/' + seqId + '/1',
    async: false,
...
export function getLoginUserInfo2 () {
  var jsoUser = utils.sessionStore.get('YUFP-SESSION-USER');
  var jsoUser2 = {};
...
export function openDialog (_title, _js, _width, _height, _pars, _callBack) {
  this.openDialogAndCloseMe(_title, _js, _width, _height, _pars, _callBack, false, true);
};
// 没有关闭按钮
export function openDialogNoCloseBtn (_title, _js, _width, _height, _pars, _callBack) {
  this.openDialogAndCloseMe(_title, _js, _width, _height, _pars, _callBack, false, false);
};
...
// 弹出消息
export function showMsgBoxAutoClose (_title, _msg, _width, _height, _callback, _type) {
  // 待实现
  this.showTextOrHtmlMsgBox(_title, _msg, false, _width, _height, ['确定'], _callback, true, _type);
};
// 弹出消息
export function showConfirmBox (_title, _msg, _width, _height, _callback, _type) {
  if (!_type) {
    _type = 'warning';
  }
  this.showTextOrHtmlMsgBox(_title, _msg, false, _width, _height, ['Yes/是', 'No/否'], function (_index) {
    if (_index == 'confirm') {
      _callback(true);
    } else {
      _callback(false);
...
// 弹出消息
export function showOptionBox (_title, _msg, _width, _height, _optionBtns, _callback) {
  this.showTextOrHtmlMsgBox(_title, _msg, false, _height, _height, _optionBtns, _callback);
};
// 弹出Html消息
export function showHtmlMsgBox (_title, _html, _width, _height, _callback) {
  this.showTextOrHtmlMsgBox(_title, _html, true, _width, _height, ['确定'], _callback);
};
// 根据主键名与值,拼出where条件,考虑了联合主键的场景..
export function getWhereConditionByPK (_pkName, _rowData) {
  if (_pkName.indexOf(',') > 0) { // 如果是联合主键
    var aryPks = _pkName.split(','); // 数组
    var strWhere = ' (';
    for (var i = 0; i < aryPks.length; i++) {
    ...
    var strPkValue = _rowData[_pkName]; // 主键值
    var strWhere = ' ' + _pkName + '=\'' + strPkValue + '\' '; // 直接是
    return strWhere;
  }
};
    ...
    case 18:
      ...
      if (parseInt(value.substr(6, 4)) % 4 == 0 ||
                      (parseInt(value.substr(6, 4)) % 100 == 0 && parseInt(value.substr(6, 4)) %
                          4 == 0)) {
        ereg = /^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|[1-2][0-9]))[0-9]{3}([0-9]|[X]|[x])$/;// 闰年出生日期的合法性正则表达式
      } else {
        ereg = /^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|1[0-9]|2[0-8]))[0-9]{3}([0-9]|[X]|[x])$/;// 平年出生日期的合法性正则表达式
      }
...
export function getParentPage (vueObj, obj, method, param) {
  const parent = vueObj.$route.matched[vueObj.$route.matched.length - 1].instances.default;
  if (obj) {
...
export function getDefaultformulaData (formula) {
  ...
  if (formula.startsWith('$')) {
    ...
    var strSeqId = formula.substring(4, formula.length);
