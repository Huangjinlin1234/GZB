...
class Service{
  constructor(args) {
    this.options = {
      ...
      headers: {
        ...
        'Accept-Language': getLanguage() === 'zh_CN' ? 'zh_CN' : 'en_US'
  ...
  request (options) {
    ...
    _options.success = function (data, status, xhr) {
      // 定义过滤事件
      successEvent = { code: 0, message: 'success', data: data};
      ...
      _this.returnObj = {code: successEvent.code, message: successEvent.message, data: successEvent.data} // code
      if(execOptioins.success) {
        return execOptioins.success(_this.returnObj.data, 0);
      }
      if(execOptioins.callback) {
        return execOptioins.callback(successEvent.code,successEvent.message, successEvent.data);
      }
    };
    _options.error = function (xhr, status) {
      ...
      _this.returnObj = {code: errorEvent.code, message: errorEvent.message, data: errorEvent.data}
      if(execOptioins.error) {
        return execOptioins.error(_this.returnObj.data, null, null);
      }
    };
    new Ajax(_options);
