...
import { X_AUTHORIZATION, XIAO_YU_TOKEN } from '@/config/constant/app.data.common';
...
export function requestSuccessFunc (config) {
  ...
  if (config.needToken && !config.headers[X_AUTHORIZATION]) {
    ...
    if (_refreshToken === true) {
      refreshTokenFn({ 'grant_type': 'refresh_token', 'refresh_token': sessionStore.get(XIAO_YU_TOKEN).refresh_token }).then(res => {
...
var successCode = [20100003, 30100022, 30100026, 30100024, 30100034, 30100029, 30100035, '00000000', '10000002'];
export function responseSuccessFunc (res) {
  ...
  if (resData && code !== 'undefined' && code !== '20000' && code !== '200' && code !== '0' && successCode.indexOf(code) === -1) {
