<template>
  <div>
    <yu-xdialog :visible.sync="visiable" :width="width" title="押品基本信息">
        ...
        <yu-xtable ref="refTable" :row-number="true" selection-type="radio" :pageable="true" :data-url="dataUrl" :default-load="true" condition-key="condition" :base-params="searchData" requestType="POST">
          ...
          <yu-xtable-column label="抵/质押率（%）" prop="mortagageRate" width="120"></yu-xtable-column>
...
export default {
  name: 'YuGuarBaseInfoRetail',
  componentName: 'YuGuarBaseInfoRetail',
  ...
  data: function () {
    return {
      ...
      searchData: {condition: {
        grtFlag: this.parms.grtFlag
      }},
      dataUrl: backend.cmisBiz + '/api/guarbaseinfo/querybymanagerid',
      // 表格中传出id数据的key
      queryCode: 'guarNo',
      // 表格中显示到input框中的字段key
      textCode: 'guarNo'
