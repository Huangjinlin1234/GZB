<template>
  <div>
    <yu-xdialog :visible.sync="visiable" :width="width" title="客户信息">
        <yu-xform form-type="search" v-model="searchFormdata" :default-sort="{prop:'cusId',order:'descending'}" label-width="60px" related-table-name="refTable">
  ...
  data: function () {
    return {
      ...
      baseParams: {condition: {
        certType: 'A',
        cusState: this.parms.cusState,
        managerId: this.parms.managerId
      },
