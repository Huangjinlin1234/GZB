<template>
  <div>
    <yu-xdialog :visible.sync="visiable" :width="width" title="用户信息">
        ...
        <yu-xtable ref="refTable" :row-number="true" selection-type="radio" :pageable="true" :data-url="dataUrl" :default-load="true" :base-params="baseParams" request-type="get">
...
export default {
  name: 'YuXuser2',
  componentName: 'YuXuser2',
  mixins: [popList],
  data: function () {
    return {
      dataUrl: backend.appOcaService + '/api/adminsmuser/pageAll',
      // 表格中传出id数据的key
      queryCode: 'loginCode',
      // 表格中显示到input框中的字段key
      textCode: 'userName',
