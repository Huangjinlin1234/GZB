<template>
  <div>
    <yu-xdialog :visible.sync="visiable" :width="width" title="签发机构查询" >
        <yu-xform form-type="search" v-model="searchFormdata" label-width="100px" related-table-name="refTable" >
          <yu-xform-group :column="3">
            <yu-xform-item label="签发机构" ctype="input" placeholder="签发机构" name="signBrNo" fuzzy-query="both"></yu-xform-item>
            <yu-xform-item label="签发机构名称" ctype="input" placeholder="签发机构名称" name="signBrName" fuzzy-query="both"></yu-xform-item>
          </yu-xform-group>
        </yu-xform>
        <yu-xtable ref="refTable" request-type="POST" :row-number="true" condition-key="condition" :pageable="true" :data-url="dataUrl" :default-load="true" :base-params="baseParams">
          <yu-xtable-column label="签发机构" prop="signBrNo"></yu-xtable-column>
          <yu-xtable-column label="签发机构名称" prop="signBrName"></yu-xtable-column>
...
export default {
  name: 'YuXwPvpOrgQf',
  componentName: 'YuXwPvpOrgQf',
  mixins: [popList],
  data: function () {
    return {
      dataUrl: backend.cmisCfg + '/api/cfgaccporgsign/selectbymodel',
