              ...
              <table width="99%" border="0" align="center" cellpadding="5" cellspacing="0">
                <tr valign="top">
                  <yu-xform ref="refForm" v-model="formdata" label-width="80px" label-suffix="" label-position="right">
                    <yu-xform-item label="引用报表" placeholder="请选择" ctype="select" :options="sheetOptions" value="[01]资产负债表" name="sel_type" @change="changeItems">
                    </yu-xform-item>
                    ...
                    <yu-xform-item label="引用指标" placeholder="请选择" ctype="select"  :options="partOptions" name="item_id">
                    </yu-xform-item>
                    <yu-xform-item label="引用数据" placeholder="请选择" ctype="select" :options="dataOptions" value="[1]期初数" name="sel_datasrc">
                    </yu-xform-item>
                  </yu-xform>
                </tr>
                <tr>
...
export default {
  ...
  data: function () {
    return {
      ...
      formulaData: {},
      formdata: {},
      limitLength: 1000, // 默认展示科目条数
      formulaDialog: false,
      debounceSearch: null, // 下拉框自定义过滤方法
      options: [], // 所有科目选项
      partOptions: [], // 部分科目选项
      sheetOptions: [
        {key: '[01]资产负债表', value: '[01]资产负债表'},
        {key: '[02]损益表', value: '[02]损益表'},
        {key: '[03]现金流量', value: '[03]现金流量'},
        {key: '[04]财务指标', value: '[04]财务指标'},
        {key: '[05]所有者权益变动表', value: '[05]所有者权益变动表'},
        {key: '[06]财务简表', value: '[06]财务简表'}
      ],
      dataOptions: [
        {key: '[1]期初数', value: '[1]期初数'},
        {key: '[2]期末数', value: '[2]期末数'},
        {key: '[1]上年同期数', value: '[1]上年同期数'},
        {key: '[2]本年累计数', value: '[2]本年累计数'},
        {key: '[1]金额', value: '[1]金额'}
      ]
    };
  },
  ...
  methods: {
      ...
      yufp.service.request({
        ...
        data: {
          condition: JSON.stringify({
            fncConfTyp: (val && val.slice(1, 3)) || '01'
          })
        },
    ...
    showFormula: function () {
      if (!this.getShowFormulaVal()) {
        this.$message({message: '请选择合适的项目编辑方式!', type: 'warning'});
        return;
      }
      this.formulaDialog = true;
    },
    insert: function () {
      var string = '{' + this.formdata.sel_type + '.' + this.formdata.item_id + '.' + this.formdata.sel_datasrc + '}';
    ...
    check: function () {
      ...
      var leftCount = this.getFlagCount(formula, '{');
      var rightCount = this.getFlagCount(formula, '}');
    ...
    getFlagCount: function (str, flag) {
      var count = 0;
      while (str.indexOf(flag) != -1) {
        str = str.substring(str.indexOf(flag) + 1);
        count = count + 1;
      }
      return count;
    },
