<template>
  <div class="yu-workflow" style="border: 1px solid #dadce0">
            ...
            for (var i in evt.getProperty("cells")) {
              ...
              if (vertex.vertex) {
                if (
                  ...
                  vertex.data &&
                  (vertex.data.nodeType == "2" ||
                    vertex.data.nodeType == "3" ||
                    vertex.data.nodeType == "5")
                ) {
                  var tmpGeo = mxUtils.clone(vertex.getGeometry());
                  tmpGeo.width = 40;
                  tmpGeo.height = 40;
    ...
    updateCellImage: function (cell, prefix = '') {
      if (cell) {
        this.graph.setCellStyles(
          "image",
          prefix + "libs/mxgraph/images/" + cell.data.nodeType + ".png",
          [cell]
        );
        // 如果是开始节点和结束节点，更新图标大小
      ...
      vertexs.forEach(function (v) {
        if (v.geometry) {
          if (!v.edges || v.edges.length < 2) {
            var str =
              "节点: " + v.data.nid + "-" + v.data.label + " 必须要有连线!";
      ...
      vertexs.forEach(function (v) {
        if (
          ...
          if (
            ...
            var str =
              "节点: " + v.data.nid + "-" + v.data.label + " 未配置处理人员!";
            result.messages.push(str);
            result.valid = false;
          }
        }
