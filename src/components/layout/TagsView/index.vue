  ...
  <div class="yu-frame-tab app-main">
    <div class="yu-frame-tabs" id="yu-frame-tabs" ref="idxTabs" v-if="groupModel" v-show="!partFlag">
      <div class="yu-tab-content">
        <div>
          <router-link v-for="tag in visitedViews" :ref="'tag_'+tag.meta.id" :key="tag.path" :class="isActive(tag)?'active ck':''" :to="{ name: tag.name, params:tag.params, path: tag.path, query: tag.query, fullPath: tag.fullPath }" tag="span" @click.native="clickTag(tag)" @click.middle.native="closeSelectedTag(tag)" @dblclick.prevent.native="dblclickReloadFn(tag)" @contextmenu.prevent.native="openMenu(tag,$event)">
            {{ generateTitle((tag.params && tag.params.title) || (tag.query && tag.query.title) || tag.title) }}
...
export default {
  ...
  watch: {
    $route () {
      if (this.$route && this.$route.params && this.$route.params._routeType === 'replace') {
        // 替换路由,更改当前标题及路径
      } else {
        this.addTags();
      }
  ...
  methods: {
    ...
    closeSelectedTag (view) {
      ...
      if (typeof view.params.afterTabClose === 'function') {
        view.params.afterTabClose();
      }
    },
    closeOthersTags () {
    ...
    sessionChange (view, flag) {
      if (window.sessionStorage.getItem('dynamicRoutes')) {
        const dynamicRoutes = JSON.parse(window.sessionStorage.getItem('dynamicRoutes')).dRoute;
