      ...
      <div class="yu-frame-top-bar-right" :style="{ width: topBarRightWidth + 'px' }">
        ...
        <div class="sys-tools-box">
          <template v-for="(item, index) in sysToolsFiltered">
            <span v-if="item.id !== 'pickup'" class="sys-tools" :key="index" :title="item.event && item.event.click ? item.text : ''" :class="[item.icon, item.className, { 'serach-show': showSearchInput && item.id === 'search', click: item.event && item.event.click }]" @click="toolClick(item, $event)" @mouseenter="toolMouseEnter(item, $event)" @mouseout="toolMouseOut(item, $event)">
              <yu-badge v-if="item.badgeDot" is-dot></yu-badge>
              <div v-if="item.id === 'sign'" class="yu-frame-dropdown-menu" title>
...
export default {
  components: { topMenu, YufpPasswordModify },
