 ...
 * @updated by
 * @description 贷后检查-定期检查-小微经营贷定期检查
 */
<template>
    ...
    <yu-panel :collapse-hide="false">
      ...
      <yu-panel ref="panel"  title="输入查询条件"  class="adjust-height" :collapse-hide="false">
              <yu-xform ref="searchForm" related-table-name="pspTaskTable" v-model="searchFormdata" >
                ...
                <yu-xform-group>
                  <yu-xform-item name="taskStartDtStart" label="任务生成日期 起" :colspan="4" ctype="datepicker"></yu-xform-item>
                  <yu-xform-item name="taskStartDtEnd" label="止" :colspan="4" ctype="datepicker" label-width="20px"></yu-xform-item>
                  ...
                  <yu-xform-item name="execBrId" :colspan="8" ctype="YuXorgForDh" @select-fn="commonSelectFn" :mapping="{'execBrId':'execBrId','execBrIdName':'execBrIdName'}" v-if="brId" label="任务执行机构"></yu-xform-item>
                </yu-xform-group>
  ...
  methods: {
    ...
    handleClick (e) {
      if (e.name === 'history') {
        ...
        this.tableTitle = '贷后定期检查历史列表（小微经营性）';
        this.searchData = {
          condition: {
            // inputId: this.$xutils.getDefaultformulaData('$LoginLoginCode'),
            checkType: '24',
        ...
        this.tableTitle = '贷后定期检查任务列表（小微经营性）';
        this.searchData = {
          condition: {
            // inputId: this.$xutils.getDefaultformulaData('$LoginLoginCode'),
            checkType: '24',
