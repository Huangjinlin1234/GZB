 ...
 * @updated by
 * @description 贷后检查-定期检查-个人消费贷定期检查
 */
<template>
      ...
      <yu-panel ref="panel"  title="输入查询条件"  class="adjust-height" :collapse-hide="false">
              <yu-xform ref="searchForm" related-table-name="pspTaskTable" v-model="searchFormdata" >
                <yu-xform-group>
                  ...
                  <yu-xform-item label="报告类型" name="rptType" ctype="select" data-code="STD_ZB_RPT_TYPE" :colspan="8"></yu-xform-item>
                </yu-xform-group>
                <yu-xform-group>
  ...
  methods: {
    ...
    handleClick (e) {
      if (e.name === 'history') {
        ...
        this.tableTitle = '定期贷后检查任务历史列表（个人消费性）';
        this.searchData = {
          condition: {
            // inputId: this.$xutils.getDefaultformulaData('$LoginLoginCode'),
            checkType: '23',
        ...
        this.tableTitle = '定期贷后检查任务列表（个人消费性）';
        this.searchData = {
          condition: {
            // inputId: this.$xutils.getDefaultformulaData('$LoginLoginCode'),
            checkType: '23',
