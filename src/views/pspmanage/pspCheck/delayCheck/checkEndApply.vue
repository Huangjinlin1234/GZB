 ...
 * @updated by
 * @description 贷后检查终止申请向导
 */
<template>
  <div id="riskAdjustApply">
    <yu-panel title="" :collapse-hide="false">
      <!--分类任务信息展示-->
      <yu-panel title="贷后检查终止申请向导" :collapse-hide="false">
  ...
  methods: {
    ...
    init: function () {
      ...
      _this.$xutils.request({
        ...
        error: (result, b) => {
          _this.$xutils.showMsgBox('提示', result + '；错误信息：' + b); // 弹出提示框
        }
      });
    },
    // 选择任务
    ...
    saveApplyData () {
      ...
      _this.applyData.taskEndDt = _this.$route.params.selectedTask.taskEndDt;
      _this.applyData.approveStatus = '000';
      _this.applyData.applyType = '2';
    ...
    nextFn: function () {
      this.saveApplyData();
      // console.log('res', this.applyData.pkId);
      ...
      this.$nextTick(function () {
        this.$router.addTab({
        // 路由名称
          name: 'pspmanage/pspCheck/delayCheck/checkDelayAppDetail',
          // 自定义唯一页签key
