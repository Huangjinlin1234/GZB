      ...
      <yu-panel title="检查任务信息" :collapse-hide="false">
        <yu-xform ref="pspTaskListForm" v-model="taskData" label-width="180px">
          <yu-xform-group :column="2">
            ...
            <yu-xform-item label="任务生成日期" disabled name="taskStartDtName"></yu-xform-item>
            <yu-xform-item label="任务要求完成日期" disabled name="taskEndDtOld"></yu-xform-item>
          </yu-xform-group>
        </yu-xform>
      </yu-panel>
      <yu-panel title="申请延期信息" :collapse-hide="false">
        <yu-xform ref="pspDelayForm" v-model="delayData" label-width="180px">
          <yu-xform-group :column="2">
            <yu-xform-item label="业务流水号" disabled name="serno"></yu-xform-item>
            <yu-xform-item label="申请延期完成日期" name="delayDate" :disabled="viewFlag"  ctype="datepicker" :rules="delayDate" ></yu-xform-item>
            <yu-xform-item label="申请延期理由"  :disabled="viewFlag" ctype="textarea" colspan="24" name="applyReason" rules="required"></yu-xform-item>
...
yufp.lookup.reg('STD_RISK_CUS_CATALOG,STD_RISK_TASK_TYPE,STD_RISK_CHECK_TYPE,STD_RISK_CHECK_STATUS,STD_ZB_CHECK_TYPE');
...
export default {
  ...
  data: function () {
    ...
    var checkDelayDate = function (rule, value, callback) {
      var taskEndDt = +new Date(_this.taskData.taskEndDtOld);
  ...
  methods: {
    ...
    init: function () {
      ...
      // // _this.taskData.execId = _this.$xutils.getDefaultformulaData('$LoginUserName');
    ...
    delayDataInit (serno) {
      ...
      let params = {};
      params.serno = serno;
      _this.delayData.serno = serno; // 页面下半部分 delayData
      // 通过任务号获取分类调整信息
    ...
    submitFn: function () {
      ...
      // 开始提交流程
      let data = _this.$route.params;
      table.delayDate = _this.delayData.delayDate;
      table.taskNo = data.pspTask.taskNo;
      table.checkType = data.pspTask.checkType;
      table.cusId = data.pspTask.cusId;
      table.cusName = data.pspTask.cusName;
      table.execId = data.pspTask.execId;
      table.execBrId = data.pspTask.execBrId;
      table.taskStartDt = data.pspTask.taskStartDt;
      table.taskEndDtOld = data.pspTask.taskEndDtOld;
      // FormData = _this.$route.params.pspTask;
      ...
      if (orgType == 'A') {
        if (loginUser.orgCode.startsWith('80')) {
          bizType = 'SGK20';// 贷后检查延期（寿光）
        } else if (loginUser.orgCode.startsWith('81')) {
          bizType = 'DHK20';// 贷后检查延期（东海）
        }
      } else {
      ...
      if (bizType != 'DH042' && bizType != 'DH046') {
        ...
        // console.log('data.startdto:', startdto);
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          if (response.code == '0') {
            ...
            _this.$message.error('查询分中心负责人失败');
          }
        }
      });
    },
