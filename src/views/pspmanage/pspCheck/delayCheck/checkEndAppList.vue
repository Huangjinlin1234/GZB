  ...
  <div class="psp-task-list">
    <yu-panel :collapse-hide="false">
      <yu-tabs v-model="activeName" @tab-click="handleClick">
        <yu-tab-pane label="贷后检查终止申请" name="todo"></yu-tab-pane>
        <yu-tab-pane label="贷后检查终止申请历史" name="history"></yu-tab-pane>
      </yu-tabs>
      ...
      <yu-panel ref="panel" :title="tableTitle" color="red" class="adjust-height" :collapse-hide="false">
        ...
        <!--任务主表显示列表-->
          <yu-xtable ref="pspTaskTable"   :data-url="listUrl" :base-params="searchData" selection-type="radio" condition-key="condition" request-type="post" >
            <yu-xtable-column align="center" label="业务流水号" prop="serno" width="200"></yu-xtable-column>
            ...
            <yu-xtable-column align="center" label="任务到期日期" prop="taskEndDt" width="120"> </yu-xtable-column>
...
export default {
  ...
  methods: {
    ...
    addFn () {
      this.$dialog.open(
        '贷后检查终止申请向导',
        'pspmanage/pspCheck/delayCheck/checkEndApply',
        1300,
        700,
        null,
        () => {
    ...
    check: function (op) {
      ...
      selections[0].applyType = '2';
      let name = 'pspmanage/pspCheck/delayCheck/checkDelayAppDetail';
      let opType = op;
      let title = '贷后检查终止申请';
      let key = 'checkEndApp' + new Date().getTime();
      this.$nextTick(function () {
        ...
        // 路由名称
          ...
          title: title,
          // 传递的业务数据，可选配置
          data: { pspTask: selections[0], opType: opType, key: key }
        });
      });
    }
