    ...
    <yu-tabs v-model="activeName">
        <yu-panel ref="panel" title="风险分类任务列表"  color="red" class="adjust-height" :collapse-hide="false">
         <div>
          ...
          <yu-xtable ref="riskTaskTable"  row-number :data-url="dataUrl" request-type="POST" :base-params="searchData" selection-type="radio" condition-key="condition" @loaded="loadedFn">
            ...
            <yu-xtable-column label="风险分类结果" prop="classType" :data-code="taskOptionType" width="370" ctype="select" disabled></yu-xtable-column>
          </yu-xtable>
      </yu-panel>
        ...
        <yu-xtable ref="refTable" selection-type="radio" row-number :data-url="dataUrl1" request-type="POST" :base-params="searchData" condition-key="condition">
          ...
          <yu-xtable-column label="贷后检查结论" prop="checkComment" width="300"></yu-xtable-column>
        </yu-xtable>
        </yu-panel>
...
<script>
// 注册字典项
yufp.lookup.reg('STD_ZB_CHECK_TYPE,STD_FIVE_CLASS,STD_TEN_CLASS');
export default {
  data: function () {
    return {
      taskOptionType: '',
      dataUrl: backend.cmisPsp + '/api/risktasklist/queryList',
      // dataUrl1: backend.cmisPsp + '/api/psptasklist/queryPspTaskList',
      //   dataUrl2: backend.cmisBiz + '/api/docDestroyAppList/query/pageList'
      searchData: {condition: { cusId: this.$route.meta.params.cusId || this.getFactory().contextData.cusId || this.getFactory().contextData.instanceInfo.cusId, approveStatus: '997'}},
      baseHistParams: {
        cusId: this.$route.meta.params.cusId || this.getFactory().contextData.cusId || this.getFactory().contextData.instanceInfo.cusId
      },
      checkType: '',
      od: '',
      name0: '',
  ...
  created () {
    var cusId = this.$route.meta.params.cusId || this.getFactory().contextData.cusId || this.getFactory().contextData.instanceInfo.cusId;
    var result = cusId.startsWith('2');
    if (result) {
      this.taskOptionType = 'STD_FIVE_CLASS';
    } else {
      this.taskOptionType = 'STD_TEN_CLASS';
    }
  },
  methods: {
    ...
    loadedFn (data) {
      ...
      this.$nextTick(() => {
        data.forEach(item => {
          if (result) {
            item.classType = item.manualClass;
          } else {
            item.classType = item.manualTenClass;
          }
        });
      });
    },
