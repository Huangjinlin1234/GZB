...
<template>
  ...
  <yu-row>
    ...
    <yu-col :sm="20" :xs="24">
      ...
      <indivRiskOperAnaly v-show="activeIndex == '1-3'" ref="indivRiskOperAnaly"></indivRiskOperAnaly>
...
import indivRiskOperAnaly from '@/views/pspmanage/riskDivide/indivRiskOperAnaly';
...
export default {
  ...
  components: {imageSystem, riskDivideDetail, indivRiskDebitAnaly, indivRiskOperAnaly, riskLoanSituAnaly, riskGuarContAnaly, riskPldimnAnaly, riskGuarntrAnaly, riskResultInfo, yufpNwfInit },
  data: function () {
    return {
  ...
  methods: {
    ...
    submitFn: function (params) {
      ...
      if (!_this.$refs.indivRiskOperAnaly.operData.indivOperSitu || !_this.$refs.indivRiskOperAnaly.operData.incomeChange || !_this.$refs.indivRiskOperAnaly.operData.profitChange || !_this.$refs.indivRiskOperAnaly.operData.cashChange ||
      !_this.$refs.indivRiskOperAnaly.operData.isIncomeSuffice) {
        _this.$xutils.showMsgBox('提示', '经营情况分析情况分析录入信息不完整！');
      ...
      if (orgType == 'A') {
        if (loginUser.orgCode.startsWith('80')) {
          startdto.bizType = 'SGK23';// 个人经营性风险分类（寿光）
        } else if (loginUser.orgCode.startsWith('81')) {
          startdto.bizType = 'DHK23';// 个人经营性风险分类（东海）
        }
      } else {
          ...
          for (let i = 0; i < dutysList.length; i++) {
            ...
            if (item.code === 'XWB01') { // 小微客户经理
              if (!_this.$refs.riskResultInfo.rstData.lastClassRst) {
                startdto.bizType = 'DH049';// 小微风险分类（新发放及未调整）新发放
              } else if (_this.$refs.riskResultInfo.rstData.lastClassRst == _this.$refs.riskResultInfo.rstData.manualClass) {
                startdto.bizType = 'DH049';// 小微风险分类（新发放及未调整）未调整
              } else if (_this.$refs.riskResultInfo.rstData.lastClassRst != _this.$refs.riskResultInfo.rstData.manualClass) {
                startdto.bizType = 'DH054';// 小微风险分类（大类调整）
              }
            } else { // 其他发起人
              if (!_this.$refs.riskResultInfo.rstData.lastClassRst) {
                startdto.bizType = 'DH057';// 对公及个人经营性风险分类（新发放及未调整）新发放
              } else if (_this.$refs.riskResultInfo.rstData.lastClassRst == _this.$refs.riskResultInfo.rstData.manualClass) {
                startdto.bizType = 'DH057';// 小微风险分类（新发放及未调整）未调整
              } else if (_this.$refs.riskResultInfo.rstData.lastClassRst != _this.$refs.riskResultInfo.rstData.manualClass) {
                startdto.bizType = 'DH059';// 对公及个人经营性风险分类（大类调整）
              }
            }
          }
        } else {
    ...
    chooseFn: function () {
      ...
      case '1-3':
        ...
        _this.vdata = _this.$refs.indivRiskOperAnaly.operData;
        _this.tabel = true;
        _this.updateFlag = _this.changeFlag == false ? _this.updateFlag : _this.$refs.indivRiskOperAnaly.updateFlag;
        _this.$refs.indivRiskOperAnaly.$refs.riskOperAnalyForm.validate(function (valid) {
          validate = valid;
        });
        break;
      case '1-4':
      ...
      case '1-9':
        url = '/api/risktasklist/';
        _this.vdata = _this.$refs.indivRiskimage.rstData;
        _this.tabel = true;
        _this.updateFlag = true;
        _this.$refs.indivRiskimage.$refs.riskResultForm.validate(function (valid) {
          validate = valid;
        });
        break;
      default:
        ...
        this.$xutils.request({
          ...
          success: (response, status, xhr) => {
            if (response.code === '0') {
              ...
              _this.$refs.indivRiskOperAnaly.init();
