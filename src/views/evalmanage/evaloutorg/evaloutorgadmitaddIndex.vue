<template>
  <div id="d1_BillCard">
      ...
      <yu-xform ref="refForm" label-width="180px" :disabled="formType == 'details'" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent" class="long-label">
        <yu-panel title="基本信息录入" panel-type="simple">
          ...
          <yu-xform-group :column="3">
            ...
            <yu-xform-item label="准入机构" ctype="input" placeholder="准入机构" name="admitBranchName" disabled hidden></yu-xform-item>
            <yu-xform-item label="准入状态" ctype="select" name="outOrgAdmitStatus" rules="required" data-code="STD_ZB_ADMIT_STATE" placeholder="准入状态" :datacode-filter="filterSelectOptions"></yu-xform-item>
    ...
    <yu-form-buttons class="yubfp-button-group" style="text-align:center;">
      <yu-button type="primary" @click="onSave()" v-norepeat.loading="{timeout:200}">保存</yu-button>
...
export default {
  ...
  methods: {
    AfterInit() {
      ...
      _this.setFormItemEditable("refForm", "outOrgAdmitStatus", false);

      // 初始化资质等级隐藏
      _this.setFormItemVisible("refForm", "outRealproAptiLevel;outLandAptiLevel;outAssetAptiLevel;outOtherAptiLevel", false);

      // 默认‘已准入’
      _this.formdata.outOrgAdmitStatus = "01";
    },
    initFormData: function() {
      let _this = this;
      let userInfo = this.$store.state.oauth;
      // _this.formdata.serno = this.formdata.CUS_BLK_SERNO;
      _this.formdata.inputName = userInfo.userName;
      _this.formdata.inputId = userInfo.loginCode;
      _this.formdata.inputBrName = userInfo.org.name;
      ...
      _this.formdata.updDate = _this.$xutils.dateFormat("yyyy-MM-dd hh:mm:ss", new Date());
      _this.formdata.oprType = "01";
      _this.formdata.inputDate = _this.$xutils.dateFormat("yyyy-MM-dd hh:mm:ss", new Date());
    ...
    onSave() {
      ...
      // 组织机构代码数据正确性的校验
      let flag = _this.checkCode(_this.formdata.outOrgCode);

      if (!flag) {
        _this.$confirm("组织机构代码不正确", "提示", {
          confirmButtonText: "确定",
          type: "warning",
          center: true
        });
        _this.formdata.outOrgCode = "";
        return;
      }
      ...
      // 若存在则给出提示
      // 若不存在则可以录入
      let rsPars = _this.querySameRegCode();
      ...
      // 插入数据
      _this.doSaveBillCardData("refForm", _this.$backend.cmisEval + "/api/guarevaloutorgauapp/", null, () => {
        _this.$dialog.close(_this.dialogId);
      });
    },
    ...
    checkCode(code) {
      ...
      let c9 = 11 - (sum % 11);
      c9 = c9 === 10 ? "X" : c9;
      return !(!reg.test(code) || c9 === code.charAt(9));
    },
    ...
    querySameRegCode() {
      ...
      _this
        .$request({
          ...
          async: false,
          method: "post",
          url: _this.$backend.cmisEval + "/api/guarevaloutorgauapp/querySameRegCode",
          data: JSON.stringify(_this.formdata)
        })
        .then(({ code, message, data }) => {
          if (code == "0" && data.data) {
            if (data.data != null) {
              rsPars["data"] = data.data;
            }

            rsPars["handleFlag"] = true;

            if (data.code != "0") {
              _this.$confirm("错误代码：" + data.code + ",错误信息：" + data.message, "提示", {
                confirmButtonText: "确定",
                type: "warning",
                center: true
              });
            ...
            _this.$message.error("错误代码：" + data.code + ",错误信息：" + data.message);
            rsPars["handleFlag"] = false;
          }
        });
