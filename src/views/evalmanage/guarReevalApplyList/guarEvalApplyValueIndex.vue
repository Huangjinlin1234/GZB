...
<script>
import d1Billcard from "./guarEvalApplyValue_d1_BillCard.vue";
/**
 *  押品价值初估认定-业务参数配置列表
 */
...
export default {
  ...
  mounted() {
    this.$lookup.reg("STD_ZB_EVAL_INOUT_TYPE,STD_ZB_CUR_TYP,STD_ZB_APP_ST");
    this.AfterInit();
  },
  methods: {
    /***
    ...
    AfterInit() {
      ...
      if (this.getFactory().contextData.evalAppSerno) {
        this.d1_BillCard.queryFromByCondition(
          ...
          {
            condition: {
              evalAppSerno: this.getFactory().contextData.evalAppSerno
            }
          },
          () => {
            // 暂存数据恢复
    ...
    save() {
      ...
      this.d1_BillCard.formdata.evalApplySerno = this.getFactory().contextData.evalApplySerno;
