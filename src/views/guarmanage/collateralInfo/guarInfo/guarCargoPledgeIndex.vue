<template>
  <div>
    ...
    <yu-form-buttons class="yubfp-button-group" style="text-align: center">
      <yu-button v-if="allowWrite" type="primary" @click="doSave">保存</yu-button>
      <yu-button v-if="allowWrite" type="primary" @click="doSubmit">提交</yu-button>
      <yu-button type="primary" @click="doClose">关闭</yu-button>
    </yu-form-buttons>
  </div>
</template>
<script>
import d1Billcard from './guarCargoPledge_d1_BillCard.vue';
export default {
  ...
  methods: {
    ...
    AfterInit () {
      ...
      this.d1_BillCard.queryDataByCondition({
        serno: this.getFactory().contextData.groupData.serno
      });

      if (this.getFactory().contextData.op == 'ADD') {
        // 新增时回显和设置默认值
        this.d1_BillCard.setItemValue('serno', this.getFactory().contextData.groupData.serno); // 流水号
    ...
    doSubmit () {
      ...
      if (flag) {
        ...
        const rsPars = {
          ...
          is_scan_mater: this.getFactory().contextData.groupData.isScanMater,
          grt_flag: this.getFactory().contextData.groupData.grtFlag,
    ...
    save (params) {
      ...
      this.$xutils.request({
        ...
        type: 'POST',
        url: this.$backend.cmisBiz + '/api/guarinfcargopledge/preserveGuarCargoPledge',
    ...
    checkGuarInfoIsExist (params) {
      ...
      this.$xutils.request({
        ...
        type: 'POST',
        url: this.$backend.cmisBiz + '/api/guarbaseinfo/checkGuarInfoIsExist/' + params.serno,
