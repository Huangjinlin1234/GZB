...
<script>
import d1Billlist from './guarBaseInfoList_d1_BillList.vue';
/* 押品信息管理主界面
 * @author lq
 * */
export default {
  ...
  methods: {
    ...
    onUpdate () {
      ...
      loginUserInfo.roles.every(function (_item, _index) {
        // 如果是小微客户经理 R0010 零售客户经理R0030 虚拟客户经理R0200 小企业客户经理R0050 综合客户经理R0020
        // SystemNo 请求的系统标识 01-信贷系统  02-小贷系统
        if (_item.code == 'R0010' || _item.code == 'R0030' || _item.code == 'R0200') {
          flag = -1;
          return false;
        } else if (_item.code == 'R0020' || _item.code == 'R0050') {
            if (!(params.approveStatus == '000' || params.approveStatus == '992' || params.approveStatus == '' || params.approveStatus == null)) {
               this.$xutils.showMsgBox('提示', '只有待发起状态或打回状态的才能修改!', 350, 150);
                return;
            }
    ...
    onDelete () {
      ...
      if (!(params.approveStatus == '000' || params.approveStatus == '992' || params.approveStatus == null || params.approveStatus == '')) {
        _this.$xutils.showMsgBox(
          ...
          200,
          () => {
            _this.d1_BillList.queryDataByCondition({ oprType: '01' });
          }
        );
        return false;
      }
        ...
        success: (response, status, xhr) => {
          if (response.code == 0) {
            _this.$xutils.showMsgBox('提示', '删除成功', 350, 200, () => {
              _this.d1_BillList.queryDataByCondition({ oprType: '01' }); // opr_type='01'
            });
          } else {
            _this.$xutils.showMsgBox(
              '提示',
