...
export default {
  ...
  methods: {
    ...
    doNext () {
      ...
      opJso['regType'] = this.pageParams.regType;
      if (this.pageParams.pathSource === '02') {
        // 如果路径来源于入库则跳转回入库页面
        // 添加押品信息判断
        var a = [];
        for (let i = 0; i < jsoPar.length; i++) {
          console.log(jsoPar);
          let guarNo = jsoPar[i].guarNo;
          // let checkFlag2 = this.checkGuarStatus(guarNo);
          // if (!checkFlag2) {
          //   return;
          // }
         // let checkFlag3 = this.checkWarrantState(guarNo, this.pageParams.guarContNo);
         // if (!checkFlag3) {
         //   return;
         // }
         // let checkFlag4 = this.checkIsOnTheWay(guarNo);
         // if (!checkFlag4) {
         //   return;
         // }
          a.push(jsoPar[i].grtFlag);
        }
        let proCertiItemVisable = false;
        if (a.indexOf('01') > -1) {
          proCertiItemVisable = true;
        }
        opJso['warrantAppType'] = this.pageParams.warrantAppType;
        opJso['warrantInType'] = this.pageParams.warrantInType;
        opJso['guarContNo'] = this.pageParams.guarContNo;
        opJso['proCertiItemVisable'] = proCertiItemVisable;
        opJso['grtFlag'] = this.pageParams.grtFlag;
        this.onClose();
        this.$dialog.closeAll();
        this.$dialog.open('权证入库新增', 'guarmanage/guarwarrant/guarwarrantin/guarWarrantInAppFormIndex', -1, -1, opJso, () => {
    ...
    // 入库检查该押品是否已入库
    checkWarrantState (guarNo, guarContNo) {
      let returnData = false;
      ...
      this.$xutils.request({
        ...
        async: false,
        type: 'POST',
        url: this.$backend.cmisBiz + '/api/guarwarrantmanageapp/checkwarrantstate',
        data: {condition: JSON.stringify({guarNo: guarNo, guarContNo: guarContNo})},
