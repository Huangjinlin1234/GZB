<template>
  <yu-tabs  type="card">
    <yu-tab-pane label="权证入库申请">
  <d1-billlist ref="d1_BillList"></d1-billlist>
    </yu-tab-pane>
    <yu-tab-pane label="权证入库历史">
  <d2-billlist ref="d2_BillList"></d2-billlist>
    </yu-tab-pane>
  </yu-tabs>
</template>
<script>
import d1Billlist from './guarWarrantInAppList_d1_BillList.vue';
import d2Billlist from './guarWarrantInAppList_d2_BillList.vue';
/** 权证入库申请列表**/
let params;
...
export default {
  ...
  methods: {
    ...
    doAdd () {
      var opJso = {};
      opJso['warrantAppType'] = '01';
      opJso['type'] = 'ADD';
      this.$dialog.open('权证入库', 'guarmanage/guarwarrant/guarwarrantin/guarWarrantInAppAddIndex', 800, 300, opJso, () => {
    ...
    doUpdate () {
      ...
      if (jsoPar.approveStatus !== '000' && jsoPar.approveStatus !== '992' && jsoPar.approveStatus !== '') {
        this.$xutils.showMsgBox('提示', '只有"待发起"或"退回"状态的权证入库申请记录才能进行修改操作！', 350, 150);
        return;
      }
      var opJso = {};
      opJso['serno'] = jsoPar.serno;
      opJso['guarNo'] = jsoPar.guarNo;
      opJso['warrantInType'] = jsoPar.warrantInType;
      opJso['warrantAppType'] = '01';
      opJso['type'] = 'EDIT';
      opJso['grtFlag'] = jsoPar.grtFlag;
      this.$dialog.open('权证入库修改', 'guarmanage/guarwarrant/guarwarrantin/guarWarrantInAppFormIndex', -1, -1, opJso);
    },
    ...
    doRighting () {
      ...
      //  this.$xutils.showMsgBox('提示', '只有"通过"状态的权证入库申请记录才能进行冲正操作！', 350, 150);
      //  return;
      }

      if (jsoPar.warrantInType !== '03'){
      //  this.$xutils.showMsgBox('提示', '只有"纸质权证柜面入库模式"的权证入库申请记录才能进行冲正操作！', 350, 150);
      //  return;
      }
      ...
      this.$xutils.request({
        ...
        url:
          ...
          '/api/guarwarrantmanageapp/warrantinrighting/' +
          jsoPar.serno,
    ...
    doDelete () {
      ...
      if (selectRow.approveStatus !== '' && selectRow.approveStatus == '992') {
        var _this = this;
      ...
      if (selectRow.approveStatus !== '' && selectRow.approveStatus !== '000') {
        this.$xutils.showMsgBox('提示', '只有"待发起"状态的权证入库申请记录才能进行删除操作！', 350, 150);
        return;
      }
    ...
    doView () {
      ...
      opJso['guarNo'] = jsoPar.guarNo;
      opJso['type'] = 'VIEW';
      this.$dialog.open('权证入库详情', 'guarmanage/guarwarrant/guarwarrantin/guarWarrantInAppFormIndex', -1, -1, opJso);
    },
