...
<template>
    <div>
      <div title="新页签中打开" class="imageFullScreen">
        <yu-button @click="openNewTabFn()" type="danger" icon="yu-icon-full-screen"   ></yu-button>
      </div>
...
export default {
  ...
  methods: {
    openNewTabFn () {
      window.open(this.imageSrc, 'newWindow', 'titlebar=no,menubar=no,scrollbar=yes,status=no,toolbar=no,location=no');
      // this.imageSrc = window.encodeURI('');
    },
    // imageLogin () {
    //   this.$request({
    //     method: 'POST',
    ...
    getImageSrc () {
      ...
      //   para: this.para || this.pageParams.para || this.getFactory().contextData.img_para,
      //   // authorization: tokenVal, // Token值
      //   s: this.s || this.pageParams.s,
      //   authority: this.authority || this.pageParams.authority || this.getFactory().contextData.img_authority
      // };
      // this.$request({
      //   method: 'POST',
      ...
      //     // 是否窗口打开
      //     if (this.isWindow == '1'){
      //       this.imageSrc = window.open(response.data, "newWindow", 'titlebar=no,menubar=no,scrollbar=yes,status=no,toolbar=no,location=no');
      //       this.imageSrc = window.encodeURI("");
      //     }else if(this.isWindow == '0'){
