...
<template>
  <div id="roleManage">
    ...
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible" size="tiny">
      <yu-xform ref="roleForm" v-model="formdata" :form-type="formType" label-width="100px" :rules="formRules">
        <yu-xform-group :column="1">
          ...
          <yu-xform-item :label="$t('roleManage.ssjg')" name="orgName" ctype="yufp-search-tree" :params="treeProps" :select-value="selectValue" :details="viewType==='DETAIL'" :details-value="detailsValue" @node-click="changeOrgFn"></yu-xform-item>
    ...
    <yu-xdialog :title="$t('roleManage.jsyh')" :visible.sync="userDialogVisible" size="small" class="role-user-xdialog">
      ...
      <!-- 已关联用户列表 -->
      <yu-xtable ref="userTable" row-number :data-url="userTableUrl" selection-type="checkbox" :base-params="userTableParams" :default-load="false" @select="selectFn($event, 'userSelected')">
...
export default {
  ...
  data() {
    return {
      ...
      treeUrl: backend.appOcaService + '/api/adminsmorg/orgtreequery', // 角色列表机构树请求 url
    ...
    saveRoleFn() {
      ...
      _this.$request({
        ...
        if (code === '0') {
          ...
          _this.$refs.rolesTable.remoteData();
        } else {
          _this.$message({
            message: message,
            type: 'error'
