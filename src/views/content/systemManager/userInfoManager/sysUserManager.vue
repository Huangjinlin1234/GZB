    ...
    <yu-panel ref="panel" :title="$t('sysUserManager.yhgl')" class="adjust-height" show-search-input :placeholder="$t('sysUserManager.gjz')" @search="fuzzyQuery">
      ...
      <template slot="right">
        <yu-toolBar>
          ...
          <yu-button v-if="checkCtrl('refreshCache')" v-norepeat.disabled @click="refreshCacheFn">{{ $t('sysUserManager.gxhc') }}</yu-button>
          <yu-button v-if="checkCtrl('refreshCache')" v-norepeat.disabled @click="refreshAllCacheFn">刷新全量系统缓存</yu-button>
        </yu-toolBar>
      </template>
      <!-- 快速查询-机构查询 -->
      ...
      <!--用户列表-->
      <yu-xtable ref="usersTable" row-number :data-url="tableUrl" selection-type="checkbox">
        <yu-xtable-column :label="$t('sysUserManager.yhmc')">
        ...
        <yu-xtable-column :label="$t('sysUserManager.sszjg')" prop="belgMainOrgId"></yu-xtable-column>
        ...
        <yu-panel :title="$t('sysUserManager.gjxx')" panel-type="simple" is-collapse :collapse-title="collapseTitle">
          <yu-row>
            <yu-col :span="12">
              ...
              <yu-xform-item :label="$t('sysUserManager.sszjg')" name="belgMainOrgId" ctype="yufp-search-tree" :params="treeParams" :select-value="selectBelgValue" :details="viewType==='DETAIL'" :details-value="detailsBelgValue" @node-click="changeBelgOrgFn"></yu-xform-item>
...
export default {
  ...
  */
      ...
      _this.$request({
        ...
        if (code === '0') {
          ...
          _this.$nextTick(function () {
            ...
            _this.selectBelgValue = _this.formdata.belgMainOrgId;
      ...
      _this.$request({
        ...
        if (code === '0') {
          ...
          _this.$nextTick(function () {
            ...
            _this.selectValue = _this.formdata.orgId;
            _this.belgMainOrgId = _this.formdata.belgMainOrgId;
            _this.selectedDptId = _this.formdata.dptId;
            _this.detailsValue = _this.formdata.orgName;
            _this.detailsBelgValue = _this.formdata.belgMainOrgName;
    ...
    */
    ...
    changeBelgOrgFn: function (node) {
      this.formdata.belgMainOrgId = node.orgId;
      this.selectBelgValue = node.orgId;
      // 部门树联动
      var temp = extend(true, {}, this.dptTreeParams);
      temp.dataParams = {belgMainOrgId: node.orgId, dptSts: 'A'};
      this.dptTreeParams = temp;
    },
    /**
    * 选择所属部门
    ...
    */
    refreshCacheFn: function () {
      ...
      _this.$request({
        method: 'GET',
        url: backend.appOcaService + '/api/userredis'
      }).then(({code, message, data}) => {
        if (code === '0') {
          _this.$message({
            message: _this.$t('sysUserManager.hcsxcg'),
            type: 'success'
          });
          _this.$refs.orgTable.remoteData();
        } else {
          _this.$message({
            message: message || _this.$t('sysUserManager.hcsxsb'),
            type: 'error'
          });
        }
      });
    },
    refreshAllCacheFn: function () {
      var _this = this;
      _this.$request({
        method: 'GET',
        url: backend.appOcaService + '/api/all/systemredis'
      }).then(({code, message, data}) => {
        if (code === '0') {
          _this.$message({
            message: _this.$t('sysUserManager.hcsxcg'),
