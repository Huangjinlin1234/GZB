      ...
      <yu-xtable ref="strategyTable" row-number :data-url="strategyTableUrl" :pageable="false" @loaded="loadedFn">
        ...
        <yu-xtable-column :label="$t('logicSysManager.clfl')" width="120">
          <template slot-scope="scope">
            ...
            <span v-else>{{ $t('logicSysManager.mmcl') }}</span>
          </template>
        </yu-xtable-column>
        <yu-xtable-column :label="$t('logicSysManager.sfqy')" width="200">
          <template slot-scope="scope">
            <yu-switch v-model="scope.row.enableFlag" on-value="1" off-value="2" :on-text="$t('logicSysManager.ty')" :off-text="$t('logicSysManager.qy')" @change="enableFn(scope.row)"></yu-switch>
          </template>
        </yu-xtable-column>
