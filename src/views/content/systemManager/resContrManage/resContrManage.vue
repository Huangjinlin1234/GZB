...
<template>
  <div class="control-Manage">
      ...
      <template slot="filter">
        <yu-xform v-model="searchFormdata" related-table-name="pointTable" ref="searchForm" form-type="search" @form-reset="resetFn">
          <yu-xform-group :column="4">
            ...
            <yu-xform-item :label="$t('resContrManage.cd')" name="menuId" ctype="custom">
              <yu-combo-tree
    ...
    <yu-xdialog :title="dialogTitle" :visible.sync="dialogVisible">
      <yu-xform ref="pointForm" label-width="120px" v-model="pointFormData" :form-type="formType" :rules="pointrRules">
    ...
    <yu-xdialog :title="$t('resContrManage.xzcd')" :visible.sync="menuDialogVisible" height="540px" class="control-dialog">
      <div class="dialog-search">
        <yu-checkbox v-model="checkMenu" @change="checkMenuChangeFn">{{ $t('resContrManage.jkyxz') }}</yu-checkbox>
        <yu-input
          v-model="menuVal"
      ...
      <yu-xtable
        ...
        @current-change="menuCurChangeFn"
        :default-load="false">
        <yu-xtable-column prop="menuName" :label="$t('resContrManage.cdmc')"></yu-xtable-column>
    ...
    <!-- 选择数据模板dialog -->
    <yu-xdialog :title="$t('resContrManage.xzsjmb')" :visible.sync="authDialogVisible" height="540px" class="control-dialog">
      <div class="dialog-search">
        <yu-checkbox v-model="checkAuthTmpl" @change="checkAuthChangeFn">{{ $t('resContrManage.jkyxz') }}</yu-checkbox>
        <yu-input
          v-model="authVal"
          :placeholder="$t('resContrManage.gjz')"
          suffix-icon="yu-icon-search1"
...
export default {
  ...
  methods: {
    ...
    */
    ...
    /**
    * 选择菜单当前行发生变化的时候会触发的事件
    * @param param curRow oldCurRow
    */
    menuCurChangeFn (curRow, oldCurRow) {
      curRow && this.$refs.menuTable.toggleRowSelection(curRow);
    },
    ...
    pointDeleteFn (row) {
      ...
      _this.$confirm(_this.$t('resContrManage.sckzdjscqsjqxjsqsjqdsc'), _this.$t('resContrManage.ts'), {
        ...
        _this.$request({
          ...
          if (code === '0') {
            _this.$message({type: 'success', message: _this.$t('resContrManage.sccg') });
            _this.$refs.pointTable.remoteData();
          } else {
            _this.$message({ message: message, type: 'error' });
          }
