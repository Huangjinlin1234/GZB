    ...
    <yu-panel ref="panel" :title="$t('dptManager.bmgl')" :placeholder="$t('dptManager.gjz')" show-search-input @search="fuzzyQuery" class="adjust-height">
      ...
      <template slot="right">
        <yu-toolBar>
          ...
          <yu-button v-if="checkCtrl('enable')" v-norepeat.disabled @click="useFn">{{ $t('dptManager.qy') }}</yu-button>
          <yu-button v-if="checkCtrl('disable')" v-norepeat.disabled @click="stopFn">{{ $t('dptManager.ty') }}</yu-button>
    ...
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible" size="small" @close="dialogCloseFn">
      <yu-xform ref="dptForm" v-model="formdata" :form-type="formType" label-width="120px" :rules="formRules">
        <yu-xform-group columns="4">
          ...
          <yu-xform-item :label="$t('dptManager.zxbgyh')" name="lastChgName" :hidden="viewType === 'ADD'" :disabled="true"></yu-xform-item>
          <yu-xform-item :label="$t('dptManager.zxbgsj')" name="lastChgDt" :hidden="viewType === 'ADD'" :disabled="true"></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
...
export default {
  data() {
    return {
      ...
      orgId: '', // 快速搜索 机构树
      orgTreeUrl: backend.appOcaService + '/api/adminsmorg/orgtreequery', // 机构树请求 url
      ...
      formRules: {
        ...
        dptSts: [{ required: true, message: this.$t('dptManager.qxzbmzt') }]
      },
  ...
  methods: {
      ...
      _this.$confirm(_this.$t('dptManager.cczjscgbmxxsfjx'), _this.$t('dptManager.ts'), {
        ...
        //处理请求成功的情况
          if (code === '0') {
            ...
            _this.$refs.dptTable.remoteData();
          } else {
            _this.$message({
    ...
    addSubDpt(row) {
      if (row.dptSts === 'I') {
        this.$message({
          message: this.$t('dptManager.tydbmbkxzzjg'),
          type: 'warning'
        });
        return;
      }
