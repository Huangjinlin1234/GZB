    ...
    <yu-panel :title="$t('schedule.dsrw')" show-search-input :collapse-hide="false" :placeholder="$t('schedule.beanmc')" @search="fuzzyQueryFn" ref="panelRef">
      <template slot="right">
        <yu-toolBar>
          ...
          <yu-button v-if="checkCtrl('edit')" @click="scheduleEditFn">{{ $t('schedule.xg') }}</yu-button>
          ...
          <yu-button @click="scheduleLogFn" v-norepeat.disabled type="danger">{{ $t('schedule.rzlb') }}</yu-button>
        </yu-toolBar>
      </template>
    ...
    <yu-xdialog :title="dialogTitle" :visible.sync="dialogVisible" size="tiny">
      <yu-xform ref="scheduleForm" label-width="148px" v-model="scheduleFormData" :rules="scheduleRules">
        <yu-xform-group :column="1">
          ...
          <yu-xform-item :label="$t('schedule.bz')" name="remark" :placeholder="$t('schedule.bz')"></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
    ...
    */
    ...
    scheduleEditFn () {
      ...
      this.$nextTick(function () {
        this.$refs.scheduleForm.setFormData(selections[0])
      })
    },
    ...
    saveFn () {
      ...
      this.$request({
        ...
        if (code === '0') {
          ...
          _this.$refs.scheduleTable.remoteData();
