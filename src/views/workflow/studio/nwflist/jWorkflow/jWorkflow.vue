...
<template>
  ...
  <div class="yu-workflow" style="margin-left: -10px;margin-right: -10px;">
    <yu-workflow ref="refWorkflow" @init="initGraph" :lock="locked" :height="height" @scale="scaleFn" @addedge="addEdgeFn" :param="comments" @cellclick="cellclickFn">
      <yu-xdialog title="XML预览" :visible.sync="dialogVisible" slot="dialog">
        <yu-row>
          <yu-col>
        ...
        <div slot="footer">
          <yu-button @click="dialogClose">返回</yu-button>
        </div>
      </yu-xdialog>
        ...
        <div class="geToolbar jWorkflow">
          <yu-row>
            ...
            <yu-col :span="10">
              ...
              <a class="geItem">
                <yu-select placeholder="请选择" v-model="viewScale" class="custom-select" @change="graphViewSelectChangeFn" :clearable="false">
                  <yu-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                  </yu-option>
                </yu-select>
              </a>
              <a class="geItem abbrImg" :title="'缩略图'" style="background-color: #ffffff;padding: 2px 6px 0;margin: 3px 12px 0px 6px;border-radius: 3px;border: 1px solid #D3DCE6;" @click="toolbarItemClickFn('outline')">
          ...
          <yu-collapse v-model="activeName">
            <yu-collapse-item name="0">
              ...
              <yu-xform-group :column="1">
                <yu-xform-item v-for="item in baseList" :key="item.key" :label="item.name" :ctype="getFormItemCtype(item.type)"
                               :readonly="item.readOnly === true" :disabled="item.readOnly === true" :name="item.key"
                               :value="item.defaultValue || ''" :options="item.value" :required="item.required" :hidden="item.hidden">
                </yu-xform-item>
              </yu-xform-group>
            </yu-collapse-item>
            <yu-collapse-item name="1" v-if="tmpCell && tmpCell.data.nodeType !='S' && tmpCell.data.nodeType !='E' && checkHiddenItem('selectable')">
              ...
              <yu-xform-group :column="1">
                <yu-xform-item v-for="item in formItems.selectable" :key="item.key" :label="item.name"
                               :ctype="getFormItemCtype(item.type)" :readonly="item.readOnly === true "
                               :value="item.defaultValue || ''"
                               :disabled="item.readOnly === true ? (item.key != 'nodeScriptTxt' && item.key != 'subFlowId'):false"
                               :hidden="item.hidden" :name="item.key" :options="item.value">
                </yu-xform-item>
              </yu-xform-group>
            </yu-collapse-item>
            <yu-collapse-item name="2" v-if="tmpCell && tmpCell.data.nodeType !='S' && tmpCell.data.nodeType !='E' && checkHiddenItem('oper')">
              ...
              <yu-xform-group :column="1">
                <yu-xform-item v-for="item in formItems.oper" :key="item.key" :label="item.name" :ctype="getFormItemCtype(item.type)"
                               :readonly="item.readOnly === true" :disabled="item.readOnly === true" :name="item.key"
                               :value="item.defaultValue || ''" :options="item.value" :hidden="item.hidden">
                </yu-xform-item>
              </yu-xform-group>
            </yu-collapse-item>
          </yu-collapse>
        ...
        <yu-xform v-if="formatType === 'edge'" ref="refFormedge" label-width="80px" v-model="formdataedge">
          <yu-collapse v-model="activeName">
            <yu-collapse-item name="0">
              ...
              <yu-xform-group :column="1">
                <yu-xform-item v-for="item in formItems.edge" :key="item.key" :label="item.name" :ctype="getFormItemCtype(item.type)"
                               :readonly="item.readOnly === true" :disabled="item.readOnly === true && item.key != 'routeScriptTxt'"
                               :name="item.key" :value="item.defaultValue || ''" :options="item.value"></yu-xform-item>
              </yu-xform-group>
            </yu-collapse-item>
          </yu-collapse>
        </yu-xform>
        <yu-xform v-if="formatType === 'workflow'" ref="refFormworkflow" label-width="80px" v-model="formdataworkflow">
          <yu-collapse v-model="activeName">
            <yu-collapse-item name="0">
              ...
              <yu-xform-group :column="1">
                <yu-xform-item v-for="item in formItems.wf" :key="item.key" :label="item.name" :ctype="getFormItemCtype(item.type)"
                               :readonly="item.readOnly === true" :disabled="item.readOnly === true" :name="item.key"
                               :value="item.defaultValue || ''" :options="item.value"></yu-xform-item>
              </yu-xform-group>
            </yu-collapse-item>
          </yu-collapse>
        </yu-xform>
    ...
    dealNodeProperty: function () {
      ...
      if (VERTEX_PRO) {
        for (var k in VERTEX_PRO) {
          tmp = yufp.clone(VERTEX_PRO[k], {}); // 将select数据转换
      ...
      if (EDGE_PRO) {
        for (var k in EDGE_PRO) {
          tmp = yufp.clone(EDGE_PRO[k], {}); // 将select数据转换
      ...
      if (WORKFLOW_PRO) {
        for (var k in WORKFLOW_PRO) {
          tmp = yufp.clone(WORKFLOW_PRO[k], {}); // 将select数据转换
