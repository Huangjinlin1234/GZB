...
<template>
  <div>
    <yu-panel title="贷审会投票列表" :collapse-hide="false">
      <yu-xform v-model="formdata" related-table-name="reftable" form-type="search">
        <yu-xform-group :column="4">
          ...
          <yu-xform-item label="投票结果" placeholder="投票结果" ctype="select" name="voteSts" :options="mettingStsOptions"></yu-xform-item>
...
import {mettingVote} from '@/utils/unitchange';
export default {
  data: function () {
    var _this = this;
    return {
    ...
    customRowOp: function (scope, op) {
      ...
      if (op == 'vote') {
        if (scope.row != null) {
          ...
          debugger
          var bizType = scope.row.bizType;
          if(bizType != null && bizType.startsWith('TY')){
              // 用于判断重新定向 页面挂载的路径  2021-9-26-cj
              let obj = mettingVote(scope.row.bizType);
              scope.row.bizPage = obj.bizPage;
              scope.row.nodeId = obj.nodeId;
              scope.row.tableName = obj.tableName;
          }
