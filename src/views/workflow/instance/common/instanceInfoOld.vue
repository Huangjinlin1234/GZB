      ...
      <el-tab-pane label="流程审批" name="second" v-if="showApproveTab">
        ...
        <el-card class="clearfix">
          ...
          <el-form ref="commentForm" label-width="160px" :model="commentInfo" :rules="commentRules" style="margin-top:20px">
            ...
            <el-form-item label="是否大额业务授信备案" v-if="isDAELmtApprShow!=''" prop="isDAELmtApprWeb">
                <yu-xselect v-model="isDAELmtAppr" data-code="STD_ZB_YES_NO" placeholder="请选择" :rules="isDAELmtApprShow"></yu-xselect>
            </el-form-item>
            <!--hxl增加对资料是否齐全进行判断-->
            <el-form-item label="资料是否提交齐全" prop="isinforcomplete" v-if="rule5!=''" >
                <yu-xselect v-model="commentInfo.isinforcomplete" data-code="STD_ZB_YES_NO" placeholder="请选择"></yu-xselect>
            </el-form-item>
            <el-form-item label="是否用印" prop="issignet" v-if="rule6!=''" >
                <yu-xselect v-model="commentInfo.issignet" data-code="STD_ZB_YES_NO" placeholder="请选择"></yu-xselect>
            </el-form-item>
            ...
            <el-form-item label="审批退回原因" v-if="showRoolReason" prop="ext" style="height: 70px;">
                <yu-xcheckbox v-model="commentInfo.ext" :options="roolBackReasonTypeOptions"></yu-xcheckbox>
            </el-form-item>
...
export default {
  ...
  data: function () {
    ...
    return {
      ...
      roolBackReasonTypeOptionZX: [{
        ...
        key: 'ZXR002',
        value: '系统录入信息与纸质查询授权书要素不一致'
      }, {
        key: 'ZXR003',
        value: '纸质查询授权书授权条款、被查询人签名（签章）、授权时间、双人免签等要素填写不全'
      }, {
        key: 'ZXR004',
        value: '纸质查询授权书上有涂改、要素填写不规范'
      }, {
        key: 'ZXR005',
        value: '实际查询日期先于签署的授权日期'
      }, {
        key: 'ZXR006',
        value: '身份证明材料不全或无效'
      }, {
  ...
  methods: {
    ...
    nodeIdHandle () {
      ...
      this.instanceId = this.bizPageData.instanceInfo.instanceId;
      var isLowerApprUrl = '';
      var isLowerAppr = false;
      if (bizType == 'TY004' || bizType == 'TY005' || bizType == 'TY006') {
        if (nodeId == '137_139') {
          isLowerApprUrl = '/api/lmtintbankappr/updateUpApprAuth';
          isLowerAppr = true;
        }
      } else if (bizType == 'TY011' || bizType == 'TY012' || bizType == 'TY013') {
        if (nodeId == '137_139') {
          isLowerApprUrl = '/api/lmtsiginvestappr/updateUpApprAuth';
          isLowerAppr = true;
        }
      ...
      if (isLowerAppr) {
        this.isLowerApprUrl = isLowerApprUrl;
        this.isUpperAppr = '0';
        this.isUpperAuthShow = [
          {
            required: true,
            message: '必填项',
            trigger: 'blur'
    ...
    chooseCommentSign: function (data) {
      ...
      if (data == 'O-1') {
        ...
        if (this.isAuthorizer && isNeedRoolbackReason) {
          _this.commentInfo.ext = ''; // 修改默认值为空，否则会通过公共必输判断
      ...
      if (data == 'O-12') {
        // TODO hxl待定
        if (bizType == 'HZ001' || bizType == 'HZ002' || bizType == 'HZ005' || bizType == 'HZ006' || bizType == 'BG005' || bizType == 'BG008' || bizType == 'BG015' || bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'SX007' || bizType == 'SX008' || bizType == 'SX009' || bizType == 'SX010' || bizType == 'SX011' || bizType == 'SX012' || bizType == 'SX013' || bizType == 'SX014' || bizType == 'SX015' || bizType == 'YX010' || bizType == 'YX011' || bizType == 'YX013' || bizType == 'YX012' || bizType == 'LS005' || bizType == 'LS006' || bizType == 'LS007' || bizType == 'XW002') {
          if (nodeId == '305_25' || nodeId == '307_21' || nodeId == '307_23' || nodeId == '256_105' || nodeId == '256_91' || nodeId == '243_6' || nodeId == '243_7' || nodeId == '244_8' || nodeId == '244_9' || nodeId == '245_9' || nodeId == '245_11' || nodeId == '232_7' || nodeId == '232_8' || nodeId == '233_6' || nodeId == '229_5' || nodeId == '296_5' || nodeId == '233_7') {
            _this.rule5 = "yes"
          }
        }
    ...
    tabClick: function (tab, event) {
      ...
      if (tab.name == 'second') {
        ...
        let meetId = _this.getLastMeetingId();
        if (meetId) {
    ...
    getLastMeetingId () {
      ...
      yufp.service.request({
        method: 'GET',
        data: {condition: JSON.stringify(params), sort: 'start_time desc'},
        async: false,
        url: backend.workflowService + '/api/bench/meetting/query',
        callback: function (code, message, response) {
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          if (response.code == 0) {
            ...
            _this.isBizType = _this.instanceIdInfo.bizType;
            _this.getOptions();
            ...
            _this.$nextTick(function () {
              try {
                ...
                if (bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'SX007' || bizType == 'SX008' || bizType == 'SX009' || bizType == 'SX010' || bizType == 'SX011' || bizType == 'SX012' || bizType == 'SX013' || bizType == 'SX014' || bizType == 'SX015' ||
                bizType == 'DHC01' || bizType == 'DHC02' || bizType == 'DHC03' || bizType == 'DHC04' || bizType == 'DHC05' || bizType == 'DHC06' || bizType == 'DHC07' || bizType == 'DHC08' || bizType == 'DHC09' || bizType == 'DHC10' || bizType == 'DHC11' || bizType == 'DHC12' || bizType == 'DHC13' || bizType == 'DHC14' ||
                bizType == 'SGC01' || bizType == 'SGC02' || bizType == 'SGC03' || bizType == 'SGC04' || bizType == 'SGC05' || bizType == 'SGC06' || bizType == 'SGC07' || bizType == 'SGC08' || bizType == 'SGC09' || bizType == 'SGC10' || bizType == 'SGC11' || bizType == 'SGC12' || bizType == 'SGC13' || bizType == 'SGC14') {
                  // 公司部是否提交其他部门审批
                  if (nodeId == '256_52') {
                  ...
                  if (nodeId == '256_105' || nodeId == '256_91') {
                    var apprMode = paramT.instanceInfo.flowParam.apprMode;
                    if (apprMode == '03') {
                  ...
                  // TODO 东海村镇流程
                  if (nodeId == '683_4' || nodeId == '683_5' || nodeId == '683_6' || nodeId == '683_7' || nodeId == '683_8' || nodeId == '683_9' || nodeId == '683_10' || nodeId == '683_11' || nodeId == '683_13' || nodeId == '683_34') {
                  // 单一审查
                  ...
                  // TODO  寿光村镇授信
                  if (nodeId == '643_4' || nodeId == '643_5' || nodeId == '643_6' || nodeId == '643_7' || nodeId == '643_8' || nodeId == '643_9' || nodeId == '643_10' || nodeId == '643_11' || nodeId == '643_13' || nodeId == '643_33') {
                  // 单一审查
                    _this.islmtReplyApproShow = true;
                    try {
                      _this.lmtReplyApproPage = function () {
                ...
                if (bizType == 'SX008' || bizType == 'SX009' || bizType == 'SX010' || bizType == 'SX011' || bizType == 'SX012' || bizType == 'SX013' || bizType == 'SX014' || bizType == 'SX015') {
                  if (nodeId == '256_91' || nodeId == '256_105' || nodeId == '256_134' || nodeId == '256_151' || nodeId == '256_2' || nodeId == '256_167' || nodeId == '256_202' || nodeId == '256_169' || nodeId == '256_183' || nodeId == '256_187' || nodeId == '256_205' || nodeId == '256_208') {
                    ...
                    try {
                      _this.lmtReplyApproMeetingPage = function () {
                        return Promise.resolve(
                          require('@/views/bizmanage/lmtBiz/lmtGrpApprBiz/lmtGrpApprInspectGrpReport.vue')
                            .default
                        );
                      };
                      _this.lmtReplyApproMeetingPage.instanceInfo = _this.instanceIdInfo;
    ...
    getOptions: function () {
      ...
      if (this.button['signIn'] !== '0') {
        this.tabValue = 'third';
        console.log('isBizType====>', this.isBizType);
        if (this.isBizType.indexOf('TY0') == 0) {
          this.tabValue = 'first';
        }
      }
    ...
    submitFn: function (opType) {
      ...
      var nodeId = this.bizPageData.instanceInfo.nodeId;
      var serno = this.bizPageData.instanceInfo.bizId;
      var isinforcomplete = this.commentInfo.isinforcomplete;
      var issignet = this.commentInfo.issignet;
      var url;
      var _this = this;
      var volidate = true;
      // 协查进入时，校验协查人意见必填 ---王友红
      if (bizType == 'SGK01' || bizType == 'DHK01' || bizType == 'DH001') {
        yufp.service.request({
          method: 'POST',
          async: false,
          data: {taskNo: serno},
          ...
          callback: function (code, message, response) {
            if (response.code == 0) {
              if (response.data.assistComment) {
                // 校验通过
                volidate = true;
              } else {
                // _this.$message.error("请先填写协查人意见");
                volidate = false;
              }
            } else {
              volidate = false;
      ...
      if (!volidate) {
        _this.tabValue = 'first';
        return _this.$message.error('请先填写协查人意见');
      }
      // 审批结论为 同意时
      if (opType == 'O-12') {
        // 零售优惠利率提交更新路由参数
        if (bizType == 'QT001') {
          yufp.service.request({
            method: 'POST',
            async: false,
            data: this.bizPageData.instanceInfo,
            url: backend.cmisBiz + '/api/retailprimerateapp/updateflowparam',
            callback: function (code, message, response) {
              _this.bizPageData.instanceInfo.param.powerFlag = response.data;
              if (response.code != 0) {
                _this.$message({ message: response.message, type: 'error' });
              }
            }
        ...
        // TYSX02  _31 信贷管理部负责人
        if (_this.isUpperAuthShow) {
          var params = {
            serno: serno,
            isUpperApprAuth: _this.isUpperAppr,
            instanceId: _this.instanceId
          };
          limitAuthority(_this.isLowerApprUrl, params);
        } else if (bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'SX007') {
          // 单一授信流程处理
          // 获取敞口金额
          let openTotalLmtAmt = JSON.parse(this.bizPageData.instanceInfo.flowParam).openTotalLmtAmt;
          if (nodeId == '256_52' || nodeId == '256_91' || nodeId == '256_105' || nodeId == '256_205' || nodeId == '256_186') {
            url = '/api/lmtappr/updatelmtapprchoose';
            updateApprChoose(url, serno);
          } else if ((nodeId == '256_9') || (nodeId == '256_20') || (nodeId == '256_144' && parseFloat(openTotalLmtAmt) <= parseFloat(10000000)) || (nodeId == '256_39')) {
            // 省心快贷线上流程 发送接口
            url = '/api/lmtapp/judgeSxkdOnlineAppr';
            judgeSxkdOnlineAppr(url, serno);
          } else {
            submitFun();
          }
          // 保存 资料是否齐全字段
          if ((bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'SX007') && (nodeId == '256_105' || nodeId == '256_91')) {
            ...
            yufp.service.request({
              method: 'POST',
              data: params,
              url: backend.cmisBiz + '/api/businessinformation/updateinfComplete',
              callback: function (code, message, response) {
                if (response.code == 0) {
                  // 修改资料是否齐全字段
                } else {
        ...
        } else if (bizType == 'HZ001' || bizType == 'HZ002' || bizType == 'HZ005' || bizType == 'HZ006' || bizType == 'HZ018' || bizType == 'BG005' || bizType == 'BG008' || bizType == 'BG015' || bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'YX010' || bizType == 'YX011' || bizType == 'YX013' || bizType == 'YX012' || bizType == 'LS005' || bizType == 'LS006' || bizType == 'LS007' || bizType == 'XW002') {
          if (nodeId == '305_25' || nodeId == '307_21' || nodeId == '307_23' || nodeId == '243_6' || nodeId == '243_7' || nodeId == '244_8' || nodeId == '244_9' || nodeId == '245_9' || nodeId == '245_11' || nodeId == '232_7' || nodeId == '232_8' || nodeId == '233_6' || nodeId == '229_5' || nodeId == '296_5' || nodeId == '233_7') {
            url = '/api/businessinformation/updateinfComplete';
            toinfComplete(url, serno, bizType, isinforcomplete, issignet);
          } else {
            //  合作方项目准入变更 贷审会秘书岗在开完贷审会后必须出具意见后才可以提交下一岗
            if (bizType == 'HZ001' || bizType == 'HZ002') {
              if (nodeId == '305_57' || nodeId == '306_37') {
                var partnerType = this.bizPageData.instanceInfo.param.partnerType;
                if (partnerType == '2' || partnerType == '4' || partnerType == '8' || partnerType == '1' || partnerType == '3' || partnerType == '5' || partnerType == '15') {
                  // 合作方流程 判断必填项目并且
                  const pfInfoComplete = window.callCoopPlanAppFlowValidate();
                  if (pfInfoComplete) {
                    submitFun();
                  } else {
                    this.$message({ message: '请先完成业务信息填写并保存', type: 'warning'});
                    return false;
                  }
                } else {
                  submitFun();
        ...
        } else if (bizType == 'HZ003' || bizType == 'HZ004') {
          //  合作方项目准入变更 贷审会秘书岗在开完贷审会后必须出具意见后才可以提交下一岗
          if (nodeId == '305_57' || nodeId == '306_37') {
            var partnerType = this.bizPageData.instanceInfo.param.partnerType;
            if (partnerType == '2' || partnerType == '4' || partnerType == '8' || partnerType == '1' || partnerType == '3' || partnerType == '5' || partnerType == '15') {
              // 合作方流程 判断必填项目并且审批结论等已保存
              const pfInfoComplete = window.callCoopPlanAppFlowValidate();
              if (pfInfoComplete) {
                submitFun();
              } else {
      ...
      function updateApprChoose (url, serno) {
        // 流程路由参数
        _this.instanceIdInfo.param.isCommitOtherDeptGSB = _this.isCommotOtherGSB == '' ? '0' : _this.isCommotOtherGSB;
        _this.instanceIdInfo.param.commitDeptTypeGSB = _this.commotOtherDeptGSB;
        _this.instanceIdInfo.param.isCommitOtherDeptXDGLB = _this.isCommotOtherSXB == '' ? '0' : _this.isCommotOtherSXB;
        _this.instanceIdInfo.param.commitDeptTypeXDGLB = _this.commotOtherDeptSXB;
        _this.instanceIdInfo.param.isUpAppr = _this.isUpperAppr == '' ? '0' : _this.isUpperAppr;
        _this.instanceIdInfo.param.upApprType = _this.upperApprAuth;
        _this.instanceIdInfo.param.isDownAppr = _this.isLowerAppr == '' ? '0' : _this.isLowerAppr;
        _this.instanceIdInfo.param.isDAELmtAppr = _this.isDAELmtAppr == '' ? '0' : _this.isDAELmtAppr;
        _this.instanceIdInfo.param.approveResult = _this.approveResult == '' ? '0' : _this.approveResult;
        var params = {
          serno: serno,
          isSubToOtherDeptCom: _this.isCommotOtherGSB == '' ? '0' : _this.isCommotOtherGSB,
          subOtherDeptCom: _this.commotOtherDeptGSB,
          isSubToOtherDeptXd: _this.isCommotOtherSXB == '' ? '0' : _this.isCommotOtherSXB,
          subOtherDeptXd: _this.commotOtherDeptSXB,
          isUpperApprAuth: _this.isUpperAppr == '' ? '0' : _this.isUpperAppr,
          upperApprAuthType: _this.upperApprAuth,
          isLowerApprAuth: _this.isLowerAppr == '' ? '0' : _this.isLowerAppr,
          isDAELmtApprWeb: _this.isDAELmtAppr == '' ? '0' : _this.isDAELmtAppr,
          apprBackReasonType: _this.apprBackReason,
          approveResult: _this.approveResult == '' ? '0' : _this.approveResult,
      ...
      // 判断对公智能风控审批
      function judgeSxkdOnlineAppr (url, serno) {
        var params = {
          serno: serno,
          bizType: bizType,
          instanceIdInfo: JSON.stringify(_this.instanceIdInfo)
     ...
     * 省心快贷获取风控审批返回结果
     */
      function getApproveResult (serno) {
        yufp.service.request({
          method: 'POST',
          url: backend.cmisBiz + '/api/lmtapp/getApproveResult',
          data: serno,
          callback: function (code, message, response) {
            if (response.code == '0') {
              var rtaData = response.data;
              if (rtaData.rtnCode == '0000') {
                _this.approveResult = rtaData.approveResult;
                url = '/api/lmtappr/updatelmtapprchoose';
                updateApprChoose(url, serno);
              } else {
    ...
    saveComment: function () {
      ...
      this.$refs['commentForm'].validate(valid => {
        if (valid) {
          ...
          var isinforcomplete = _this.commentInfo.isinforcomplete;
          var issignet = _this.commentInfo.issignet;
          data.commentSign = '';
          data.ext = _this.commentInfo.ext.toString();
          yufp.service.request({
            method: 'POST',
            data: data,
            ...
            callback: function (code, message, response) {
              if (response.code == 0) {
                if (response.data != null) {
                  ...
                  _this.commentInfo.isinforcomplete = isinforcomplete;
                  _this.commentInfo.issignet = issignet;
                } else {
                  _this.$message({
                    duration: 6000,
                    message: '保存意见失败',
    ...
    agreeUserCheck: function (data) {
      ...
      _this.$nextTick(() => {
        // 只有一个节点直接选中
        if (_this.agreeShowRadio) {
          _this.agreeRadio = data[0].nodeId;
        } else {
          _this.agreeCheckBox.push(data[0].nodeId);
        }
        _this.agreeRadioChange(data[0].nodeId);
      });
    },
    ...
    wfCallBack: function () {
      ...
      param.comment = _this.commentInfo;
      param.ext = _this.commentInfo.ext.toString();
      // 打回接口调用
      yufp.service.request({
        method: 'POST',
        data: param,
