<template>
  <div>
    <yu-panel title="保证人列表" panel-type="simple">
      <template slot="right">
        <yu-button-drop>
          <!-- 是否授信项下 0-否 展示新增 1-是 展示引入 -->
          <yu-button ref="btn_viewDetail" @click="add" v-if="pageParams.type == 'VIEW' || pageParams.isUnderLmt === '0'">新增</yu-button>
          <yu-button ref="btn_importGuarantee" @click="customClick('importGuarantee')" v-if="pageParams.type == 'VIEW' || pageParams.isUnderLmt === '1'">引入</yu-button>
          <yu-button ref="btn_viewDetail" @click="edit" :hidden="pageParams.type == 'VIEW'">修改</yu-button>
          <yu-button ref="btn_viewDetail" @click="info">查看</yu-button>
          <yu-button ref="btn_viewDetail" @click="freshList">刷新</yu-button>
...
yufp.lookup.reg('STD_ZB_CERT_TYP,STD_ZB_GUARANTEE_TY');
export default{
  name: 'D1CBilllist',
  mixins: [mixinList],
  props: {
  ...
  methods: {
    ...
    edit () {
      ...
      dataParams['serno'] = this.contNoSerno;
    ...
    freshList () {
      this.baseParams = {
        condition: { guarContNo: this.pageParams.guarContNo }
      };
    },
  }
};
