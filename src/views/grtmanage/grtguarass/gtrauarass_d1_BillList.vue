<template>
  <div>
    <yu-panel title="一般担保合同">
      ...
      <template slot="right">
        <yu-button-drop>
          <yu-button ref="btn_onInsert" @click="customClick('onInsert')" v-if="max">新增</yu-button>
          <yu-button ref="btn_onUpdate" @click="customClick('onUpdate')" v-if="max">修改</yu-button>
          <yu-button ref="btn_onDelete" @click="customClick('onDelete')" v-if="max">删除</yu-button>
          <yu-button ref="btn_onView" @click="customClick('onView')">查看</yu-button>
          <yu-button ref="btn_onSign" @click="customClick('onSign')" v-if="max">签订</yu-button>
          <yu-button ref="btn_onLogout" @click="customClick('onLogout')" v-if="max">注销</yu-button>
          <yu-button ref="btn_onPrint" @click="customClick('onPrint')" >打印</yu-button>
          <yu-button ref="btn_Insert" @click="customClick('Insert')" v-if="max">多表新增</yu-button>
          <yu-button ref="btn_update" @click="customClick('update')" v-if="max">多表修改</yu-button>
        </yu-button-drop>
      </template>
      <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl" :default-load="true" :base-params="baseParams" request-type="POST">
        ...
        <yu-xtable-column label="借款人编号" prop="borrowerId" width="110"></yu-xtable-column>
        <yu-xtable-column label="借款人名称" prop="borrowerName" width="110"></yu-xtable-column>
        ...
        <yu-xtable-column label="担保起始日" prop="guarStartDate" width="110"></yu-xtable-column>
        <yu-xtable-column label="担保终止日" prop="guarEndDate" width="110"></yu-xtable-column>
        <yu-xtable-column label="登记日期" prop="inputDate" width="130"></yu-xtable-column>
...
export default{
  ...
  data: function () {
    return {
      ...
      baseParams: {condition: JSON.stringify({guarContType: "A"})},
