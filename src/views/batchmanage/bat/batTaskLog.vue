...
<template>
  <!--
    ...
    @updated by 2021-07-07 19:35:26
    @description 任务运行日志-增删改查
  -->
  <div>
    <yu-panel title="任务运行日志列表" panel-type="simple">
      <yu-xform related-table-name="refTable" form-type="search" v-model="searchFormdata" label-width="120px">
        <yu-xform-group :column="3">
          <yu-xform-item placeholder="任务日期" name="taskDate" value-format="yyyy-MM-dd" ctype="datepicker"></yu-xform-item>
          ...
          <yu-xform-item placeholder="任务状态" name="taskStatus"  data-code="STD_RZ_TASK_STATUS" ctype="select"></yu-xform-item>
          <yu-xform-item placeholder="任务开始时间" name="taskBeginTime" value-format="yyyy-MM-dd HH:mm:ss" ctype="datepicker" type="datetime"></yu-xform-item>
          <yu-xform-item placeholder="任务结束时间" name="taskEndTime" value-format="yyyy-MM-dd HH:mm:ss" ctype="datepicker" type="datetime"></yu-xform-item>
          <yu-xform-item placeholder="错误码" name="errorCode" ctype="input" fuzzy-query="both"></yu-xform-item>
          <yu-xform-item placeholder="错误信息" name="errorInfo" ctype="input" fuzzy-query="both"></yu-xform-item>
      ...
      <yu-xtable ref="refTable" selection-type="radio" row-number request-type="POST" :data-url="dataUrl" condition-key="condition" style="width: 100%" height="500">
        <yu-xtable-column fixed label="任务日期" prop="taskDate" value-format="yyyy-MM-dd" width="100" ></yu-xtable-column>
        ...
        <yu-xtable-column label="任务状态" prop="taskStatus" data-code="STD_RZ_TASK_STATUS">
          <template slot-scope="scope">
            <yu-tag type="gray" v-if="scope.row.taskStatus=='000'">待执行</yu-tag>
            <yu-tag type="gray" v-if="scope.row.taskStatus=='001'">外部依赖不满足</yu-tag>
            <yu-tag type="primary" v-if="scope.row.taskStatus=='010'">执行中</yu-tag>
            <yu-tag type="danger" v-if="scope.row.taskStatus=='101'">执行失败</yu-tag>
            <yu-tag type="success" v-if="scope.row.taskStatus=='100'">执行成功</yu-tag>
          </template>
        </yu-xtable-column>
        ...
        <yu-xtable-column label="依赖编号" prop="relNo" width="100"></yu-xtable-column>
        <yu-xtable-column label="依赖任务编号" prop="relTaskNo" width="300"></yu-xtable-column>
        <yu-xtable-column label="是否存在互斥" prop="mutexFlag" data-code="STD_ZB_Y_N"></yu-xtable-column>
        <yu-xtable-column label="互斥编号" prop="mutexNo" width="100"></yu-xtable-column>
        <yu-xtable-column label="互斥任务编号" prop="mutexTaskNo" width="300"></yu-xtable-column>
        <yu-xtable-column label="任务开始时间" prop="taskBeginTime" value-format="yyyy-MM-dd HH:mm:ss" width="160"></yu-xtable-column>
        <yu-xtable-column label="任务结束时间" prop="taskEndTime" value-format="yyyy-MM-dd HH:mm:ss" width="160"></yu-xtable-column>
        <yu-xtable-column label="错误码" prop="errorCode"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
    <!-- 新增、修改、查看表单弹窗 -->
    <yu-xdialog :title="viewTitle[viewType]" width="1024px" :visible.sync="dialogVisible">
      <yu-xform ref="refForm" label-width="80px" v-model="formdata" :disabled="formDisabled">
        <yu-xform-group>
          <yu-xform-item label="任务日期" placeholder="任务日期" name="taskDate" ctype="input"></yu-xform-item>
          ...
          <yu-xform-item label="任务状态" placeholder="任务状态" name="taskStatus" data-code="STD_RZ_TASK_STATUS" ctype="select"></yu-xform-item>
          <yu-xform-item label="任务开始时间" placeholder="任务开始时间" name="taskBeginTime" ctype="input"></yu-xform-item>
          <yu-xform-item label="任务结束时间" placeholder="任务结束时间" name="taskEndTime" ctype="input"></yu-xform-item>
        </yu-xform-group>
        <yu-xform-group column="1">
          <yu-xform-item label="错误码" placeholder="错误码" name="errorCode" ctype="input"></yu-xform-item>
          <yu-xform-item label="错误信息" placeholder="错误信息" name="errorInfo" ctype="textarea"></yu-xform-item>
        </yu-xform-group>
        <yu-xform-group>
          <yu-xform-item label="任务类型" placeholder="任务类型" name="taskType" data-code="STD_TASK_TYPE" ctype="select"></yu-xform-item>
          <yu-xform-item label="优先级别" placeholder="优先级别" name="priFlag" data-code="STD_PRI_FLAG" ctype="select"></yu-xform-item>
          <yu-xform-item label="执行频率" placeholder="执行频率" name="runPeriod" data-code="STD_RUN_PERIOD" ctype="select"></yu-xform-item>
          <yu-xform-item label="频率规则" placeholder="频率规则" name="periodRule" data-code="STD_PERIOD_RULE" ctype="select"></yu-xform-item>
          ...
          <yu-xform-item label="是否存在依赖" placeholder="是否存在依赖" name="relationFlag" data-code="STD_ZB_Y_N" ctype="select"></yu-xform-item>
          <yu-xform-item label="依赖编号" placeholder="依赖编号" name="relNo" ctype="input"></yu-xform-item>
          <yu-xform-item label="依赖任务编号" placeholder="依赖任务编号" name="relTaskNo" ctype="input"></yu-xform-item>
          ...
          <yu-xform-item label="文件尾类型" placeholder="文件尾类型" name="fileEndType" ctype="input"></yu-xform-item>
          ...
          <yu-xform-item label="MD5存放类型" placeholder="MD5存放类型" name="md5Flag"  data-code="STD_RZ_MD5_TYPE" ctype="select"></yu-xform-item>
...
export default {
  ...
  data: function() {
    return {
      searchFormdata: {},
      dataUrl: this.$backend.cmisBatch + '/api/battasklog/query',
      updateUrl: this.$backend.cmisBatch + '/api/battasklog/update',
      addUrl: this.$backend.cmisBatch + '/api/battasklog/',
      batchDeleteUrl: this.$backend.cmisBatch + '/api/battasklog/batchdelete/',
      formdata: {},
      rule: [
        {
          required: true,
  ...
  methods: {
    ...
    deleteFn: function () {
      ...
      for (let i = 0; i < selections.length; i++) {
        arr.push(selections[i].taskDate);
      }
