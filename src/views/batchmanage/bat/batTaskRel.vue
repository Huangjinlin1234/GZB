...
<template>
  <!--
    ...
    @updated by 2021-07-07 19:35:26
    @description 任务依赖关系-增删改查
  -->
  <div>
    <yu-panel title="任务依赖关系列表" panel-type="simple">
      <yu-xform related-table-name="refTable" form-type="search" v-model="searchFormdata" label-width="120px">
        <yu-xform-group :column="3">
          ...
          <yu-xform-item placeholder="依赖任务名称" name="relTaskName" ctype="input" fuzzy-query="both"></yu-xform-item>
          <yu-xform-item placeholder="启用标志" data-code="STD_ZB_Y_N" ctype="select"></yu-xform-item>
      ...
      <yu-xtable ref="refTable" selection-type="radio" row-number request-type="POST" :data-url="dataUrl" condition-key="condition" style="width: 100%" height="500">
        <yu-xtable-column fixed label="依赖编号" prop="relNo" width="100"></yu-xtable-column>
        <yu-xtable-column fixed label="依赖任务编号" prop="relTaskNo" width="100"></yu-xtable-column>
        <yu-xtable-column fixed label="依赖任务名称" prop="relTaskName" width="360"></yu-xtable-column>
    ...
    <yu-xdialog :title="viewTitle[viewType]" width="1024px" :visible.sync="dialogVisible">
      <yu-xform ref="refForm" label-width="80px" v-model="formdata" :disabled="formDisabled">
        <yu-xform-group>
          ...
          <yu-xform-item label="依赖任务名称" placeholder="依赖任务名称" name="relTaskName" ctype="input"></yu-xform-item>
...
export default {
  ...
  data: function() {
    return {
      searchFormdata: {},
      dataUrl: this.$backend.cmisBatch + '/api/battaskrel/query',
      updateUrl: this.$backend.cmisBatch + '/api/battaskrel/update',
      addUrl: this.$backend.cmisBatch + '/api/battaskrel/',
      batchDeleteUrl: this.$backend.cmisBatch + '/api/battaskrel/batchdelete/',
      formdata: {},
      rule: [
        {
          required: true,
