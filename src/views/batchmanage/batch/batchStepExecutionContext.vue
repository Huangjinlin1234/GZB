...
<template>
  <!--
    @created by creazyCder 2021-07-07 19:28:53
    @updated by 2021-07-07 19:28:53 
    @updated by 2021-07-07 19:28:53
    @description -增删改查
  -->
  <div>
    <yu-panel title="列表" panel-type="simple">
      ...
      <yu-xtable ref="refTable" selection-type="radio" row-number request-type="POST" :data-url="dataUrl" condition-key="condition">
        ...
        <yu-xtable-column label="一个字符串版本的SERIALIZED_CONTEXT" prop="shortContext"></yu-xtable-column>
        <yu-xtable-column label="整个上下文序列化" prop="serializedContext"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
    <!-- 新增、修改、查看表单弹窗 -->
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible">
      <yu-xform ref="refForm" label-width="80px" v-model="formdata" :disabled="formDisabled">
        <yu-xform-group>
          ...
          <yu-xform-item label="一个字符串版本的SERIALIZED_CONTEXT" placeholder="一个字符串版本的SERIALIZED_CONTEXT" name="shortContext" ctype="input"></yu-xform-item>
          <yu-xform-item label="整个上下文序列化" placeholder="整个上下文序列化" name="serializedContext" ctype="input"></yu-xform-item>
        </yu-xform-group>
...
export default {
  ...
  data: function() {
    return {
      searchFormdata: {},
      dataUrl: this.$backend.cmisBatch + '/api/batchstepexecutioncontext/query',
      updateUrl: this.$backend.cmisBatch + '/api/batchstepexecutioncontext/update',
      addUrl: this.$backend.cmisBatch + '/api/batchstepexecutioncontext/',
      batchDeleteUrl: this.$backend.cmisBatch + '/api/batchstepexecutioncontext/batchdelete/',
      formdata: {},
      rule: [
        {
          required: true,
