<template>
  <div id="print">
    <div id="printContent">
      <yu-xtable ref="printTable" :data="dataValue" :data-url="dataUrl"  row-number  :base-params="searchData" :pageable="false" >
        <yu-xtable-column label="名称" prop="tempName"></yu-xtable-column>
        <yu-xtable-column label="操作" prop="opr" align="center" width="200">
          <template slot-scope="drawScope">
            ...
            <yu-button size="small" type="primary" :hidden="contPageType != '2'" @click="drawQZpdf(drawScope.$index, drawScope.row)">部分打印</yu-button>
            <yu-button size="small" type="primary" :hidden="contPageType != '3'" @click="drawQZpdf(drawScope.$index, drawScope.row)">尾页打印</yu-button>
          </template>
        </yu-xtable-column>
      </yu-xtable>
    </div>
...
export default {
  ...
  methods: {
    initData () {
      var pageData = this.pageParams;
      this.contPageType = pageData.contPageType;
      this.cptName = pageData.cptName;
      this.pvpSerno = pageData.serno;
      this.dataValue = [{
        tempName: pageData.tempName}];
    },
    cancel () {
    ...
    // 打开帆软报表
    viewfrpt (row) {
      let cptName = this.cptName;
      let pvpSerno = this.pvpSerno;
      let params = {};
      let name = 'printManage/frptdemo'; // 引入另外一个vue文件
      params.src = this.$backend.frptRptService + cptName + '&pvpSerno=' + pvpSerno;
      this.$dialog.open(
        '帆软打印',
        name,
        1200,
    ...
    viewCfca (row) {
      ...
      let cptName = this.cptName;
      let pvpSerno = this.pvpSerno;
      let contPageType = this.contPageType;
      // 构建cfcaDto对象
      var model = {
        pdfFileName: pvpSerno + '_' + cptName.replace('.cpt', '.pdf'), // 生成的PDF文件名称
        managerId: loginCode,
        managerBrId: orgCode,
        sealserno: pvpSerno, // 印章流水号
        crossSealFlag: contPageType == '2', // 骑缝章标识(true/false)
        keySealFlag: contPageType == '3', // 专用章标识(true/false)
        keyWord: '（盖章处）', // 盖章关键字，仅用于专用章标识为true,在此字段周围盖章
        templateId: cptName, // 报表英文名称
        // map: map, // 帆软报表参数
