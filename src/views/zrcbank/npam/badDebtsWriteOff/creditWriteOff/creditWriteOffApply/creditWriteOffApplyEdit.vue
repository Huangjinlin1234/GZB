...
yufp.lookup.reg('STD_ZB_YES_NO,STD_WRITEOFF_TYPE,STD_BCM_MODE,STD_DATABOOK_TYPE,STD_WRITEOFF_ASSET_TYPE,STD_ZB_GUAR_WAY,STD_FIVE_CLASS');
export default {
  props: {
    bizPageData: Object
  },
  mixins: [mixin],
  components: { imageSystem, yufpNwfInit, YufpExcelImport, YufpExcelExport, fineReport, AdjustmentJudgeHis },
  data: function () {
    return {
      ...
      imageBizParam: [{
        ...
        'index': {
          'businessid': this.$route.meta.params.josnData.pcwaSerno
        }
      }],
      fineReportParam: {},
      image: '1',
  ...
  computed: {
    ...
    childParams: function () {
      let params = {
        serno: this.bizPageData == undefined ? this.$route.meta.params.josnData.pcwaSerno : this.bizPageData.instanceInfo.bizId
      };
      return params;
    }
  },
    ...
    this.fineReportParam =
        {
          ...
          'index': '&pbdwasSerno=' + this.$route.meta.params.josnData.pcwaSerno
        };
  },
  mounted () {
    this.afterInit();
  ...
  methods: {
    ...
    afterInit () {
      var _this = this;
      var url = this.$backend.cmisNpam + '/api/placardwriteoffapp/queryPlaCardWriteoffAppInfo';
      _this.bizDataParams = {pcwaSerno: _this.$route.meta.params.josnData.pcwaSerno};
      yufp.service.request({
        method: 'POST',
        url: url,
        data: {'pcwaSerno': _this.$route.meta.params.josnData.pcwaSerno},
    ...
    saveApplyFn () {
      ...
      if (!validate) {
        ...
        var url = this.$backend.cmisNpam + '/api/placardwriteoffapp/update';
        yufp.service.request({
          method: 'POST',
          url: url,
          data: model,
