  ...
  <div>
    <yu-tabs v-model="applyActiveName"  @tab-click="handleClick">
              <yu-tab-pane label="呆账核销记账信息" name="applyBase">
                <yu-xform ref="applyBaseForm" label-width="140px" v-model="applyBaseData">
                  <yu-panel title="核销记账业务列表" panel-type="simple">
                      ...
                      <yu-xtable ref="applyBussTable" selection-type="radio" row-number :data-url="applyBussDataUrl" :base-params="baseParams" :default-load="false" request-type="POST" style="margin-top:10px">
                  ...
                  <yu-panel title="核销记账申请信息" panel-type="simple">
                    <yu-xform-group :column="2">
                      <yu-xform-item  name="pbdwasSerno" label="serno" hidden></yu-xform-item>
                  ...
                  <yu-panel title="登记信息" panel-type="simple">
                    <yu-xform-group :column="2">
                        <yu-xform-item  name="inputId" label="登记人"  :hidden="true" :value="loginCode"></yu-xform-item>
                        <yu-xform-item  name="inputBrId" label="登记机构"  :hidden="true" :value="orgCode"></yu-xform-item>
      ...
      <yu-xdialog :visible.sync="dialogVisibleDebt" width="1200px" title="借据列表">
        ...
        <yu-xtable ref="refDebtTable" :row-number="true" selection-type="radio" request-type="POST" :pageable="true" :data-url="dataBillListUrl" :default-load="true" condition-key="condition" :base-params="baseParams1">
          <yu-xtable-column label="合同编号" prop="contNo" width="180">
          </yu-xtable-column>
          <yu-xtable-column label="借据编号" prop="billNo" width="200" ></yu-xtable-column>
          <yu-xtable-column label="客户号" prop="cusId" width="200" ></yu-xtable-column>
...
yufp.lookup.reg('YESNO,PUBLISH_STATUS,STD_FIVE_CLASS,STD_ZB_CUR_TYP');
export default {
  mixins: [mixin],
  ...
  data: function () {
    return {
      ...
      applyBaseData: {},
      baseParams: {},
      baseParams1: {condition: JSON.stringify({ queryFlag: 'applyTable'})},
  ...
  methods: {
    ...
    afterInit () {
      var _this = this;
      _this.applyBaseData.pbdwraSerno = this.$xutils.getSEQWithParamFromServer('IQP_WRITEOFF_APP_SERNO');
      _this.applyBaseData.inputDate = this.$xutils.dateFormat('yyyy-MM-dd', new Date());
      _this.baseParams = {condition: JSON.stringify({ pbdwraSerno: _this.applyBaseData.pbdwraSerno}) };
    },
    cancelFn () {
      var _this = this;
    ...
    submitApplyFn () {
      ...
      if (_this.$refs.applyBussTable.tabledata.length == '0') {
        return _this.$message.error('请至少引入一笔借据');
      }
      this.saveApplyFn();
      if (!_this.ifSave) {
      ...
      var cusId = _this.$refs.applyBussTable.tabledata[0].cusId;
      var cusName = _this.$refs.applyBussTable.tabledata[0].cusName;
      ...
      startdto.param = {
        pbdwasSerno: _this.applyBaseData1.pbdwasSerno
      };
    ...
    addDebtFn () {
      var _this = this;
      // if (_this.$refs.applyBussTable.tabledata.length > 0) {
      //   _this.baseParams1 = {condition: {'contNo': _this.$refs.applyBussTable.tabledata[0].contNo}};
      // }
      if (_this.serno) {
        _this.baseParams1 = {condition: {'serno': _this.serno}};
      }
    ...
    updateAmtFn (op) {
      ...
      yufp.service.request({
        method: 'POST',
        url: url,
        data: {'pbdwraSerno': _this.applyBaseData.pbdwraSerno},
        callback: function (code, message, response) {
          if (response.code == '0') {
            ...
            _this.serno = response.data.pbdwasSerno;
            _this.imageBizParam = [
              {
                'top_outsystem_code': 'XXD_ZCBQ',
                'outsystem_code': 'XXD_ZCBQ02',
                'index': {
                  'businessid': _this.serno
                }
              }];
            _this.fineReportParam =
            {
              'top_outsystem_code': 'plaBadDebtWriteoffAppBatchDZHXRp.cpt',
              'index': '&pbdwasSerno=' + _this.serno
            };
            _this.showReport = op;
          }
        }
