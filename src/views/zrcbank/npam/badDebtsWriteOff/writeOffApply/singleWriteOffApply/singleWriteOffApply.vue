 ...
 * @updated by  youhong
 * @description 单客户呆账核销申请
 */
<template>
  <div>
    <yu-tabs v-model="activeName">
      <yu-tab-pane label="单户呆账核销申请" name="writeOffApply">
      <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
        <yu-xform related-table-name="applyTable" form-type="search">
          <yu-xform-group :column="2">
            <yu-xform-item label="业务流水号" name="pbdwasSerno"></yu-xform-item>
            <yu-xform-item label="客户号" name="cusId"></yu-xform-item>
      ...
      <yu-panel title="单户呆账核销申请列表" :hideFilter="false" :collapseHide="false" style="margin-top:10px">
        <div>
        ...
        <yu-xtable ref="applyTable" selection-type="radio" row-number :data-url="dataApplyListUrl" request-type="POST" :base-params="baseApplyParams" condition-key="condition" style="margin-top:10px">
          <yu-xtable-column align="center" label="业务流水号" prop="pbdwasSerno" width="180"></yu-xtable-column>
          ...
          <yu-xtable-column align="center" label="核销总金额" prop="totalWriteoffAmt"  :formatter="Currency"></yu-xtable-column>
          <yu-xtable-column align="center" label="核销总本金" prop="totalWriteoffCap"  :formatter="Currency"></yu-xtable-column>
          <yu-xtable-column align="center" label="核销总利息" prop="totalWriteoffInt"  :formatter="Currency"></yu-xtable-column>
          <yu-xtable-column align="center" label="核销总笔数" prop="totalWriteoffNum" ></yu-xtable-column>
          <yu-xtable-column align="center" label="登记日期" prop="inputDate" width="110"></yu-xtable-column>
          <yu-xtable-column align="center" label="登记人" prop="inputIdName" width="110"></yu-xtable-column>
          <yu-xtable-column align="center" label="登记机构" prop="inputBrIdName" width="110"></yu-xtable-column>
          <yu-xtable-column align="center" label="审批状态" prop="approveStatus" width="120" data-code="STD_ZB_APPR_STATUS"></yu-xtable-column>
        </yu-xtable>
      </yu-panel>
      </yu-tab-pane>
      <yu-tab-pane label="单户呆账核销申请历史" name="writeoffapplyHist">
       <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
       ...
       <yu-panel title="单户呆账核销申请历史列表" :hideFilter="false" :collapseHide="false" style="margin-top:10px">
        <div>
...
export default {
  ...
  data: function () {
    return {
      dataApplyListUrl: this.$backend.cmisNpam + '/api/plabaddebtwriteoffappsig/queryPlaBadDebtWriteoffAppSig',
      dataApplyHistListUrl: this.$backend.cmisNpam + '/api/plabaddebtwriteoffappsig/queryPlaBadDebtWriteoffAppSig',
  ...
  methods: {
    ...
    */
    addApplyFn () {
      ...
      _this.$router.addTab({
        // 路由名称
        name: 'zrcbank/npam/badDebtsWriteOff/writeOffKeepAccount/writeOffKeepAccountApply/writeOffKeepAccountApplyxiangdao',
        // 自定义唯一页签key
        ...
        // 传递的业务数据，可选配置
        data: {
          pbdwasSerno: _this.$xutils.getDefaultformulaData('SEQ:TRADE_ID_SEQ'),
          viewType: 'ADD'
        }
      });
    },
    ...
    */
    editApplyFn () {
      ...
      _this.$router.addTab({
        // 路由名称
        name: 'zrcbank/npam/badDebtsWriteOff/writeOffApply/singleWriteOffApply/singleWriteOffApplyEdit',
        // 自定义唯一页签key
        key: 1 + new Date().getTime(), // 必传
        // 页签名称
        title: '单户呆账核销申请修改',
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              ...
              url: _this.$backend.cmisNpam + '/api/plabaddebtwriteoffappsig/delete',
    ...
    * 查看详情
    ...
    detailApplyFn (info) {
      ...
      if (info === 'apply') {
        ...
        _this.$router.addTab({
          // 路由名称
          name: 'zrcbank/npam/badDebtsWriteOff/writeOffApply/singleWriteOffApply/singleWriteOffApplyDetail',
          // 自定义唯一页签key
          key: 1 + new Date().getTime(), // 必传
          // 页签名称
          title: '单户呆账核销申请详情',
