<template>
  ...
  <div id="d1_BillCard">
   <div id="d1_BillCardContent">
      <yu-xform ref="refForm" label-width="150px" :disabled="viewType == 'DETAIL'" v-model="formdata" style="text-align: center;" >
        <yu-xform-group>
         <yu-xform-item label="案件编号"  name="caseSerno"  :hidden="true"></yu-xform-item>
         <yu-xform-item label="再审法律文书号"  name="legalDcmntsNo" ></yu-xform-item>
         <yu-xform-item label="申请人名称"  name="cusName"  rules="required"></yu-xform-item>
        <yu-xform-item label="法律文书名称"  name="legalDcmntsName" ></yu-xform-item>
        </yu-xform-group>
        <yu-xform-group>
        <yu-xform-item label="再审原因"  name="retrialReason" ctype="select" rules="required" data-code="STD_RETRIAL_REASON"></yu-xform-item>
        <yu-xform-item label="再审法院"  name="acceptCourt"  rules="required"></yu-xform-item>
        <yu-xform-item label="主审法官"  name="judge" ></yu-xform-item>
        ...
        <yu-xform-item label="再审受理日期"  name="retrialAcceptDate" ctype="datepicker" value-format="yyyy-MM-dd"></yu-xform-item>
        <yu-xform-item label="再审判决（裁定）日期"  name="retrialJudgeDate" value-format="yyyy-MM-dd"  ctype="datepicker"></yu-xform-item>
        <yu-xform-item label="再审结果"  name="retrialRst" ctype="select" data-code="STD_RETRIAL_RST"></yu-xform-item>
        <yu-xform-item label="是否为终审判决（裁定）"  name="isFinalJudge" ctype="select" data-code="STD_ZB_YES_NO"></yu-xform-item>
        <yu-xform-item label="法律文书落款日期"  name="dcmntsInscribeDate" value-format="yyyy-MM-dd"  ctype="datepicker"></yu-xform-item>
        <yu-xform-item label="法律文书生效日期"  name="dcmntsInureDate" value-format="yyyy-MM-dd"  ctype="datepicker"></yu-xform-item>
        <yu-xform-item label="登记日期"  name="inputDate" value-format="yyyy-MM-dd" hidden="true" ctype="datepicker"></yu-xform-item>
        </yu-xform-group>
        <yu-button-drop>
...
// 注册字典项
yufp.lookup.reg('STD_RETRIAL_REASON,STD_RETRIAL_RST,STD_ZB_YES_NO');
export default {
  name: 'D1BillCard',
  mixins: [mixinForm],
  props: {
  ...
  created () {
    ...
    yufp.service.request({
      method: 'POST',
      url: `${backend.cmisNpam}/api/plalawretrialbaseinfo/queryPlaLawRetrialBaseInfo`,
  ...
  methods: {
    ...
    doSave () {
      ...
      _this.$refs.refForm.validate(volid => {
        ...
        yufp.service.request({
          method: 'POST',
          url: backend.cmisNpam + '/api/plalawretrialbaseinfo/update',
          async: false,
          data: model,
          callback: function (code, message, response) {
            debugger;
