<template>
  ...
  -->
  <div>
    <yu-panel title="诉讼申请向导" :hideFilter="false" :collapseHide="false">
      <yu-xform ref="refForm" label-width="120px" v-model="formdata" style="text-align: center;">
        ...
        <!--<yu-xform-item label="当事人客户编号"  placeholder="客户编号" name="cusId" rules="required" icon="search"  @click.native="openDialog" ></yu-xform-item>-->
          <yu-xform-item label="当事人客户名称" name="cusName" rules="required" disabled></yu-xform-item>
        </yu-xform-group>
        <yu-button-drop>
  ...
  methods: {
    ...
    doGo () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisNpam + '/api/plalawapp/savePlaLawApp',
        data: model,
        callback: function (code, message, response) {
          if (response.code == 0) {
            _this.doBack();
            appSerno = response.data.appSerno;
            userId = response.data.inputId;
            orgId = response.data.inputBrId;
            _this.$router.addTab({
              name: 'zrcbank/npam/LiqLawRpt/liqLawRptDetail/plaLawAppDetail',
              key: 'addPlaLawApp' + appSerno, // 必传
              title: '诉讼申请新增',
              data: {
                appSerno: appSerno,
                userId: userId,
