<template>
  ...
  -->
  <div>
    <yu-panel title="以物抵债登记向导" :hideFilter="false" :collapseHide="false">
  ...
  methods: {
    ...
    doNext () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisNpam + '/api/plabartplddebtappinfo/save',
        data: model,
        callback: function (code, message, response) {
          if (response.code == 0) {
            _this.doBack()
            pdraiSerno = response.data; //业务流水号
            _this.$router.addTab({
              ...
              data: {
                ...
                // cusName: _this.formdata.cusName,
