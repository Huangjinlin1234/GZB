<template>
      <yu-tabs v-model="tabName" type="border-card" @tab-click="tabClick">
        <yu-tab-pane label="当前任务" name="wait">
          ...
          <yu-panel ref="panel"  title="首次贷后检查任务列表（微业贷）" color="red" class="adjust-height" :collapse-hide="false">
              ...
              <template>
                <yu-toolBar>
                  <yu-button ref="btn_$update" @click="update" v-if="isShow">分配</yu-button>
                  <yu-button type="primary" @click="detail()">查看</yu-button>
                </yu-toolBar>
              </template>
            <!--任务主表显示列表  审批状态：待发起、退回、审批中，显示在该列表-->
              <yu-xtable ref="refTable" row-number :data-url="tableUrl" :base-params="baseParams" selection-type="radio" request-type="POST"  :default-load="false" condition-key="condition" >
...
export default {
  ...
  methods: {
    ...
    update () {
      var _this = this;
      const params = _this.$refs.refTable.selections.length;
      if (params == '0') {
      ...
      _this.$dialog.open('分配', 'zrcbank/psp/pspCheck/wyd/wydFirstCheckUpdate', 1000, 500, row, () => {
        // 列表刷新
    ...
    resetFn: function (data) {
        ...
        // 重新执行查询
        this.$refs.refTable.remoteData({ condition: '' });
    },
    iskhjl(){
