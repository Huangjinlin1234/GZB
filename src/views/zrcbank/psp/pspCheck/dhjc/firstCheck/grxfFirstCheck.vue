 ...
 * @LastEditTime: 2021-06-17 15:26:47
 * @LastEditors: Please set LastEditors
 * @Description: 个人消费性首次检查
 * @FilePath: \dscms-web\src\views\zrcbank\psp\pspCheck\dhjc\firstCheck\grxfFirstCheck.vue
-->
<template>
      <yu-tabs v-model="tabName" type="border-card" @tab-click="tabClick">
        <yu-tab-pane label="待检查任务" name="wait">
          <yu-panel ref="panel"  title="输入查询条件"  class="adjust-height" :collapse-hide="false">
              <yu-xform ref="searchForm" related-table-name="pspTaskTable" v-model="searchFormdata" >
                <yu-xform-group>
                  ...
                  <yu-xform-item  name="cusName" :colspan="7" label-width="120px" label="客户名称" fuzzy-query="both"></yu-xform-item>
          ...
          <yu-panel ref="panel"  title="首次贷后检查任务列表（个人消费性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
        ...
        <yu-tab-pane label="历史检查任务" name="history">
          ...
          <yu-panel ref="panel"  title="首次贷后检查历史列表（个人消费性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
...
export default {
  data: function () {
    ...
    return {
      // pageName: '',
      // pageTitle: '', //首次贷后检查任务列表
      // hisPageTitle: '', //首次贷后检查历史列表
      baseParams: {condition: JSON.stringify({checkType: '13', approveStatus: '000,111,992'}), sort: 'taskStartDt desc'},
      hisBaseParams: {condition: JSON.stringify({checkType: '13', approveStatus: '997,998'}), sort: 'taskStartDt desc'},
