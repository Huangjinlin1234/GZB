<!--
 * @Author: liut
 * @Date: 2021-06-10 20:20:59
 * @LastEditTime: 2021-06-17 18:01:12
 * @LastEditors: Please set LastEditors
 * @Description: 对公及个人经营性首次检查
 * @FilePath: \dscms-web\src\views\zrcbank\psp\pspCheck\dhjc\firstCheck\dggrFirstCheck.vue
-->
<template>
      <yu-tabs v-model="tabName" type="border-card" @tab-click="tabClick">
        <yu-tab-pane label="待检查任务" name="wait">
          ...
          <yu-panel ref="panel"  title="首次贷后检查任务列表（对公及个人经营性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
            ...
            <!--任务主表显示列表  审批状态：待发起、退回、审批中，显示在该列表-->
              <yu-xtable ref="pspTaskTable"  :data-url="tableUrl" :base-params="baseParams" selection-type="radio" request-type="POST" condition-key="condition" >
                ...
                <yu-xtable-column label="检查类型" align="center" prop="checkType" data-code="STD_ZB_CHECK_TYPE" width="180px"> </yu-xtable-column>
                <yu-xtable-column label="客户编号" align="center" prop="cusId" width="120px"> </yu-xtable-column>
                <yu-xtable-column label="客户名称" align="center" prop="cusName" width="120px"> </yu-xtable-column>
                <yu-xtable-column label="借据编号" align="center" prop="billNo" width="180px"> </yu-xtable-column>
                <yu-xtable-column label="合同编号" align="center" prop="contNo" width="150px"> </yu-xtable-column>
                <yu-xtable-column label="产品名称" align="center" prop="prdName" width="120px"></yu-xtable-column>
                <yu-xtable-column label="任务生成日期" align="center" prop="taskStartDt" width="140px"> </yu-xtable-column>
                ...
                <yu-xtable-column label="任务执行机构" align="center"  prop="execBrIdName"  width="200px"> </yu-xtable-column>
                <yu-xtable-column label="检查状态" align="center" prop="checkStatus" data-code="STD_ZB_CHECK_STATUS"> </yu-xtable-column>
                <yu-xtable-column label="审批状态" align="center" prop="approveStatus" data-code="STD_ZB_APPR_STATUS"></yu-xtable-column>
              </yu-xtable>
          </yu-panel>
        </yu-tab-pane>
        ...
        <yu-tab-pane label="历史检查任务" name="history">
          <yu-panel ref="panel"  title="输入查询条件"  class="adjust-height" :collapse-hide="false">
              <yu-xform ref="hisSearchForm" related-table-name="pspTaskHisTable" v-model="hisSearchFormdata" label-width="150px">
                <yu-xform-group>
                  ...
                  <yu-xform-item  name="execId" :colspan="7" ctype="YuXuserForDh" @select-fn="commonSelectFn1" :mapping="{'execId':'execId','execIdName':'execIdName'}" label-width="120px" label="任务执行人"></yu-xform-item>
                  <yu-xform-item  name="execBrId" :colspan="8" ctype="YuXorgForDh" @select-fn="commonSelectFn1" :mapping="{'execBrId':'execBrId','execBrIdName':'execBrIdName'}" label="任务执行机构" label-width="120px"></yu-xform-item>
                </yu-xform-group>
                <div class="button-group" align="center">
          ...
          <yu-panel ref="panel"  title="首次贷后检查历史列表（对公及个人经营性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
            ...
            <!--任务主表显示列表  审批状态：审批通过，显示在该列表-->
              <yu-xtable ref="pspTaskHisTable"  :data-url="tableUrl" :base-params="hisBaseParams"  selection-type="radio" request-type="POST" condition-key="condition" >
                <yu-xtable-column label="任务编号" align="center" prop="taskNo"  width="170px"></yu-xtable-column>
                <yu-xtable-column label="任务类型" align="center" prop="taskType" data-code="STD_ZB_TASK_TYPE"></yu-xtable-column>
                <yu-xtable-column label="检查类型" align="center" prop="checkType" data-code="STD_ZB_CHECK_TYPE" width="180px"> </yu-xtable-column>
                <yu-xtable-column label="客户编号" align="center" prop="cusId" width="120px"> </yu-xtable-column>
...
export default {
  data: function () {
    let _this = this;
    return {
      baseParams: {condition: {checkType: '11,12', approveStatus: '000,111,992'}, sort: 'taskStartDt desc'}, // 查询检查类型为对公及个人经营性的任务
      hisBaseParams: {condition: JSON.stringify({checkType: '11,12', approveStatus: '997,998'}), sort: 'taskStartDt desc'}, // 历史列表
      tableUrl: backend.cmisPsp + '/api/psptasklist/getPspTaskList', // 任务列表查询地址
      // hisTableUrl: backend.cmisPsp + '/api/psptasklist/getPspTaskListHis', // 历史任务列表查询地址
  ...
  methods: {
    ...
    check: function () {
      ...
      if (selections.length != 1) {
        _this.$message({ message: '请选择一条数据进行操作！', type: 'warning', showClose: true });
        return;
      }
      ...
      let pspTask = {};
      yufp.clone(selections[0], pspTask);
      let status = 'check';
      let pageName = 'dggr';
      ...
      _this.$nextTick(function () {
        this.$router.addTab({
          ...
          key: 'pspCheckRst' + new Date().getTime(),
          // 页签名称
          title: '首次贷后检查',
          // 传递的业务数据，可选配置
    ...
    searchFn: function (data) {
      ...
      if (data !== undefined && data !== '' && data === 'his') {
        _this.$refs.pspTaskHisTable.remoteData({ condition: JSON.stringify(_this.hisSearchFormdata) });
      } else {
        _this.$refs.pspTaskTable.remoteData({ condition: JSON.stringify(_this.searchFormdata) });
      }
    },
    // 重置
