<!--
 * @Author: liut
 * @Date: 2021-06-10 20:22:25
 * @LastEditTime: 2021-06-17 15:27:11
 * @LastEditors: Please set LastEditors
 * @Description: 小微经营性首次检查
 * @FilePath: \dscms-web\src\views\zrcbank\psp\pspCheck\dhjc\firstCheck\xwjyFirstCheck.vue
-->
<template>
      <yu-tabs v-model="tabName" type="border-card" @tab-click="tabClick">
        <yu-tab-pane label="待检查任务" name="wait">
          ...
          <yu-panel ref="panel"  title="首次贷后检查任务列表（小微经营性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
        ...
        <yu-tab-pane label="历史检查任务" name="history">
          <yu-panel ref="panel"  title="输入查询条件"  class="adjust-height" :collapse-hide="false">
              <yu-xform ref="hisSearchForm" related-table-name="pspTaskHisTable" v-model="hisSearchFormdata" label-width="150px">
                <yu-xform-group>
                  ...
                  <yu-xform-item  name="execBrId" :colspan="8" ctype="YuXorgForDh" @select-fn="commonSelectFn1" :mapping="{'execBrId':'execBrId','execBrIdName':'execBrIdName'}" v-if="brId" label="任务执行机构" label-width="120px"></yu-xform-item>
                </yu-xform-group>
                <div class="button-group" align="center">
          ...
          <yu-panel ref="panel"  title="首次贷后检查历史列表（小微经营性）" color="red" class="adjust-height" :collapse-hide="false">
              <!-- 查看详情按钮 -->
              <template>
                <yu-toolBar>
...
export default {
  data: function () {
    let _this = this;
    return {
      baseParams: {condition: JSON.stringify({checkType: '14', approveStatus: '000,111,992'}), sort: 'taskStartDt desc'},
      hisBaseParams: {condition: JSON.stringify({checkType: '14', approveStatus: '997,998'}), sort: 'taskStartDt desc'},
