<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <yu-tab-pane label="押品详情信息-建设用地使用权" name="second">
      <guarBuildUse ref="guarBuildUse"></guarBuildUse>
    </yu-tab-pane>
...
export default {
  // 导入的组件
  components: { guarBaseInfoAdd, guarBuildUse, yufpNwfInit, mapState},
  data: function () {
    return {
      activeName: 'first'
    };
  ...
  methods: {
    ...
    onSave () {
      ...
      yufp.clone(_this.$refs.guarBuildUse.formdata, model.guarInfBuildUse);
      yufp.clone(_this.$refs.guarBaseInfoAdd.formdata, model.guarInfBuildUse);
      ...
      this.$xutils.request({
        // 同步请求
        async: true,
        url: this.$backend.cmisBiz + '/api/guarbaseinfo/saveByGuarBasicGuarInfBuildUseVo',
