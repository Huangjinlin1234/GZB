<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <yu-tab-pane label="押品详情信息-商业用房" name="second">
      <guarBusinessIndustry ref="guarBusinessIndustry"></guarBusinessIndustry>
    </yu-tab-pane>
...
export default {
  // 导入的组件
  components: { guarBaseInfoAdd, guarBusinessIndustry, yufpNwfInit, mapState},
  data: function () {
    return {
      activeName: 'first'
    };
  ...
  methods: {
    ...
    onSave () {
      ...
      yufp.clone(_this.$refs.guarBusinessIndustry.formdata, model.guarInfBusinessIndustryHousr);
      yufp.clone(_this.$refs.guarBaseInfoAdd.formdata, model.guarInfBusinessIndustryHousr);
      model.guarInfBusinessIndustryHousr.buildArea = Number(model.guarInfBusinessIndustryHousr.buildArea);
      // console.log(model);
      this.$xutils.request({
        // 同步请求
        async: true,
