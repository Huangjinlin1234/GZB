<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <yu-tab-pane label="押品详情信息-其他使用权" name="second">
      <guarUsufLand ref="guarUsufLand"></guarUsufLand>
    </yu-tab-pane>
...
export default {
  // 导入的组件
  components: { guarBaseInfoAdd, guarUsufLand, yufpNwfInit, mapState},
  data: function () {
    return {
      activeName: 'first'
    };
  ...
  methods: {
    ...
    onSave () {
      ...
      yufp.clone(_this.$refs.guarUsufLand.formdata, model.guarInfUsufLand);
      yufp.clone(_this.$refs.guarBaseInfoAdd.formdata, model.guarInfUsufLand);
      ...
      this.$xutils.request({
        // 同步请求
        async: true,
        url: this.$backend.cmisBiz + '/api/guarbaseinfo/saveByGuarBasicGuarInfUsufLandVo',
    ...
    onCommit () {
      ...
      _this.$refs.guarUsufLand.$refs.refForm.validate(function (valid) {
        validate = valid;
      });
      if (!validate) {
