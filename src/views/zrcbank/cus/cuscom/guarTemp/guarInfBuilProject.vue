<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <yu-tab-pane label="押品详情信息-在建工程" name="second">
      <guarBuilProject ref="guarBuilProject"></guarBuilProject>
    </yu-tab-pane>
...
import guarBuilProject from '@/views/guarmanage/collateralInfo/guarInfo/guarBuilProject_d1_BillCard';
import yufpNwfInit from '@/components/widgets/YufpNwfInit';

export default {
  // 导入的组件
  components: { guarBaseInfoAdd, guarBuilProject, mapState, yufpNwfInit},
  data: function () {
    return {
      activeName: 'first',
      formType: ''
  ...
  mounted () {
    // const _this = this;
    console.log(this);
    // let datas = _this.$route.meta.params;
    // _this.formType = datas.op_type;
  },
  ...
  methods: {
    ...
    onSave () {
      ...
      yufp.clone(_this.$refs.guarBuilProject.formdata, model.guarInfBuilProject);
      yufp.clone(_this.$refs.guarBaseInfoAdd.formdata, model.guarInfBuilProject);
      ...
      this.$xutils.request({
        // 同步请求
        async: true,
        url: this.$backend.cmisBiz + '/api/guarbaseinfo/saveByGuarBasicBuildingVo',
    ...
    onCommit () {
      ...
      _this.$refs.guarBuilProject.$refs.refForm.validate(function (valid) {
        validate = valid;
      });
      if (!validate) {
