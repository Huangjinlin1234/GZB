<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick">
    ...
    <yu-tab-pane label="押品详情信息-居住用房" name="second">
      <guarLivingRoom ref="guarLivingRoom"></guarLivingRoom>
    </yu-tab-pane>
...
import guarLivingRoom from '@/views/guarmanage/collateralInfo/guarInfo/guarLivingRoom_d1_BillCard';
import yufpNwfInit from '@/components/widgets/YufpNwfInit';
export default {
  // 导入的组件
  components: { guarBaseInfoAdd, guarLivingRoom, yufpNwfInit, mapState},
  data: function () {
    return {
      activeName: 'first'
    };
  ...
  methods: {
    ...
    onSave () {
      ...
      yufp.clone(_this.$refs.guarLivingRoom.formdata, model.guarInfLivingRoom);
      yufp.clone(_this.$refs.guarBaseInfoAdd.formdata, model.guarInfLivingRoom);
      ...
      this.$xutils.request({
        // 同步请求
        async: true,
        url: this.$backend.cmisBiz + '/api/guarbaseinfo/saveByGuarBasicGuarInfLivingRoomVo',
    ...
    onCommit () {
      ...
      _this.$refs.guarLivingRoom.$refs.refForm.validate(function (valid) {
        validate = valid;
      });
      if (!validate) {
