  ...
  <div>
    ...
    <yu-panel title="授信成员信息" :hideFilter="false" :collapseHide="false">
      <yu-xtable ref="refTable" row-number :data-url="`${$backend.cmisBiz}/api/lmtgrpmemrel/getMemrelList?ids=1`" request-type="POST">
        ...
        <yu-xtable-column label="敞口额度" prop="openLmtAmt" data-code="NATIONALITY"></yu-xtable-column>
        <yu-xtable-column label="低风险额度" prop="lowRiskLmtAmt" data-code="NATIONALITY"></yu-xtable-column>
        ...
        <yu-xtable-column label="操作">
          <template slot-scope="scope">
            <yu-button type="primary" size="small" @click="handlePerfectInfo(scope.$index, scope.row)">完善申报信息</yu-button>
          </template>
        </yu-xtable-column>
      </yu-xtable>
    </yu-panel>
...
export default {
  ...
  methods: {
    ...
    // 操作列
    handlePerfectInfo: function (index, row) {
      yufp.logger.info(index, row);
    },
