  <!--
    @created by xiamc
    @updated by
    @description 集团客户预授信细化，待备用（改为模板工厂实现）
  -->
<template>
    ...
    <yu-row>
      <yu-col :sm="6" :xs="24">
        <yu-menu class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose" theme="light" @select="selectFn" :default-active="activeIndex">
          <yu-submenu index="1">
            <template slot="title"
              ><i class="el-icon-message"></i>集团客户预授信细化</template
            >
            <yu-menu-item-group>
              <template slot="title"
                >授信方案信息</template
              >
              <yu-menu-item index="1">集团授信基本信息</yu-menu-item>
              <yu-menu-item index="2">集团成员授信明细</yu-menu-item>
              <yu-menu-item index="3">成员客户限额及债项评级</yu-menu-item>
              <yu-menu-item index="4">集团预授信细化调查报告</yu-menu-item>
            </yu-menu-item-group>
            ...
            <yu-menu-item index="113">集团统一额度视图</yu-menu-item>
      ...
      <yu-col :sm="18" :xs="24">
        <memLmtDetail v-if="activeIndex == 2" ref="memLmtDetail"></memLmtDetail>
        <cusCorpMgr v-if="activeIndex == 3" ref="cusCorpMgr"></cusCorpMgr>
        <cusCorpStock v-if="activeIndex == 4" ref="cusCorpStock"></cusCorpStock>
        <lmtPreAppBaseInfo v-if="activeIndex == 1" ref="lmtPreAppBaseInfo"></lmtPreAppBaseInfo>
      </yu-col>
    </yu-row>
  </div>
</template>
...
import memLmtDetail from '@/views/zrcbank/biz/lmtGrpApp/lmtSubInfo/memLmtDetail';

export default {
  components: { cusCorpBase, cusCorpBaseTempLoginList, cusCorpBaseAddress, cusCorpBaseProp, indivCusFncStatBase, cusCorpMgr, cusCorpStock, lmtPreAppBaseInfo, memLmtDetail },
  data: function () {
    return {
      color: [],
      activeIndex: 1,
