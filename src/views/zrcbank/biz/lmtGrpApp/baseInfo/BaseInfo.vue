        ...
        <yu-panel title="授信基本信息" is-collapse>
          <yu-xform ref="refForm" label-width="100px" v-model="baseFormdata">
            <yu-xform-group>
              ...
              <yu-xform-item v-show="grpCusIdIsShow" label="集团客户编号" ctype="input" name="grpCusId" :rules="baseFormRules[2]" data-code="EDUCATION_TYPE"></yu-xform-item>
              <yu-xform-item label="客户类型" ctype="input" name="cusType" :rules="baseFormRules[3]" data-code="IDENT_TYPE"></yu-xform-item>
              <yu-xform-item label="集团客户名称" ctype="input" name="grpCusName" :rules="baseFormRules[3]" data-code="IDENT_TYPE" colspan="24"></yu-xform-item>
              <yu-xform-item v-show="cesuanIsShow" label="测算最高流动资金贷款额度" ctype="input" name="cardType" :rules="baseFormRules[3]" data-code="IDENT_TYPE"></yu-xform-item>
              ...
              <yu-xform-item v-show="lmtTermIsShow" label="宽限期（月）" ctype="input" name="lmtGraperTerm" :rules="baseFormRules[3]" data-code="IDENT_TYPE"></yu-xform-item>
            </yu-xform-group>
          </yu-xform>
        </yu-panel>
        <yu-panel title="原授信批复信息" is-collapse>
          <yu-xform ref="refForm1" label-width="100px" v-model="baseFormdata2">
            <yu-xform-group>
              ...
              <yu-xform-item label="宽限期限" ctype="input" name="origiLmtTerm"></yu-xform-item>
              <yu-xform-item label="原敞口额度合计(元)" ctype="input" name="openTotalLmtAmt"></yu-xform-item>
              <yu-xform-item label="低风险额度(元)" ctype="input" name="lowRiskTotalLmtAmt"></yu-xform-item>
            </yu-xform-group>
          </yu-xform>
        </yu-panel>
...
export default {
  ...
  mounted() {
      const flagtest = this.getFactory().contextData.flagtest;
      console.log(flagtest);

      //1.先获取标记的数值,跳转到当前的界面
      if(flagtest == "01"){
      //2.因为是全部字段隐藏相关字段
      this.grpCusIdIsShow = false;
      this.cesuanIsShow = false;
      this.lmtTermIsShow = false;
      }
      var _this = this;
      //赋值全量显示
      var retdata;
