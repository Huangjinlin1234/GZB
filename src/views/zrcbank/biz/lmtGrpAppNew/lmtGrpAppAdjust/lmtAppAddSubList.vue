  ...
  <div>
    <yu-panel title="授信新增分项明细" :hideFilter="false" :collapseHide="false">
      ...
      <yu-xtable :data="lmtAppSubList" style="width: 100%;margin-bottom: 20px;" row-key="subPrdSerno" border
        ...
        <yu-xtable-column prop="lmtBizTypeProp" label="授信品种类型属性"  data-code="STD_PRD_TYPE_PROP" hide-column></yu-xtable-column>
        ...
        <yu-xtable-column prop="origiLmtAccSubPrdAmt" label="调剂前授信额度(元)"  :formatter="Currency"></yu-xtable-column>
        <yu-xtable-column prop="lmtAmt" label="调剂后授信额度(元)" :formatter="Currency"></yu-xtable-column>
        <yu-xtable-column prop="origiLmtAccSubPrdAmt" label="原授信额度"  hide-column></yu-xtable-column>
        <yu-xtable-column prop="lmtAmt" label="授信额度（元）" hide-column :formatter="Currency"></yu-xtable-column>
        ...
        <yu-xtable-column prop="todo" label="操作" width="120">
          <template slot-scope="scope">
            <a class="underline" style="text-decoration:none" v-if="scope.row.isSfcaLmt !='0'&&scope.row.isSfcaLmt !='1'&&type!='VIEW'" @click="editSub(scope.row)" >调剂</a>
      ...
      <yu-xdialog title="授信分项新增" :visible.sync="dialogVisibleSelected" width="800px">
            <yu-xform ref="subForm" label-width="160px" v-model="formdata">
              <yu-xform-group column="2">
                ...
                <yu-xform-item label="担保方式" rules="required" ctype="select" name="guarMode" data-code="STD_ZB_GUAR_WAY" v-model="guarMode" @change="getGuarMode(formdata.guarMode)" exclude-key="21,22,40,50,101,102" colspan="10"></yu-xform-item>
                ...
                <yu-xform-item label="授信期限(月)" ctype="input" name="lmtTerm" hidden></yu-xform-item>
