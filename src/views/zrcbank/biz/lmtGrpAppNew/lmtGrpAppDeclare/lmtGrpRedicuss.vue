  ...
  <div>
    <yu-panel title="再议申请表"  :hideFilter="false" :collapseHide="false">
      <yu-xform ref="lmtBaseForm" label-width="160px" v-model="baseFormdata" :disabled="type=='VIEW'" :form-type="viewType">
        <yu-panel title="集团再议申请概述"  :hideFilter="false" :collapseHide="false">
            <yu-xform-item label="集团再议申请概述" ctype="textarea" name="grpRediAppMemo" rules="required"></yu-xform-item>
      ...
      <yu-panel title="各成员申请内容"  :hideFilter="false" :collapseHide="false">
        <yu-xtable ref="refTable" row-number :data-url="dataUrl" condition-key="condition" :base-params="baseParams" selection-type="radio" request-type="POST">
          ...
          <yu-xtable-column label="上期申请授信情况" prop="lastAppLmtMemo"></yu-xtable-column>
          <yu-xtable-column label="上期总行审批意见" prop="lastAppHeadBankAdvice"></yu-xtable-column>
          <yu-xtable-column label="补充材料说明" prop="spplMaterDesc"></yu-xtable-column>
        </yu-xtable>
      </yu-panel>
    </yu-panel>
...
export default {
  ...
  data () {
    return {
      type: '',
      majorGradeForm: null,
      dataUrl: backend.cmisBiz + '/api/lmtredicussdetail/queryDetailByLmtSerno',
      baseParams: {}
    };
  },
  created () {
  ...
  methods: {
    ...
    printFn: function () {
      ...
      let keydemo = 'frptdemozysqbjt';
      params.src = _this.$backend.frptRptService + 'DGSX-ZYSQB-JT.cpt&op=view&grpSerno=' + params.grpSerno;
      _this.$router.addTab({
        // 路由名称
        name: name,
