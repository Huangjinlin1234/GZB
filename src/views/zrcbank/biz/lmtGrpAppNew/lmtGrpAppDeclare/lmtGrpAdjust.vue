<template>
  <div>
    <yu-panel title="授信额度调剂申请"  :hideFilter="false" :collapseHide="false">
      <yu-xform ref="lmtBaseForm" label-width="160px" v-model="baseFormdata" :disabled="type=='VIEW'" :form-type="viewType">
        <yu-panel title="授信额度调剂申请概述"  :hideFilter="false" :collapseHide="false">
            <yu-xform-item label="集团额度调剂申请概述" ctype="textarea" name="grpAdjustAppMemo" rules="required"></yu-xform-item>
    ...
    <yu-panel title="各成员申请内容"  :hideFilter="false" :collapseHide="false">
      <yu-xtable ref="refTable" row-number :data-url="dataUrl" :base-params="baseParams" selection-type="radio" request-type="POST">
        ...
        <yu-xtable-column label="本次授信额度调剂内容" prop="lmtAdjustContent"></yu-xtable-column>
        <yu-xtable-column label="授信额度调剂理由" prop="lmtAdjustResn"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
...
export default {
  ...
  data () {
    return {
      type: '',
      majorGradeForm: null,
      dataUrl: backend.cmisBiz + '/api/lmtadjustdetail/queryDetailByLmtSerno',
      baseParams: {},
      baseFormdata: {}
    };
  },
  ...
  methods: {
    ...
    doMustCheck: function () {
      ...
      obj.pageId = 'jtedtjsqb';
      obj.pageName = '集团额度调剂申请表';
      yufp.service.request({
        method: 'POST',
    ...
    printFn: function () {
      ...
      params.src = _this.$backend.frptRptService + 'DGSX-EDTJSQS-JT.cpt&op=view&grpSerno=' + params.lmtSerno;
      _this.$router.addTab({
        // 路由名称
        name: name,
