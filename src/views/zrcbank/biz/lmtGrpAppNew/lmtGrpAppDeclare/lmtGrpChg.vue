  ...
  <div>
    <yu-panel title="授信变更申请"  :hideFilter="false" :collapseHide="false">
      <yu-xform ref="lmtBaseForm" label-width="160px" v-model="baseFormdata" :disabled="type=='VIEW'" :form-type="viewType">
        <yu-panel title="授信变更申请概述"  :hideFilter="false" :collapseHide="false">
            <yu-xform-item label="集团变更申请概述" ctype="textarea" name="grpChgAppMemo" rules="required"></yu-xform-item>
      ...
      <yu-panel title="各成员申请内容"  :hideFilter="false" :collapseHide="false">
        <yu-xtable ref="refTable" row-number :data-url="dataUrl" :base-params="baseParams" selection-type="radio" request-type="POST">
          ...
          <yu-xtable-column label="本次授信变更内容" prop="lmtChgContent"></yu-xtable-column>
          <yu-xtable-column label="授信变更理由" prop="lmtChgResn"></yu-xtable-column>
        </yu-xtable>
      </yu-panel>
    </yu-panel>
...
export default {
  ...
  methods: {
    ...
    doMustCheck: function () {
      ...
      obj.pageId = 'jtsxbgsqb';
      obj.pageName = '集团授信变更申请表';
      yufp.service.request({
        method: 'POST',
    ...
    printFn: function () {
      ...
      params.src = _this.$backend.frptRptService + 'DGSX-SXBGSQS-JT.cpt&op=view&grpSerno=' + params.lmtSerno;
      _this.$router.addTab({
        // 路由名称
        name: name,
