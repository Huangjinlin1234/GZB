<template>
  <!--
    @created by 屈文
    @description 集团客户预授信细化列表、历史列表
  -->
  <div class="tab-search">
    <yu-tabs v-model="activeName" @tab-click="handleClick">
      <yu-tab-pane label="集团客户预授信细化申请" name="messageTip">
      ...
      <yu-tab-pane label="集团客户预授信细化历史" name="deal">
        <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
      ...
      <yu-xdialog title="授信台账列表" ref="refLmtAccDialog" :visible.sync="dialogVisibleReply" width="1000px" height="400px">
        ...
        <yu-xtable ref="refLmtAccTable" row-number condition-key="condition" :data-url="dataReplyAccForRefineList" :default-load="true" request-type="POST" size="full">
          <yu-xtable-column label="批复编号" prop="grpReplySerno" width="120"></yu-xtable-column>
          <yu-xtable-column label="集团客户编号">
              <template slot-scope="scope">
                <span style="color: blue;" @click="openDialog(scope.row)">{{ scope.row.grpCusId }}</span>
              </template>
          ...
          <yu-xtable-column label="审批模式" prop="apprMode" width="120" data-code="STD_APPR_MODE"></yu-xtable-column>
          <yu-xtable-column label="终审机构" prop="finalApprBrType" width="120" data-code="STD_FINAL_APPR_BR_TYPE" ></yu-xtable-column>
          <yu-xtable-column label="审批结论" prop="apprResult" width="120" data-code="STD_ZB_APPR_STATUS"></yu-xtable-column>
          <yu-xtable-column label="批复生效日期" prop="replyInureDate" width="120"></yu-xtable-column>
          ...
          <yu-xtable-column label="批复状态" prop="replyStatus" width="120" data-code="STD_XD_REPLY_STATUS"></yu-xtable-column>
        </yu-xtable>
...
<script>
yufp.lookup.reg('STD_APPR_MODE,STD_ZB_APPR_STATUS,STD_XD_REPLY_STATUS,STD_SX_LMT_TYPE,STD_FINAL_APPR_BR_TYPE');
...
export default {
  ...
  data: function () {
    return {
      ...
      dataUrl: backend.cmisBiz + '/api/lmtgrpapp/queryfinerlist',
      dataHisUrl: backend.cmisBiz + '/api/lmtgrpapp/queryfinerhislist',
      dataReplyAccForRefineList: backend.cmisBiz + '/api/lmtgrpreplyacc/getreplyaccforrefine',
      rule: [
        {
          required: true,
          message: '必填项',
  ...
  methods: {
    ...
    addTab: function (jsoPar) {
      var _this = this;
      var name = 'zrcbank/biz/lmtGrpAppNew/lmtGrpAppDetails/lmtGrpAppDetailsAdd';
      _this.$router.addTab({
        name: name,
        title: '集团客户预授信细化发起',
        key: 'key',
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          ...
          if (retvalue == '0') {
            ...
            idList = 'jtysxxhsqb';
            pageList = '集团预授信细化申请表';
            _this.insertMustCheck(grpSerno, '07', idList, pageList);
            _this.$message('保存成功');
            // 返回对应的流水号去查询
    ...
    addNewTab: function (grpSerno) {
      ...
      _this.$router.addTab({
        name: 'zrcbank/biz/lmtGrpAppNew/lmtGrpAppDetails/lmtGrpAppDetailsAdd',
        title: '集团客户授信申报发起',
        key: 'key',
        data: {
          data: grpSerno
