    ...
    <yu-xform ref="refForm" label-width="100px" v-model="formdata" disabled>
      ...
      <yu-panel title="授信成员信息" :hideFilter="false" :collapseHide="false">
        <yu-xtable ref="refTable" row-number :data="lmtMemberInfo" selection-type="radio" request-type="POST">
         ...
         <yu-xtable-column label="本次是否变更">
          <template slot-scope="scope">
          <yu-xform-item ctype="select" data-code="STD_ZB_YES_NO" v-model="isCurtChg[scope.$index]" :disabled="!isInFlow(scope.row)" ></yu-xform-item>
          </template>
          </yu-xtable-column>
          <yu-xtable-column label="操作" width="120">
            <template slot-scope="scope">
              <a class="underline" @click="isChange(scope.row,scope.$index)">保存</a>
              <a class="underline" @click="editSub(scope.row,scope.$index)">完善变更信息</a>
            </template>
          </yu-xtable-column>
        </yu-xtable>
      </yu-panel>
      <yu-panel title="原授信批复信息" :hideFilter="false" :collapseHide="false" :disabled="form2Disabled">
...
export default {
  ...
  methods: {
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          ...
          response.data.forEach(function (item, index) {
            _this.isCurtChg[index] = item.isCurtChg;
          });
        }
      });
    },
    ...
    isChange: function (data, index) {
      ...
      data.isCurtChg = _this.isCurtChg[index];
      yufp.service.request({
        async: false,
        method: 'POST',
