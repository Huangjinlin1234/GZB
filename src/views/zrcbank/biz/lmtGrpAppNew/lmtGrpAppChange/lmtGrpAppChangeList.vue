<template>
  <!--
    @created by 屈文
    @description 集团客户变更申请列表、历史列表
  -->
  <div class="tab-search">
    <yu-tabs v-model="activeName" @tab-click="handleClick">
      <yu-tab-pane label="集团客户授信变更申请" name="messageTip">
      ...
      <yu-tab-pane label="集团客户授信变更历史" name="deal">
        <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
      ...
      <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisibleGuide" width="650px">
        <yu-xform ref="refForm" label-width="140px" v-model="formdata" :disabled="formDisabled">
          <yu-xform-group column="1">
            ...
            <yu-xform-item label="集团客户编号" ctype="input" placeholder="集团客户编号" colspan="19" name="grpCusId" rules="required"></yu-xform-item>
            ...
            <yu-xform-item label="业务类型" rules="required" ctype="select" placeholder="业务类型" name="lmtType" data-code="STD_SX_LMT_TYPE" disabled colspan="22"></yu-xform-item>
          </yu-xform-group>
      ...
      <yu-xdialog title="授信台账列表" ref="refLmtAccDialog" :visible.sync="dialogVisibleReply" width="1000px" height="400px">
        ...
        <yu-xtable ref="refLmtAccTable" row-number condition-key="condition" :data-url="dataReplyAccForRefineList" :default-load="true" request-type="POST" >
          ...
          <yu-xtable-column label="批复状态" prop="replyStatus" data-code="STD_REPLY_STATUS"></yu-xtable-column>
        </yu-xtable>
...
<script>
yufp.lookup.reg('CRUD_TYPE,NATIONALITY,PUBLISH_STATUS,STD_SX_LMT_TYPE,STD_ZB_APPR_STATUS');
...
export default {
  ...
  data: function () {
    return {
      ...
      dataUrl: backend.cmisBiz + '/api/lmtgrpapp/querychglist',
      dataHisUrl: backend.cmisBiz + '/api/lmtgrpapp/querychghislist',
  ...
  methods: {
    ...
    addTab: function (jsoPar) {
      var _this = this;
      var name = 'zrcbank/biz/lmtGrpAppNew/lmtGrpAppChange/lmtGrpAppChangeAdd';
      _this.$router.addTab({
        name: name,
        title: '集团客户授信变更发起',
        key: 'key',
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          ...
          if (retvalue == '0') {
            ...
            idList = 'jtsxbgsqb';
            pageList = '集团授信变更申请表';
            _this.insertMustCheck(grpSerno, '02', idList, pageList);
            _this.$message('保存成功');
            // 返回对应的流水号去查询
    ...
    addNewTab: function (grpSerno) {
      ...
      _this.$router.addTab({
        name: 'zrcbank/biz/lmtGrpAppNew/lmtGrpAppChange/lmtGrpAppChangeAdd',
        title: '集团客户授信变更发起',
        key: 'key',
        data: {
          data: grpSerno
