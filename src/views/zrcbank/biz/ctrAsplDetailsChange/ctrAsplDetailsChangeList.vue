...
<template>
  ...
  <div class="tab-search">
    <yu-tabs v-model="activeName">
      <yu-tab-pane label="资产池协议变更" name="messageTip">
        <yu-panel title="查询条件" :hideFilter="false" :collapseHide="false">
          ...
          <yu-xtable ref="refTable" row-number  selection-type="radio" @row-click="onRowClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl" request-type="POST" condition-key="condition" :base-params="{condition: {'chgFlag':'1','oprType':'01'}}">
            ...
            <yu-xtable-column label="登记人编号" prop="inputId"></yu-xtable-column>
            ...
            <yu-xtable-column label="业务变更" prop="chgFlag" hide-column></yu-xtable-column>
          </yu-xtable>
        </yu-panel>
      </yu-tab-pane>
      </yu-tabs>
        ...
        <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisibleGuide" width="650px">
          <yu-xform ref="refForm" label-width="140px" v-model="formdata" :disabled="formDisabled">
            <yu-xform-group column="1">
              ...
              <yu-xform-item label="原资产池协议编号" ctype="input" placeholder="资产池协议编号" name="contNo" disabled></yu-xform-item>
...
export default {
  ...
  methods: {
    ...
    nextFn: function () {
      ...
      if (this.formdata.contStatus != '200'){
      //  _this.$message({message: '该协议未生效，如要新增，请做协议申请！', type: 'warning'});
      //  return; 
      }
      let op = 'ADD';
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisBiz + '/api/iqpappaspl/chIqpappaspl',
        data:  {condition: JSON.stringify({ contNo: contNo, chgFlag: '1', serno: serno}) },
        callback: function (code, message, response) {
          if (response.code == 0) {
            if (response.data) {
              _this.$xutils.showMsgBox('提示', '协议变更申请成功', 350, 150);
            ...
            _this.$xutils.showMsgBox('提示', '数据获取失败', 350, 150);
            return;
          }
        }
      });
    ...
    doDelete: function () {
      ...
      if (jsoPar.approveStatus != '000' && jsoPar.approveStatus != '992') {
        _this.$xutils.showMsgBox('提示', '仅允许删除【待发起】【打回】数据', 350, 150);
        return;
      }
