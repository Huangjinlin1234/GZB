  ...
  <div>
    ...
    <yu-panel title="变更后的还款方式信息" panel-type="simple">
      ...
      <yu-xform ref="refForm" label-width="160px" v-model="formdata.now" :disabled="nowformDisabled">
        <yu-xform-group>
          <yu-xform-item label="还款方式" name="repayMode" rules="required" @change="showplan" :disabled="pageParams.opType == 'VIEW' ||isSign" ctype="select" data-code="STD_REPAY_MODE" exclude-key="A012,A013,A014,A015,A016,A017,A018,A019,A020,A021,A022,A023,A030,A031,A041" :datacode-filter="datacodeFilterFn"></yu-xform-item>
          <yu-xform-item label="结息间隔周期" name="eiIntervalCycle" rules="required" :hidden='hiddenFlag' ctype="input" :disabled="pageParams.opType == 'VIEW' ||isSign" :colspan="8"></yu-xform-item>
          <yu-xform-item label="" name="eiIntervalUnit" ctype="select" rules="required" :hidden='hiddenFlag' data-code="STD_RATE_ADJ_UNIT" :disabled="pageParams.opType == 'VIEW' ||isSign" :colspan="4"></yu-xform-item>
        </yu-xform-group>
  ...
  methods: {
    ...
    showplan (newval) {
      ...
      if(newval=='A009'){
        ...
        this.$refs.refForm.fields[1].rules = '';//改表单位置的时候需要同步修改！！！！！
        this.$refs.refForm.fields[2].rules = '';//改表单位置的时候需要同步修改！！！！！
        this.formdata.eiIntervalCycle = '';
        this.formdata.eiIntervalUnit = '';
      }else{
        this.hiddenFlag = false;
        this.$refs.refForm.fields[1].rules = 'required';
        this.$refs.refForm.fields[2].rules = 'required';
      }
    ...
    submit () {
      ...
      if(_this.formdata.base.belgLine == '01'){
          startdto.bizType = 'BG017';
      }
      //零售
      if(_this.formdata.base.belgLine == '02'){
          startdto.bizType = 'BG016';
          startdto.param = {
            ...
            prdId : _this.formdata.base.prdId,
            prdName: _this.formdata.base.prdName
          };
      }
      //对公
      if(_this.formdata.base.belgLine == '03'){
          startdto.bizType = 'BG015';
          startdto.param = {
            approveStatus: _this.formdata.base.approveStatus,
            orgLevel: '6',
            lmtAmt: "0",
            openTotalLmtAmt: "0",
            isTZBAppr:'0',
      ...
      if('A' == loginUser.org.orgType){
        ...
        if (loginUser.orgCode.startsWith('80')) {
            startdto.bizType = 'SGH10';// （寿光）
          } else if (loginUser.orgCode.startsWith('81')) {
            startdto.bizType = 'DHH10';// （东海）
          }
      }
    ...
    addFn: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.planformdata.oprType = '00'; // 还款计划调整 00-未生效
        _this.planformdata.billNo = _this.formdata.base.billNo;
      });
    },
