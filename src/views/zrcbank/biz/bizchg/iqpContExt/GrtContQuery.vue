<template>
  <div>
    <yu-panel title="担保合同信息" :hideFilter="false" :collapseHide="false">
      ...
      <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" :pageable="true" :data-url="dataUrl" request-type="post" :base-params="baseParams" :default-load="true">
...
export default{
  ...
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: this.$backend.cmisBiz + '/api/grtguarbizrstrel/querygrtguarcontbyparams',
      baseParams: {contNo: this.pageParams.contNo},
  ...
  methods:{
    infoFn(){
      ...
      params['guarContNo'] = _this.$refs.refTable.selections[0].guarContNo;
      params['type'] = 'VIEW';
