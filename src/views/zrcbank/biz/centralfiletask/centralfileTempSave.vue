  ...
  <div class="blank_template">
    ...
    <yu-panel ref="simplePanel" title="档案信息" panel-type="simple">
      <yu-xform ref="refForm2" label-width="160px" v-model="centralFileFormdata" :form-type="formType">
        <yu-xform-group>
          <yu-xform-item label="是否与现有库位合并" name="isMerge" :disabled="isMergeDisabled" :notice="isMergeNotice" ctype="select" data-code="STD_ZB_YES_NO" :rules="{required: true, message: '必输项不允许为空'}"></yu-xform-item>
...
export default {
    ...
    'centralFileFormdata.isMerge': function (val) {
      if(val){
        if('0' == val){ // 不合并
          // 获取临时库位号
          this.getFileNo();
          this.getTempLocationNo();
          this.setNoBtnVisiable(false);
        }else if('1' == val){ // 合并
          // 控制临时库位号后的按钮显示
          this.setNoBtnVisiable(true);
          // 清空档案编号、临时库位号
          this.centralFileFormdata.tempLocationNo = '';
          this.centralFileFormdata.fileNo = '';
        }
      }
    }
  },
  ...
  methods: {
        ...
        callback: function(code, message, response) {
          if(response.code == '0' && response.data){
            ...
            _this.$nextTick(function(){
              _this.checkBizSerno(_this.taskFormdata.serno, _this.taskFormdata.traceId);
            });
          }else{
    ...
    setNoBtnVisiable(visiable){
      let _this = this;
      if(visiable){
        _this.isMergeNotice = '已存在临时库位';
        _this.showGetButton = true;
        _this.fileNoSpan = 10;
      }else{
        _this.isMergeNotice = '系统默认临时库位';
        _this.showGetButton = false;
        _this.fileNoSpan = 12;
      }
    },
    // 基于业务流水号，查看是否存在同比业务的库位号
    checkBizSerno(serno, traceId){
      let _this = this;
      let model = {
        condition: JSON.stringify({
          accStatus: '01,02', // 01-已暂存 02-已派发 03-已派发
          serno: serno,
          traceId: traceId
        })
      };
      yufp.service.request({
        method: "POST",
        url: `${backend.cmisBiz}/api/centralfileinfo/query`,
        data: model,
        callback: function(code, message, response) {
          if(response.code == '0' && response.data.length > 0){
            _this.setNoBtnVisiable(true);
            _this.isMergeDisabled = false;
          }else{
            _this.setNoBtnVisiable(false);
            _this.centralFileFormdata.isMerge = '0';
            _this.isMergeDisabled = true;
          }
        }
      });
    },
    selectFileNo(){
      ...
      this.$dialog.open('档案台账', 'zrcbank/biz/centralfiletask/centralfileInfoPopList', 1500, 600, model, (rowData) => {
