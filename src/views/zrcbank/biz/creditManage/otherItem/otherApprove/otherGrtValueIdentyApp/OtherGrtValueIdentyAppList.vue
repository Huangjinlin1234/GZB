...
<template>
  <!--
    ...
    @description 授信抵押物价值认定申请信息-列表
  -->
  <div>
    <yu-panel panel-type="simple" title="输入查询条件">
      <yu-xform related-table-name="otherGrtValueIdentyAppTable" form-type="search" ref="searchForm" v-model="searchFormdata" label-width="120px">
        <yu-xform-group :column="4">
          <yu-xform-item label="客户名称" placeholder="客户名称" name="cusName" ctype="input" clearable></yu-xform-item>
          <yu-xform-item label="客户编号" placeholder="客户编号 " name="cusId" ctype="input" fuzzy-query="both" clearable></yu-xform-item>
          <yu-xform-item label="审批表编号" placeholder="审批表编号 " name="serno" ctype="input" clearable></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
    </yu-panel>
    <yu-panel panel-type="simple" title="授信抵押物价值认定申请列表">
      ...
      <yu-xtable ref="otherGrtValueIdentyAppTable" condition-key="condition" selection-type="radio" row-number request-type="POST" :data-url="dataUrl">
...
export default {
  name: 'OtherGrtValueIdentyAppList',
  ...
  mounted: function () {
    // 开启监听表格监听事件
    yufp.globalEventBus.$on('refreshOtherGrtValueIdentyAppTable', this.refreshOtherGrtValueIdentyAppTable);
  },
  destroyed: function () {
    yufp.globalEventBus.$off('refreshOtherGrtValueIdentyAppTable');
  },
  methods: {
    /**
     * 表格刷新
     */
    refreshOtherGrtValueIdentyAppTable: function () {
      let _this = this;
      _this.$refs.otherGrtValueIdentyAppTable.remoteData();
    },
    /**
     * 审批状态格式化
     */
    ...
    addFn: function () {
      let _this = this;
      let path = 'zrcbank/biz/creditManage/otherItem/otherApprove/otherGrtValueIdentyApp/otherGrtValueIdentyAppAdd/OtherGrtValueIdentyAppAddIndex';
      _this.$router.addTab({
        name: path,
        key: new Date().getTime(),
        title: '新增授信抵押物价值认定申请信息',
        data: {
    ...
    modifyFn: function () {
      let _this = this;
      let selData = _this.$refs.otherGrtValueIdentyAppTable.selections;
      if (selData.length !== 1) {
        _this.$message({
          message: '请先选择一条记录',
          type: 'warning'
    ...
    deleteFn: function () {
      let _this = this;
      let selections = _this.$refs.otherGrtValueIdentyAppTable.selections;
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: _this.$backend.cmisBiz + '/api/othergrtvalueidentyapp/deleteInfo/' + selections[0].serno,
    ...
    infoFn: function () {
      let _this = this;
      let selectionsAry = _this.$refs.otherGrtValueIdentyAppTable.selections;
