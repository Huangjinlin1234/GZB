...
<template>
  <!--
    ...
    @description 外币利率定价申请信息-列表
  -->
  <div>
    <yu-panel panel-type="simple" title="输入查询条件">
      <yu-xform related-table-name="otherForRateAppTable" form-type="search" ref="searchForm" v-model="searchFormdata" label-width="120px">
    ...
    <yu-panel panel-type="simple" title="外币利率定价申请列表">
      ...
      <yu-xtable ref="otherForRateAppTable" condition-key="condition" selection-type="radio" row-number request-type="POST" :data-url="dataUrl">
...
export default {
  ...
  mounted: function () {
    // 开启监听表格监听事件
    yufp.globalEventBus.$on('refreshOtherForRateAppTable', this.refreshOtherForRateAppTable);
  },
  destroyed: function () {
    yufp.globalEventBus.$off('refreshOtherForRateAppTable');
  },
  methods: {
    /**
     * 表格刷新
     */
    refreshOtherForRateAppTable: function () {
      let _this = this;
      _this.$refs.otherForRateAppTable.remoteData();
    },
    /**
     * 审批状态格式化
     */
    ...
    addFn: function () {
      let _this = this;
      let path = 'zrcbank/biz/creditManage/otherItem/otherRate/otherForRateApp/otherForRateAppAdd/OtherForRateAppAddIndex';
      _this.$router.addTab({
        name: path,
    ...
    modifyFn: function () {
      let _this = this;
      let selData = _this.$refs.otherForRateAppTable.selections;
      if (selData.length !== 1) {
        _this.$message({
          message: '请先选择一条记录',
          type: 'warning'
    ...
    deleteFn: function () {
      let _this = this;
      let selections = _this.$refs.otherForRateAppTable.selections;
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: _this.$backend.cmisBiz + '/api/otherforrateapp/deleteInfo/' + selections[0].serno,
    ...
    infoFn: function () {
      let _this = this;
      let selectionsAry = _this.$refs.otherForRateAppTable.selections;
