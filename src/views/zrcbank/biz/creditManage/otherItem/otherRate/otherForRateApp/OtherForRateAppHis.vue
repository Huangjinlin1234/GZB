...
<template>
  <!--
    ...
    @description 外币利率定价申请历史
  -->
  <div>
    <yu-panel panel-type="simple" title="输入查询条件">
      <yu-xform related-table-name="otherForRateAppHisTable" form-type="search" v-model="searchFormdata" label-width="120px">
    ...
    <yu-panel panel-type="simple" title="外币利率定价申请历史列表">
      ...
      <yu-xtable ref="otherForRateAppHisTable" condition-key="condition" selection-type="radio" row-number request-type="POST" :data-url="dataUrl">
...
export default {
  name: 'OtherForRateAppHis',
  components: {},
  data: function () {
    return {
      searchFormdata: {},
      dataUrl: this.$backend.cmisBiz + '/api/otherforrateapp/getotherforrateapphis'
    };
  },
  methods: {
    ...
    infoFn: function () {
      let _this = this;
      let selectionsAry = _this.$refs.otherForRateAppHisTable.selections;
    ...
    outputHis () {
      var _this = this;
      var rowData = _this.$refs.otherForRateAppHisTable.selections;
      if (rowData.length !== 1) {
        _this.$message({
          message: '请先选择一条记录',
          type: 'warning'
      ...
      rowData.src = _this.$backend.frptRptService + 'qtsx-wblldj.cpt&serno=' + rowData[0].serno;
      _this.$router.addTab({
        // 路由名称
        name: name,
