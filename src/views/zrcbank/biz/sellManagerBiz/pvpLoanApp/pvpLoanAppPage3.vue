...
export default {
  ...
  data: function () {
    return {
      ...
      baseParams: {condition: {bizSerno: this.$route.meta.params.pvpSerno, oprType: '01'}},
  ...
  methods: {
    ...
    onAdd () {
      ...
      this.$xutils.request({
        // 同步请求
        async: true,
        url: this.$backend.cmisBiz + '/api/pvploanapp/selectbypvpserno',
        data: {pvpSerno: this.$route.meta.params.pvpSerno},
        success: (response, status, xhr) => {
          if (response.code == '0') {
            // 受托支付
            if (response.data.payMode == '1') {
              _this.dialogVisible = true;
              let params = {
                bizSerno: _this.$route.meta.params.pvpSerno
              };
              _this.formdata.bizSerno = _this.$route.meta.params.pvpSerno;
              _this.$nextTick(function () {
                _this.$refs.refForm.resetFields();
                yufp.clone(params, _this.formdata);
              });
            } else {
              this.$message({message: '受托支付有权限操作！'});
            }
          } else {
      ...
      this.$xutils.request({
        ...
        success: (response, status, xhr) => {
          if (response.code == '0') {
            ...
            if (response.data.payMode == '1') {
              // console.log(this);
              let params = _this.$refs.refTable1.selections[0];
              _this.dialogVisible = true;
              _this.$nextTick(function () {
                yufp.clone(params, _this.formdata);
              });
    ...
    savedata () {
      ...
      var index1 = _this.formdata.toppName.indexOf('基金');
      var index2 = _this.formdata.toppName.indexOf('证券');
      var index3 = _this.formdata.toppName.indexOf('国债');
      var index4 = _this.formdata.toppName.indexOf('账务公司');
      var index5 = _this.formdata.toppName.indexOf('期货');
      var index6 = _this.formdata.toppName.indexOf('经纪');
      var index7 = _this.formdata.toppName.indexOf('担保');
      var index8 = _this.formdata.toppName.indexOf('土地');
      // 校验 交易对手名称不能带有基金
      if (index1 != -1 || index2 != -1 || index3 != -1 || index4 != -1 || index5 != -1 || index6 != -1 || index7 != -1 || index8 != -1) {
        _this.$message({message: '交易对手名称不能含有基金、证券、国债、账务公司、期货、经纪、担保、土地等字段'});
        return;
      }
      // 如果是本行账户，默认赋值张家港农商银行
