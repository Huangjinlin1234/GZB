<template>
  <div>
    <el-tabs v-model="tabValue" @tab-click="tabClick" tab-position="top">
      <yu-tab-pane label="放款信息" name="first" >
      <pvpLoanAppView ref="pvpLoanAppView" :bizPageData="childParams"></pvpLoanAppView>
      </yu-tab-pane>

      <yu-tab-pane label="影像信息" name="imageSystem">
        <yu-panel title="影像信息">
          <imageSystem :authority="authority" :s="power" :para="imageBizParam"></imageSystem>
        </yu-panel>
      </yu-tab-pane>

      <yu-tab-pane label="借据打印" name="pvpPrint"  v-if="flag">
      <pvpPrint ref="pvpPrint" :bizPageData="childParams"></pvpPrint>
      </yu-tab-pane>
      ...
      <el-tab-pane label="流程审批" name="second" v-if="showApproveTab">
        ...
        <el-card class="clearfix">
          ...
          <el-form ref="commentForm" label-width="160px" :model="commentInfo" :rules="commentRules" style="margin-top:20px" v-if="hisFlag">
            <el-form-item label="是否提交其他部门审批" v-if="rule1!=''" prop="isSubToOtherDeptCom">
                <yu-xselect v-model="value1" data-code="STD_ZB_YES_NO" placeholder="请选择" :rules="rule1" @change="chooseIsSubToOther(value1)"></yu-xselect>
            </el-form-item>
            <!--hxl增加对资料是否齐全进行判断-->
            <el-form-item label="资料是否提交齐全"  prop="value5"  v-if="value5flag" >
                <yu-xselect v-model="commentInfo.value5" data-code="STD_ZB_YES_NO" placeholder="请选择"></yu-xselect>
            </el-form-item>
            <el-form-item label="是否用印" prop="value6"   v-if="value6flag" >
                <yu-xselect v-model="commentInfo.value6" data-code="STD_ZB_YES_NO" placeholder="请选择"></yu-xselect>
            </el-form-item>


            <el-form-item label="其他审批部门选择" v-if="rule2!=''" prop="subOtherDeptCom">
                <yu-xselect v-model="value2" :options="subOtherDeptComOptions" placeholder="请选择" :rules="rule2"></yu-xselect>
            </el-form-item>
            ...
            <el-form-item label="审批退回原因" v-if="rule3!=''" prop="apprBackReasonType">
                <yu-xselect v-model="value3" data-code="STD_APPR_BACK_REASON_TYPE" placeholder="请选择" :rules="rule3"></yu-xselect>
            </el-form-item>
          ...
          <el-form-item v-for="(node, index1) in agreeNextNodes" style="min-width: 380px" align="left" :key="index1">
            ...
            <el-col :span="9">
              <el-row :gutter="2">
                ...
                <el-col :span="8">
                  <el-button type="primary" @click="agreeSelectNodeUser(node, index1)" v-if="node.isSysUser != true && node.selected == true">选择</el-button>
                </el-col>
              </el-row>
            </el-col>
            <el-col :span="6" v-show="node.nodeType != 'E' && node.copyUserShow == true && node.selected == true">
              <el-row :gutter="2">
                ...
                <el-col :span="8">
                  <el-button type="primary"  @click="agreeSelectCopyUser(node, index1)">抄送</el-button>
                </el-col>
              </el-row>
            </el-col>
          </el-form-item>
        ...
        <el-radio-group v-model="jumpNodeModel" @change="jumpradioChange" style="width: 600px; margin-left: auto">
          <el-form-item v-for="(node, index1) in jumpNodes" style="margin: auto" :key="index1">
            ...
            <el-col :span="4" v-show="node.selected">
              <el-button type="primary" @click="jumpSelectUser(node, index1)">选择</el-button>
            </el-col>
          </el-form-item>
        </el-radio-group>
      </el-form>
    ...
    <el-dialog :title="UserSelect[title]" :visible.sync="UserSelect.showUserDialog">
      ...
      <div style="margin-top: 20px">
        <el-button type="primary" v-if="title == 'jump' || title == 'copy'" @click="UserDialogClose">选取返回</el-button>
...
import pvpLoanAppView from '@/views/zrcbank/biz/sellManagerBiz/pvpLoanApp/approvePvpLoanAppPage';
import imageSystem from '@/views/imageManage/imageSystem';
import pvpPrint from '@/views/zrcbank/biz/sellManagerBiz/pvpLoanApp/pvpPrint';

let data = {};
export default {
  components: {pvpLoanAppView, imageSystem, pvpPrint},
  name: 'InstanceInfo',
  props: {
    bizPageData: {
      type: Object,
  ...
  data: function () {
    ...
    return {
      flag: false,
      pageParams: {pvpSerno: this.$route.params.serno},
      isSignUser: false,
      value1: '',
      value2: '',
      value3: '',
  ...
  computed: {
    ...
    childParams: function () {
      let _this = this;
      const data = _this.bizPageData || _this.$route.params;
      let params = {data};
      return params;
    },
    authority () {
    ...
    power () {
      if (this.bizPageData.instanceInfo.nodeId == '232_6' || this.bizPageData.instanceInfo.nodeId == '232_7' || this.bizPageData.instanceInfo.nodeId == '233_6' || this.bizPageData.instanceInfo.nodeId == '233_7') {
        return '1';
      } else {
        return '2';
      }
    ...
    imageBizParam () {
      let docid = '';
      let imageCode = 'GRXFDKCZJB;GRXFDKCZDY;GRXFDKCZZY;GRXFDKCZBZDB;GRXFDKCZDCCZ';

      yufp.service.request({
        async: false,
        method: 'post',
        url: backend.cmisBiz + '/api/guarcontrelwarrant/selectcoreguarantynobycontno/' + this.bizPageData.instanceInfo.param.contNo,
  ...
  mounted: function () {
    ...
    if (this.bizPageData.instanceInfo.nodeId == '232_6' || this.bizPageData.instanceInfo.nodeId == '232_7') {
      this.flag = true;
    }
    ...
    if (this.bizPageData.instanceInfo.pageType != 'TODO') {
      this.hisFlag = false;
    }
  },
  methods: {
    chooseIsSubToOther: function (data) {
      var _this = this;
      if (data == '1') {
        _this.rule2 = [
          {
    ...
    chooseCommentSign: function (data) {
      ...
      if (data == 'O-12') {
        ...
        if (bizType == 'HZ001' || bizType == 'HZ002' || bizType == 'HZ005' || bizType == 'HZ006' || bizType == 'BG005' || bizType == 'BG008' || bizType == 'BG015' || bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'YX010' || bizType == 'YX011' || bizType == 'YX013' || bizType == 'YX012' || bizType == 'LS005' || bizType == 'LS006' || bizType == 'LS007' || bizType == 'XW002') {
          if (nodeId == '305_25' || nodeId == '307_14' || nodeId == '256_105' || nodeId == '256_91' || nodeId == '243_6' || nodeId == '243_7' || nodeId == '244_8' || nodeId == '245_9' || nodeId == '245_11' || nodeId == '232_7' || nodeId == '232_8' || nodeId == '233_6' || nodeId == '229_5' || nodeId == '296_5' || nodeId == '233_7') {
            _this.value5flag = true;
          }
        }
        if (bizType == 'YX011' || bizType == 'YX013' || bizType == 'YX012' || bizType == 'LS005' || bizType == 'LS006' || bizType == 'LS007' || bizType == 'XW002') {
          if (nodeId == '244_8' || nodeId == '245_9' || nodeId == '245_11' || nodeId == '232_7' || nodeId == '232_8' || nodeId == '233_6' || nodeId == '229_5' || nodeId == '296_5' || nodeId == '233_7') {
            _this.value6flag = true;
          }
        }
      } else {
        _this.value5flag = false;
        _this.value6flag = false;
      }
    },
    /** 风险拦截相关 --- 开始 */
    riskFn: function () {
    ...
    tabClick: function (tab, event) {
      ...
      if (tab.name == 'second') {
        ...
        yufp.service.request({
          ...
          data: {
            mainInstanceId: _this.bizPageData.instanceInfo.mainInstanceId
          },
          callback: function (code, message, response) {
            if (response.code == 0) {
          ...
          if (response.code == 0) {
            ...
            _this.$nextTick(function () {
              try {
                ...
                // 是否提交其他部门审批控制
                if (bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005') {
                  if (nodeId == '256_52') {
                    _this.rule1 = [
                      {
                        required: true,
    ...
    submitFn: function (opType) {
      ...
      var isSubToOtherDeptCom = this.value1;
      var subOtherDeptCom = this.value2;
      var apprBackReasonType = this.value3;
      var isinforcomplete = this.commentInfo.value5;
      var issignet = this.commentInfo.value6;
      var url;
      var _this = this;
      // 审批结论为 同意时
      if (opType == 'O-12') {
        // TYSX02  _139 信贷管理部负责人
        ...
        } else if (bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005') {
          if (nodeId == '256_52') {
            url = '/api/lmtappr/updatelmtappr';
            toOtherCom(url, serno, isSubToOtherDeptCom, subOtherDeptCom);
          } else if (nodeId == '256_105') {
            url = '/api/lmtappr/updatelmtapprauthirty';
            messageBox(url, serno);
          } else {
            submitFun();
          }
        ...
        if (bizType == 'SX001' || bizType == 'SX002' || bizType == 'SX003' || bizType == 'SX004' || bizType == 'SX005' || bizType == 'SX006' || bizType == 'SX007') {
          if (nodeId != '256_9' || nodeId != '256_30' || nodeId != '256_6' || nodeId != '256_280' || nodeId != '256_12' || nodeId != '256_13' || nodeId != '256_28' || nodeId != '256_5' || nodeId != '256_20' || nodeId != '256_144' || nodeId != '256_8' || nodeId != '256_39') {
            url = '/api/lmtappr/updateapprbackreasontype';
            saveApprBackReasonType(url, serno, apprBackReasonType);
          }
        } else if (bizType == 'SX008' || bizType == 'SX009' || bizType == 'SX010' || bizType == 'SX011' || bizType == 'SX012' || bizType == 'SX013' || bizType == 'SX014' || bizType == 'SX015') {
          if (nodeId != '256_9' || nodeId != '256_30' || nodeId != '256_6' || nodeId != '256_280' || nodeId != '256_12' || nodeId != '256_13' || nodeId != '256_28' || nodeId != '256_5' || nodeId != '256_20' || nodeId != '256_144' || nodeId != '256_8' || nodeId != '256_39') {
            url = '/api/lmtgrpappr/updateapprbackreasontype';
            saveApprBackReasonType(url, serno, apprBackReasonType);
          }
        }
        submitFun();
      ...
      // 保存退回原因
      function saveApprBackReasonType (url, serno, apprBackReasonType) {
        var _this = this;
        var params = {
          serno: serno,
      ...
      function toOtherCom (url, serno, isSubToOtherDeptCom, subOtherDeptCom) {
        var _this = this;
        var params = {
          serno: serno,
