        ...
        <yu-panel title="输入查询条件" hideFilter="false" :collapseHide="false">
          <yu-xform related-table-name="refDealTable" form-type="search" v-model="searchFormdata" ref="refForm">
            <yu-xform-group :column="3">
              ...
              <yu-xform-item label="合同编号" placeholder="合同编号" ctype="input" name="contNo"></yu-xform-item>
              <yu-xform-item label="客户编号" placeholder="客户编号" ctype="input" name="cusId"></yu-xform-item>
              <yu-xform-item label="合同状态" placeholder="合同状态" ctype="select" name="contStatus" data-code="STD_CONT_STATUS" :datacode-filter="datacodeFn"></yu-xform-item>
            </yu-xform-group>
          </yu-xform>
          <yu-toolbar>
...
// 合同类型
yufp.lookup.reg(
  'STD_ZB_CONT_TYPE,STD_ZB_CUR_TYP,STD_ZB_GUAR_WAY,STD_CONT_STATUS,STD_LOAN_MODAL,STD_PRD_TYPE_PROP'
);
...
export default {
  ...
  data: function () {
    return {
      ...
      dataHisUrl: backend.cmisBiz + '/api/ctrloancont/tcont/donesignlist',
  ...
  methods: {
    ...
    signFn: function () {
      ...
      _this.$router.addTab({
        // 路由名称
        name: 'zrcbank/biz/ctrLoanCont/ctrTCont/ctrTContSign',
        // 自定义唯一页签key,请统一使用custom_前缀开头
        key: 'custom_sign' + data.iqpSerno, // 必传
        // 页签名称
        title: '贸易融资合同签订',
        // 传递的业务数据，可选配置
        data: {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              ...
              url:
                backend.cmisBiz +
                '/api/ctrloancont/tcont/againsign/' +
                selections[0].contNo,
              callback: function (code, message, response) {
                if (response.code == '0') {
                  if (response.data.rtnCode == '000000') {
