...
<template>
  ...
  <div class="tab-search">
    <yu-tabs v-model="activeName" @tab-click="handleClick">
      <yu-tab-pane label="待签协议" name="messageTip">
        <yu-panel title="待签协议" :hideFilter="false" :collapseHide="false">
          <yu-xform related-table-name="refTable" form-type="search" v-model="searchFormdata" ref="refForm">
            <yu-xform-group :column="4">
              ...
              <yu-xform-item label="协议状态" ctype="select" name="contStatus" data-code="STD_CONT_STATUS" exclude-key="200,600,500,800,700" :datacode-filter="datacodeFilterFn"></yu-xform-item>
            </yu-xform-group>
          </yu-xform>
          <yu-toolbar>
            <yu-button @click="print" type="primary" v-if="checkCtrl('print')">打印</yu-button>
            <yu-button @click="sign" type="primary" v-if="checkCtrl('sign')">签订</yu-button>
            <yu-button @click="logout" type="primary" v-if="checkCtrl('logout')">注销</yu-button>
          ...
          <yu-xtable ref="refTable" row-number :data-url="dataUrl" condition-key="condition" request-type="POST" selection-type="radio">
            ...
            <yu-xtable-column label="协议金额" prop="agrAmt" width:="120" :formatter="formatNumber('0,000.00',2)"></yu-xtable-column>
      ...
      <yu-tab-pane label="历史协议" name="deal">
        <yu-panel title="历史协议" :hideFilter="false" :collapseHide="false">
          <yu-xform related-table-name="refDealTable" form-type="search" v-model="searchFormdata2" ref="refForm" selection-type="radio">
            <yu-xform-group :column="4">
              <yu-xform-item label="客户名称" ctype="input" name="cusName" fuzzy-query="both"></yu-xform-item>
              <yu-xform-item label="担保方式" ctype="select" name="guarMode" data-code="STD_ZB_GUAR_WAY"></yu-xform-item>
              <yu-xform-item label="合同编号" ctype="input" name="contNo"></yu-xform-item>
              <yu-xform-item label="客户编号" ctype="input" name="cusId"></yu-xform-item>
              <yu-xform-item label="协议状态" ctype="select" name="contStatus" data-code="STD_CONT_STATUS" exclude-key="100" :datacode-filter="datacodeFilterFn"></yu-xform-item>
            </yu-xform-group>
          </yu-xform>
          <yu-toolbar>
            <yu-button @click="printHis" type="primary" v-if="checkCtrl('hisprint')">打印</yu-button>
            <yu-button @click="onLogoutHis" type="primary" v-if="checkCtrl('hislogout')">注销</yu-button>
            <yu-button @click="infoFnHis" type="primary" v-if="checkCtrl('hisview')">查看</yu-button>
            <yu-button @click="onReSign" type="primary" v-if="checkCtrl('resign')">重签</yu-button>
          </yu-toolbar>
          <yu-xtable ref="refDealTable" row-number :data-url="dataHisUrl" condition-key="condition" request-type="POST">
            <yu-xtable-column label="合同编号" prop="contNo" width:="120"></yu-xtable-column>
            <yu-xtable-column label="合同类型" prop="contType" width:="120" data-code="STD_CONT_TYPE"></yu-xtable-column>
            <yu-xtable-column label="客户编号" prop="cusId" data-code="NATIONALITY" width:="120">
              <template slot-scope="scope">
                <a class="underline" style="text-decoration:none" @click="viewCus(scope.row)">{{ scope.row.cusId }}</a>
              </template>
            </yu-xtable-column>
            ...
            <yu-xtable-column label="协议起始日" prop="startDate" width:="120"></yu-xtable-column>
            <yu-xtable-column label="协议到期日" prop="endDate" width:="120"></yu-xtable-column>
            <yu-xtable-column label="是否电子用印" prop="isESeal" width:="120" data-code="STD_ZB_YES_NO"></yu-xtable-column>
            <yu-xtable-column label="线上合同启用标志" prop="ctrBeginFlag" data-code="STD_ZB_YES_NO" width:="120"></yu-xtable-column>
            <yu-xtable-column label="责任人" prop="managerId" width:="120" hide-column></yu-xtable-column>
            <yu-xtable-column label="责任机构" prop="managerBrId" width:="120" hide-column></yu-xtable-column>
            <yu-xtable-column label="责任人" prop="managerIdName" width:="120"></yu-xtable-column>
            <yu-xtable-column label="责任机构" prop="managerBrIdName" width:="120"></yu-xtable-column>
...
yufp.lookup.reg(
  'STD_ZB_GUAR_WAY,STD_CONT_STATUS,STD_CONT_TYPE,STD_ZB_CUR_TYP,STD_ZB_YES_NO,STD_ZB_APPR_STATUS'
);
...
export default {
  data: function () {
    return {
      dataUrl: backend.cmisBiz + '/api/ctrhighamtagrcont/tosignlist',
      dataHisUrl: backend.cmisBiz + '/api/ctrhighamtagrcont/donesignlist',
  ...
  methods: {
    ...
    //   params.src = _this.$backend.frptRptService + 'ZGESXXY.cpt&SERNO=' + params.serno;
    //   _this.$router.addTab({
    //     // 路由名称
    //     name: name,
    ...
    sign: function () {
      ...
      _this.$router.addTab({
        // 路由名称
        name: 'zrcbank/biz/ctrHighAmtAgrCont/ctrHighAmtContSign',
        // 自定义唯一页签key,请统一使用custom_前缀开头
        key: 'custom_sign', // 必传
        // 页签名称
        title: '合同签订',
      ...
      _this.$confirm('是否注销?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/ctrhighamtagrcont/contlogout',
    ...
    onLogoutHis () {
      var _this = this;
      let params = _this.$refs.refDealTable.selections;
      if (params == null || params == '') {
        _this.$message({
          message: '请先选择一条记录',
          type: 'warning'
    ...
    onReSign () {
      var _this = this;
      let params = _this.$refs.refDealTable.selections[0];
      if (params == null || params == '') {
        this.$xutils.showMsgBox(
          '提示',
          '必须选择一条记录进行操作!\r\n请重新操作!',
        ...
        (isOK) => {
          if (isOK) {
            this.$xutils.request({
              // 同步请求
              async: false,
              url: this.$backend.cmisBiz + '/api/ctrhighamtagrcont/contReSign',
