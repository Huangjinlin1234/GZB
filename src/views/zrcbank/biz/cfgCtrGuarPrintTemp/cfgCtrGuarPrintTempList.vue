...
<template>
  <!--
    @created by 屈文
    @description 担保合同打印模板配置
  -->
  <div class="tab-search">
    <yu-tabs v-model="activeName">
      <yu-tab-pane label="担保合同模板配置" name="messageTip">
        <yu-panel title="查询条件" :hideFilter="false" :collapseHide="false">
          <yu-xform ref="refForm" form-type="search" v-model="searchFormdata" label-width="120px" :custom-search-fn="customSearch" related-table-name="refTable">
              <yu-xform-group :column="3">
                ...
                <yu-xform-item label="适用合同类型" ctype="select" placeholder="适用合同类型" name="suitGuarContType" data-code="STD_ZB_GUAR_CONT_TYPE"></yu-xform-item>
                <yu-xform-item label="适用担保方式" ctype="select" placeholder="适用担保方式" name="suitGuarMode" data-code="STD_ZB_GUAR_WAY"></yu-xform-item>
              </yu-xform-group>
          </yu-xform>
          <yu-button-drop>
          ...
          <yu-xtable ref="refTable" row-number selection-type="radio" @row-click="onRowClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl" request-type="POST">
            ...
            <yu-xtable-column label="适用担保合同类型" prop="suitGuarContType" data-code="STD_ZB_GUAR_CONT_TYPE"></yu-xtable-column>
            <yu-xtable-column label="适用担保方式" prop="suitGuarMode" data-code="STD_ZB_GUAR_WAY"></yu-xtable-column>
      ...
      <yu-panel title="" :hideFilter="false" :collapseHide="false">
        <yu-xdialog title="担保合同打印模板配置新增向导" :visible.sync="dialogVisibleGuide" width="650px">
          <yu-xform ref="refForm" label-width="140px" v-model="formdata" :disabled="formDisabled">
            <yu-xform-group column="1">
              ...
              <yu-xform-item label="适用担保合同类型" ctype="select" placeholder="适用担保合同类型" name="suitGuarContType" rules="required" data-code="STD_ZB_GUAR_CONT_TYPE"></yu-xform-item>
              <yu-xform-item label="适用担保方式" ctype="select" name="suitGuarMode" rules="required" data-code="STD_ZB_GUAR_WAY" placeholder="适用担保方式"></yu-xform-item>
            </yu-xform-group>
...
<script>
yufp.lookup.reg('STD_ZB_GUAR_CONT_TYPE');
...
export default {
  ...
  data: function () {
    return {
      ...
      searchFormdata: {},
      dataUrl: backend.cmisCfg + '/api/cfgctrguarprinttemp/tosignlist',
      rule: [
        {
          required: true,
          message: '必填项',
  ...
  methods: {
    ...
    nextFn: function () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisCfg + '/api/cfgctrguarprinttemp/savecfgctrguarprinttempinfo',
        data: data,
    ...
    logicDelete: function (jsoPar) {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisCfg + '/api/cfgctrguarprinttemp/logicdelete',
    ...
    addNewTab: function (jsoPar, op) {
      var _this = this;
      _this.$router.addTab({
        name: 'zrcbank/biz/cfgCtrGuarPrintTemp/cfgCtrGuarPrintTempInfo',
        title: '担保合同打印模板配置详情',
        key: jsoPar.tempNo,
        data: {
          data: jsoPar,
