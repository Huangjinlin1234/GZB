      ...
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :form-change="triggerEditChangeEvent" :disabled="formIsDisabled">
        <yu-panel title="基本信息" :hideFilter="false" :collapseHide="false">
          <yu-xform-group :column="2">
            ...
            <yu-xform-item label="适用担保合同类型" ctype="select" name="suitGuarContType" rules="required" placeholder="适用合同类型" data-code="STD_ZB_GUAR_CONT_TYPE"></yu-xform-item>
            <yu-xform-item label="适用报表全称" ctype="input" name="suitReportName" rules="required" placeholder="适用报表全称"></yu-xform-item>
            <yu-xform-item label="适用担保方式" ctype="select" name="suitGuarMode" rules="required" placeholder="适用担保方式" data-code="STD_ZB_GUAR_WAY"></yu-xform-item>
            <yu-xform-item label="是否浮动抵押" ctype="select" name="isFloatPld" rules="required" data-code="STD_ZB_YES_NO" placeholder="是否浮动抵押"></yu-xform-item>
            <yu-xform-item label="质押合同类型" ctype="select" name="pldContType" rules="required" placeholder="质押合同类型" data-code="STD_ZB_PLD_CONT_TYPE"></yu-xform-item>
...
<script>
yufp.lookup.reg('STD_ZB_GUAR_CONT_TYPE,STD_ZB_YES_NO,STD_ZB_GUAR_WAY,STD_MUBAN_STATUS,STD_ZB_PLD_CONT_TYPE');
  ...
  methods: {
    ...
    ininForm: function (tempNo) {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisCfg + '/api/cfgctrguarprinttemp/querycfgctrguarprinttempdatabyparams',
        data: {tempNo: tempNo},
    ...
    save () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisCfg + '/api/cfgctrguarprinttemp/commonsavecfgctrguarprinttempinfo',
