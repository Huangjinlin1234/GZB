    ...
    <yu-tabs v-model="activeName2" type="card" @tab-click="handleClickFn">
      <yu-tab-pane label="信息预录入" name="preIn">
        <yu-xform ref="prefForm" label-width="200px" v-model="preformdata" :rules="preRules" :disabled="formDisable">
          <yu-xform-group>
            <yu-xform-item label="申请类型" placeholder="申请类型" name="applyType" ctype="select" data-code="STD_CARD_APPLY_TYPE"></yu-xform-item>
            ...
            <yu-xform-item label="客户类型" placeholder="客户类型" name="cardCusType" ctype="select"  :options="cardCusTypeOpt"></yu-xform-item>
            ...
            <yu-xform-item label="推荐人名称" placeholder="推荐人名称" name="recomName" ctype="input" disabled></yu-xform-item>
            ...
            <yu-xform-item label="进件渠道" name="appChnl" ctype="input" hidden></yu-xform-item>
          </yu-xform-group>
...
export default {
  ...
  computed: {
    authority () {
      return this.$route.query.type === 'add' || this.$route.query.type === 'edit' ? 'import;scan;delImg' : 'download';
    },
    imageBizParam () {
      let imageBizParam = [
        {
          ...
          'index': {
            'businessid': this.$route.query.serno,
            'custid': '',
            'custname': '',
            'custconduct': '',
            'custconductname': '',
  ...
  watch: {
    ...
    'preformdata.applyType': function (val, oldVal) {
    ...
    'preformdata.applyCardPrd': function (val, oldVal) {
      if (val) {
        ...
        if (val === '000010' || val === '000013') {
          this.cardCusTypeOpt = datacode2.filter(function (op) {
    ...
    'changeFlag': function (val) {
      ...
      this.preformdata.applyCardPrd = this.oldApplyCardPrd;
    }
  },
  mounted: function () {
    this.preformdata.serno = this.$route.query.serno;
    this.preformdata.appChnl = this.$route.query.appChnl;
    this.getData(this.$route.query.serno);
  },
  methods: {
    getData (serno) {
      const _this = this;
    ...
    getManagerName () {
      var _this = this;
      var recomId = this.preformdata.recomId;
      if (recomId == null) {
      ...
      this.$request({
        ...
        if (code == '0') {
          _this.preformdata.recomName = data.userName;
          _this.preformdata.recomPhone = data.userMobilephone;
        } else {
    ...
    submitFn () {
      ...
      this.$request({
        ...
        method: 'POST',
        data: {
          cusName: this.preformdata.cusName,
          certType: this.preformdata.certType,
    ...
    submit (isConsider, secondFlag) {
      this.handleClickFn();
      var bizType = 'XK001';
      if (this.$route.query.appChnl != '02') {
        bizType = 'XK002';
      }
      ...
      startdto.bizType = bizType; // ZYB18 ZYB19 ZYB20 XDB11 XDB01
      startdto.bizId = this.serno;
  ...
  activated () {
    ...
    if (this.type === 'edit' || this.type === 'detail') {
      ...
      this.preformdata.applyType = 'B';
      this.preformdata.recomId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
      this.preformdata.recomName = this.$xutils.getDefaultformulaData('$LoginUserName');
    }
