<template>
  <div>
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :form-change="triggerEditChangeEvent" :disabled="formIsDisabled">
        <yu-panel title="出池基本信息" :hideFilter="false" :collapseHide="false">
          <yu-xform-group :column="2">
            ...
            <yu-xform-item label="出池原因" ctype="input" name="resn" disabled placeholder="出池原因"></yu-xform-item>
            <yu-xform-item label="出池说明" ctype="textarea" name="memo" disabled placeholder="出池说明"></yu-xform-item>
          </yu-xform-group>
        </yu-panel>
      </yu-xform>
...
export default{
  ...
  mounted () {
    ...
    if(_this.$route.meta.params){
      ...
      jsoPar = _this.getFactory().contextData;
    }
    serno = jsoPar.serno;
    if (jsoPar.op == 'VIEW') {
