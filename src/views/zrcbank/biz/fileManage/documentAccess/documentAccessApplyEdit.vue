      ...
      <yu-tab-pane label="档案调阅申请信息" name="first">
        <yu-panel title="档案调阅申请信息" panel-type="simple" is-collapse :collapse-title="collapseTitle">
          <yu-xform ref="applyBaseForm" label-width="140px" v-model="applyBaseData">
            ...
            <yu-xform-item ctype="YuXuser"  name="readId"  label="实际调阅人" rules="required" @select-fn="selectedUserFn"></yu-xform-item>
        ...
        <yu-panel title="档案信息列表" panel-type="simple" is-collapse :collapse-title="collapseTitle">
          <div class="yu-toolBar">
            <yu-button-group>
              <yu-button @click="addDebtFn" >引入</yu-button>
              <yu-button @click="deleteDebtFn">删除</yu-button>
            </yu-button-group>
          </div>
...
export default {
  mixins: [mixin],
  components: { yufpNwfInit, FineReport, coopApproval },
  data: function () {
    return {
      activeName: 'first',
      viewType: '',
      ...
      baseBussParams: {condition: JSON.stringify({ draiSerno: this.$route.meta.params.jsonData.draiSerno}) },
      // ifSave: false, // 是否已保存
  ...
  computed: {
    ...
    childParams: function () {
      let params = {
        serno: this.$route.meta.params.jsonData.draiSerno
      };
      return params;
    }
  },
  created () {
    ...
    _this.fineReportParam = {
      ...
      'index': '&draiSerno=' + _this.$route.meta.params.jsonData.draiSerno
    };
  },
  mounted () {
    this.afterInit();
  ...
  methods: {
    ...
    afterInit () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisBiz + '/api/docreadappinfo/detail',
        data: {draiSerno: _this.$route.meta.params.jsonData.draiSerno},
    ...
    */
    ...
    saveApplyFn () {
      ...
      let url = this.$backend.cmisBiz + '/api/docreadappinfo/update';
      yufp.service.request({
        method: 'POST',
        url: url,
        data: model,
