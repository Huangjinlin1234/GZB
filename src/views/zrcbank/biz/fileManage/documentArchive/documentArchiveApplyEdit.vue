    ...
    <yu-tabs v-model="activeName" @tab-click="handleClick">
      <yu-tab-pane label="档案归档信息" name="first" >
        ...
        <yu-panel title="信贷档案资料清单" panel-type="simple" is-collapse :collapse-title="collapseTitle">
          <yu-xtable ref="archiveTable" row-number :pageable="false" :data-url="archiveDataUrl" :base-params="archiveParam" @loaded="loadFn" request-type="POST" >
            <yu-xtable-column label="材料类别" prop="docTypeData" ctype="input"></yu-xtable-column>
            <yu-xtable-column label="材料清单" prop="docListData" ctype="input"></yu-xtable-column>
            <yu-xtable-column label="资料页数" prop="pagesNum" ctype="yu-num" :min="0" :precision="0" maxlength="5" @change="totalPage"></yu-xtable-column>
            <yu-xtable-column label="是否必须原件" prop="isSourceFlag"   data-code="STD_ZB_YES_NO"></yu-xtable-column>
      ...
      <yu-panel title="档案任务池" panel-type="simple" is-collapse :collapse-title="collapseTitle">
        ...
        <yu-xtable ref="refTable" :base-params="baseParams" reserve-selection request-type="POST"  row-key="fileNo"  condition-key="condition" row-number :data-url="dataUrl1">
...
yufp.lookup.reg('STD_ARCHIVE_MODE,STD_DOC_CLASS,STD_DOC_TYPE,STD_ZB_CERT_TYP,STD_TASK_FLAG,STD_DOC_STAUTS,STD_ZB_YES_NO,STD_FILE_ACC_STATUS,STD_BIZ_SUB_TYPE,STD_FILE_TASK_STATUS,STD_FILE_TASK_TYPE,STD_OPT_TYPE');
export default {
  ...
  data: function () {
    return {
      ...
      archiveDataUrl: this.$backend.cmisBiz + '/api/docarchivematerlist/querybydocserno',
  ...
  methods: {
    ...
    saveFn () {
      ...
      var _this = this;
      var modal = {};
      var table = _this.$refs.archiveTable.tabledata;
      for (let i = 0; i < table.length; i++) {
        table[i].sort = table[i].__vkey + 1;
      }
      yufp.clone(_this.formdata, modal);
      modal.archiveParam = table;
      yufp.service.request({
        method: 'POST',
    ...
    optFn () {
      ...
      let pageFlag = this.checkPage();
      if (pageFlag) {
        _this.$message.warning('资料总页数为0，请先修改档案资料清单！');
        return;
      }
      var selections4 = _this.$refs.refTable.tabledata;
      if (selections4) {
        selections4.forEach((item) => {
          if (item.accStatus != '03' && item.accStatus != '04') {
            flag1 = false;
          }
        });
      }
      if (!flag1) {
        _this.$message.warning('存在未出库的临时档案，无法提交');
        return;
      }
      var selections5 = _this.$refs.refTable2.tabledata;
      if (selections5) {
        selections5.forEach((item) => {
          if (item.taskStatus != '03' && item.taskStatus != '04') {
            flag2 = false;
          }
        });
      }
      if (!flag2) {
        _this.$message.warning('存在未完成的档案任务池任务，无法提交');
        return;
      }
      yufp.service.request({
        method: 'POST',
        ...
        callback: function (code, message, response) {
          ...
          // TODO 提交入库有未知事务问题,暂时在提交入库成功后回调中处理更新失败的情况
          yufp.service.request({
            method: 'POST',
            url: backend.cmisBiz + '/api/docarchiveinfo/updatedocafteropt',
            data: _this.$route.meta.params.jsonData.docSerno,
            callback: function (code, message, response) {
            }
          });
        }
    ...
    checkPage () {
      let totalPage = isNaN(parseInt(this.formdata.totalPage)) == true ? 0 : parseInt(this.formdata.totalPage);
      var totalPageSum = 0;
      var archiveTableData = this.$refs.archiveTable.tabledata;
      for (let i = 0; i < archiveTableData.length; i++) {
        let pagesNum = isNaN(parseInt(archiveTableData[i].pagesNum)) == true ? 0 : parseInt(archiveTableData[i].pagesNum);
        totalPageSum = totalPageSum + pagesNum;
      };
      if (totalPage === 0 || totalPageSum === 0) {
        return true;
      } else {
        return false;
      }
