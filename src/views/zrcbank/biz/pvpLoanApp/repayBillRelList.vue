<template>
  ...
  <div>
    <yu-panel title="偿还借据" :hideFilter="false" :collapseHide="false" :disabled="isFormDisabled">
      <yu-toolbar >
          <yu-button type="primary" @click="addFn" :hidden="isHidden">引入</yu-button>
          <yu-button type="primary" @click="modifyFn" :hidden="isHidden">取消引入</yu-button>
          <yu-button type="primary" @click="claJxhj" :hidden="isHidden">批量试算</yu-button>
    ...
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible" width="1600px" height="600px">
      <yu-panel title="借据台账" :hideFilter="false" :collapseHide="false">
        ...
        <yu-xtable ref="refTable"  row-number :data-url="dataUrlAccLoan" :default-load="true" condition-key="condition" :base-params="baseParams" request-type="POST">
    ...
    <yu-panel title="试算结果信息" :hideFilter="false" :collapseHide="false">
        <yu-xtable ref="refSTable" row-number :data-url="dataUrlSAccLoan" condition-key="condition" request-type="POST" :base-params="jxhjParams" >
          ...
          <yu-xtable-column label="借新本金(元)" prop="newLoanAmt" ctype="yu-num" number-formatter="0,000.00" :disabled="isFormDisabled"></yu-xtable-column>
          <yu-xtable-column label="操作" prop="opr" align="center">
            <template slot-scope="drawScope">
              <yu-button type="primary" size="small" @click="resultSave(drawScope.$index, drawScope.row)">保存</yu-button>
            </template>
          </yu-xtable-column>
        </yu-xtable>
     </yu-panel>
...
export default {
  ...
  methods: {
    ...
    // 保存测算结果
    resultSave (index, row) {
      var _this = this;
      // var cvtCnyAmt = _this.obj.cvtCnyAmt;
      var nextPrcp = row.nextPrcp;
      var newLoanAmt = row.newLoanAmt;
      // if (parseFloat(newLoanAmt) > parseFloat(cvtCnyAmt)) {
      //   _this.$message({ message: '借新本金不能大于本次放款金额！', type: 'warning' });
      //   return;
      // }
      if (parseFloat(newLoanAmt) > parseFloat(nextPrcp)) {
        _this.$message({ message: '借新本金不能大于未到期本金！', type: 'warning' });
        return;
      }
      row.selfPrcp = parseFloat(nextPrcp) - parseFloat(newLoanAmt);
      yufp.service.request({
        method: 'POST',
        url: backend.cmisBiz + '/api/pvpjxhjrepayloan/update',
        data: row,
        callback: function (code, message, response) {
          if (code == 0) {
      ...
      _this.$confirm('此操作将取消引入, 是否继续?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              data: {billNo: selections[0].billNo, serno: _this.obj.pvpSerno},
              url: backend.cmisBiz + '/api/pvploanapprepaybillrell/deleterepaybillrellandcalculateresult',
              callback: function (code, message, response) {
                if (code == 0) {
                  _this.$refs.refTableParent.remoteData();
                  _this.$refs.refSTable.remoteData();
