...
<template>
  ...
  -->
  <div>
    <yu-panel title="制造业" panel-type="simple">
      <div id="opera">
       ...
       <colgroup class="el-table_group">
        ...
        <td rowspan="5">生产</td>
        <td>生产模式</td>
        <td colspan="3"><yu-input type="input" v-model="basicFormData.productMode"></yu-input></td>
        </tr>
        <tr height="50px">
        <td>主要工艺流程</td>
        <td colspan="3"><yu-input type="input" v-model="basicFormData.mainProcessFlow"></yu-input></td>
        </tr>
        <tr height="50px">
        <td>生产周期</td>
        <td colspan="3"><yu-input type="input" v-model="basicFormData.productCycle"></yu-input></td>
        </tr>
        <tr height="50px">
        <td>技术含量</td>
        <td colspan="3"><yu-input type="input" v-model="basicFormData.technicalContent"></yu-input></td>
        </tr>
        <tr height="50px">
        <td>设备装备水平和开台率</td>
        <td colspan="3"><yu-input type="input" v-model="basicFormData.equipmentLevelOpeningRate"></yu-input></td>
        </tr>
        </table>
      </div>
      <yu-panel title="销售" panel-type="simple">
        <yu-toolbar :show-length="8" style="margin-bottom:10px;" v-if="salesTableType">
        ...
        <yu-xtable ref="salesTable" row-number :pageable="false" v-if="salesTableType" :data="salesData">
        ...
        <yu-xform ref="salesForm" label-width="160px" v-model="salesFormData" v-if="salesFormType" :disabled="op =='VIEW'">
          <yu-xform-group :column="2">
            <yu-xform-item label="下游客户分析" name="downstreamCusAnaly" ctype="textarea" @change="salesFn"></yu-xform-item>
            <yu-xform-item label="目前订单情况" name="currOrderStatus" ctype="textarea" @change="salesFn"></yu-xform-item>
            <yu-xform-item label="其他需说明事项" name="saleOtherNeedDesc" ctype="textarea" @change="salesFn"></yu-xform-item>
          </yu-xform-group>
        </yu-xform>
      </yu-panel>
      <yu-panel title="采购" panel-type="simple">
        <yu-toolbar :show-length="8" style="margin-bottom:10px;" v-if="purchaseTableType">
        ...
        <yu-xtable ref="purchaseTable" row-number :pageable="false" v-if="purchaseTableType" :data="purchaseData">
        ...
        <yu-xdialog title="上游供应商分析" :visible.sync="dialogPurchase" width="1000px">
          <yu-xform ref="purchaseDialog" label-width="160px" v-model="dialogFormPurchase">
            <yu-xform-group :column="2">
              ...
              <yu-xform-item label="合作年限" name="coopYear" ctype="input"></yu-xform-item>
              <yu-xform-item label="价格波动情况" name="priceWaveCase" ctype="textarea"></yu-xform-item>
              <yu-xform-item label="年末应收付款余额" name="lastCurrAccouantPay" ctype="yu-num"></yu-xform-item>
            </yu-xform-group>
            <div class="yu-grpButton">
        ...
        <yu-xform ref="purchaseForm" label-width="160px" v-model="purchaseFormData" v-if="purchaseFormType" :disabled="op =='VIEW'">
          <yu-xform-group :column="2">
            <yu-xform-item label="上游供应商分析" name="upperstreamCusAnaly" ctype="textarea" @change="purchaseFn"></yu-xform-item>
            <yu-xform-item label="其他需说明事项" name="buyOtherNeedDesc" ctype="textarea" @change="purchaseFn"></yu-xform-item>
          </yu-xform-group>
        </yu-xform>
      </yu-panel>
      <div class="yu-grpButton">
...
export default {
  ...
  data: function () {
    return {
      ...
      salesTableType: true,
      salesFormType: true,
      purchaseTableType: true,
      purchaseFormType: true,
      salesData: [],
      purchaseData: [],
      dialogSales: false,
      dialogFormSales: {},
  ...
  methods: {
                ...
                callback: function (code, message, response) {
                  if (code == 0) {
                    ...
                    if (response.data.length > 0) {
                      _this.salesFormType = false;
                    } else {
                      _this.salesFormType = true;
                    }
                  }
                }
              });
            }
            if (_this.purchaseTableType) {
              yufp.service.request({
                method: 'POST',
                url:
                  _this.$backend.cmisBiz +
                ...
                callback: function (code, message, response) {
                  if (code == 0) {
                    ...
                    if (response.data.length > 0) {
                      _this.purchaseFormType = false;
                    } else {
                      _this.purchaseFormType = true;
                    }
                  }
                }
              });
    ...
    salesFn: function () {
      var _this = this;
      var downstreamCusAnaly = _this.salesFormData.downstreamCusAnaly;
      var currOrderStatus = _this.salesFormData.currOrderStatus;
      var saleOtherNeedDesc = _this.salesFormData.saleOtherNeedDesc;
      if (
        downstreamCusAnaly != '' ||
        currOrderStatus != '' ||
        saleOtherNeedDesc != ''
      ) {
        _this.salesTableType = false;
      } else {
        _this.salesTableType = true;
      }
    },
    purchaseFn: function () {
      var _this = this;
      var upperstreamCusAnaly = _this.purchaseFormData.upperstreamCusAnaly;
      var buyOtherNeedDesc = _this.purchaseFormData.buyOtherNeedDesc;
      if (upperstreamCusAnaly != '' || buyOtherNeedDesc != '') {
        _this.purchaseTableType = false;
      } else {
        _this.purchaseTableType = true;
      }
    },
    addSalesFn: function () {
      var _this = this;
    ...
    saveSales: function () {
      ...
      if (_this.salesType == 'add') {
        ...
        yufp.service.request({
          ...
          url:
            _this.$backend.cmisBiz +
            '/api/rptoperenergysalesdetail/insertSalesDetail',
    ...
    savePurchase: function () {
      ...
      if (_this.purchaseType == 'add') {
        ...
        yufp.service.request({
          ...
          url:
            _this.$backend.cmisBiz +
            '/api/rptoperenergypurchasedetail/insertPurchaseDetail',
    ...
    saveBtn: function () {
      var _this = this;
      var obj = {};
      obj = _this.basicFormData;
      obj.serno = _this.param.serno;
      if (_this.salesFormType) {
        obj.downstreamCusAnaly = _this.salesFormData.downstreamCusAnaly;
        obj.currOrderStatus = _this.salesFormData.currOrderStatus;
        obj.saleOtherNeedDesc = _this.salesFormData.saleOtherNeedDesc;
      }
      if (_this.purchaseFormType) {
        obj.upperstreamCusAnaly = _this.purchaseFormData.upperstreamCusAnaly;
