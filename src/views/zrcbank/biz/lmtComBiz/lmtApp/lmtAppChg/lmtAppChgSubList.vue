<template>
  <div>
    <yu-panel title="授信变更分项明细" :hideFilter="false" :collapseHide="false">
      <template slot="right">
      ...
      <yu-xtable :data="lmtAppSubList" style="width: 100%;margin-bottom: 20px;" row-key="subPrdSerno" border
        ...
        <yu-xtable-column prop="origiLmtAccSubPrdNo" label="授信分项额度编号" sortable ></yu-xtable-column>
        ...
        <yu-xtable-column prop="origiLmtAccSubPrdAmt" label="原授信额度" :formatter="Currency" ></yu-xtable-column>
        <yu-xtable-column prop="lmtAmt" label="变更后授信额度" :formatter="Currency" ></yu-xtable-column>
        ...
        <yu-xtable-column prop="chgFlag" data-code="STD_CHG_FLAG" label="变更标识"></yu-xtable-column>
        <yu-xtable-column prop="todo" label="操作" width="120">
          <template slot-scope="scope">
            <a class="underline" style="text-decoration:none" v-if="scope.row.chgFlag !='2'&&scope.row.chgFlag !='1'&&scope.row.chgFlag !='0'&&type!='VIEW'&&apprStatus!='111'" @click="editSub(scope.row)">修改</a>
            <a class="underline" style="text-decoration:none" v-if="scope.row.chgFlag !='2'&&scope.row.chgFlag !='1'&&scope.row.chgFlag !='0'&&type!='VIEW'&&apprStatus!='111'" @click="deleteSub(scope.row)">删除</a>
...
export default {
  ...
  mounted () {
    if (this.getFactory().contextData.serno) {
      ...
      this.serno = this.getFactory().contextData.instanceInfo.bizId;
      this.type = this.getFactory().contextData.flowParam.op;
      this.getSubAndPrd(this.getFactory().contextData.instanceInfo.bizId);
    }
  },
  methods: {
    // 字典项过滤
