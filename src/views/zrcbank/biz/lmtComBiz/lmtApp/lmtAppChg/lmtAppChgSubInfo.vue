    ...
    <div>
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent">
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="授信金额" ctype="yu-num" number-formatter="0,000.00" name="lmtAmt" placeholder="授信金额" :disabled="disAble"></yu-xform-item>
...
<script>
yufp.lookup.reg('STD_PRD_TYPE_PROP,STD_ZB_YES_NO,STD_ZB_GUAR_WAY');
export default {
  data: function () {
    return {
      lmtAppSubPrdList: [],
  ...
  methods: {
    ...
    onSave () {
      ...
      if (_this.type == 'REFINE' || _this.type == 'CHG') {
        var origiLmtAccSubAmt = _this.formdata.origiLmtAccSubAmt - 0;
        var lmtAmt = _this.formdata.lmtAmt - 0;
        if (origiLmtAccSubAmt < lmtAmt) {
          _this.formdata.lmtAmt = _this.lmtAmt;
          _this.$xutils.showMsgBox('提示', '当前授信金额不允许增加');
          return;
        }
      }
