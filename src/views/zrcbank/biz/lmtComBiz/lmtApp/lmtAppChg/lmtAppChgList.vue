...
<template>
  <!--
    ...
    @updated by
    @updated by
    @description 单一客户授信变更列表
  -->
  <div class="tab-search">
    <yu-tabs v-model="activeName" @tab-click="handleClick">
      <yu-tab-pane label="单一客户授信变更申请" name="lmtChgApp">
        <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
            <yu-xform related-table-name="refTable" form-type="search" v-model="searchFormdata" label-width="120px">
              <yu-xform-group :column="3">
                ...
                <yu-xform-item label="审批状态"  :datacode-filter="datacodeFilterFnStatus" ctype="select" data-code="STD_ZB_APPR_STATUS"></yu-xform-item>
              </yu-xform-group>
            </yu-xform>
          </yu-panel>
          <yu-panel title="单一客户授信变更申请列表" :hideFilter="false" :collapseHide="false">
            <yu-toolbar>
          ...
          <yu-xdialog :visible.sync="dialogVisibleSelected" width="650px">
            <yu-xform ref="refForm" label-width="120px" v-model="formdata" :disabled="formDisabled">
              <yu-xform-group column="1">
                ...
                <yu-xform-item label="业务类型" rules="required" ctype="select" name="lmtType" data-code="STD_SX_LMT_TYPE" value="02" disabled colspan="22"></yu-xform-item>
                <yu-xform-item label="原授信批复台账编号" rules="required" ctype="select" name="accNo" hidden="true" disabled colspan="22"></yu-xform-item>
              </yu-xform-group>
          ...
          <yu-xdialog title="授信台账列表" ref="refLmtAccDialog" :visible.sync="showDialog">
            <yu-xform related-table-name="refLmtAccTable" form-type="search" v-model="searchFormdata1" label-width="120px">
              <yu-xform-group :column="3">
                <yu-xform-item label="客户编号" name="cusId" ctype="input" fuzzy-query="both"></yu-xform-item>
                <yu-xform-item label="客户名称" name="cusName" ctype="input" fuzzy-query="both"></yu-xform-item>
                <yu-xform-item label="批复编号" name="accNo" ctype="input" fuzzy-query="both"></yu-xform-item>
              </yu-xform-group>
            </yu-xform>
            <yu-xtable ref="refLmtAccTable" row-number :data-url="dataReplyAccList" :base-params="replyAccBaseParams" condition-key="condition" :default-load="true" requestType="POST" size="full">
          ...
          <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" :data-url="dataUrl" :base-params="baseParams" requestType="POST">
            ...
            <yu-xtable-column label="授信类型" prop="lmtType" data-code="STD_SX_LMT_TYPE" width="120" hide-column></yu-xtable-column>
      ...
      <yu-tab-pane label="单一客户授信变更历史" name="lmtChgHis">
        <yu-panel title="输入查询条件" :hideFilter="false" :collapseHide="false">
          ...
          <yu-panel title="单一客户授信变更历史列表" :hideFilter="false" :collapseHide="false">
            <yu-toolbar>
...
yufp.lookup.reg('STD_ZB_CUS_TYP,STD_REPLY_STATUS,STD_APPR_MODE,STD_FINAL_APPR_BR_TYPE');
...
export default {
  ...
  data: function () {
    return {
      ...
      searchFormdata1: {},
      searchFormdata2: {},
      dataUrl: backend.cmisBiz + '/api/lmtapp/getlmtmodify',
      dataUrlHis: backend.cmisBiz + '/api/lmtapp/getlmtmodifyhis',
      dataReplyAccList: backend.cmisBiz + '/api/lmtreplyacc/getlmtacclist',
      replyAccBaseParams: {
        condition: {
          accStatus: '01',
          isGrp: '0'
  ...
  methods: {
    ...
    */
    modifyFn: function () {
      ...
      // rowData['model_group_no'] = 'CMG000654';
      ...
      _this.$confirm('是否确认变更此纪录?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/lmtapp/onmodify',
              data: accNo,
              callback: function (code, message, response) {
                if (response.code == '0') {
                  ...
                  if (rtnData.rtnCode == '0000') {
                    ...
                    if (_this.formdata.cusId.startsWith('8')) {
                      idList = 'sxbgjbxx,khxejzxpj,ldzjcsb,sxbgsqs';
                      pageList = '授信变更基本信息,客户限额及债项评级,流动资金测算表,授信变更申请书';
                    } else {
                      idList = 'sxbgjbxx,sxbgsqs';
                      pageList = '授信变更基本信息,授信变更申请书';
                    }
                    _this.insertMustCheck(response.data.serno, _this.formdata.lmtType, idList, pageList);
                    _this.dialogVisibleSelected = false;
                    _this.$refs.refTable.remoteData();
                    // var jsoPar = [];
                    // jsoPar['model_group_no'] = 'CMG000654';
                    // jsoPar['op'] = 'ADD';
                    // jsoPar['flag'] = 'add';
