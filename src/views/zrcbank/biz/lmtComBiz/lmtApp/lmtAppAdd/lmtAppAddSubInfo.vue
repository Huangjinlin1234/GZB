    ...
    <div>
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent">
        <yu-xform-group :column="2">
          ...
          <yu-xform-item :label="origiLmtAccSubAmtLabel" ctype="yu-num" number-formatter="0,000.00" placeholder="原授信金额" name="origiLmtAccSubAmt" disabled :hidden="origiLmtAccSubAmtHidden"></yu-xform-item>
          <yu-xform-item :label="lmtAmtLabel" ctype="yu-num" number-formatter="0,000.00" name="lmtAmt" rules="required" placeholder="授信金额" :disabled="disAble"></yu-xform-item>
          <yu-xform-item label="授信期限" ctype="input" placeholder="授信期限" name="lmtTerm" hidden></yu-xform-item>
          <yu-xform-item label="是否预授信额度" ctype="select" name="isPreLmt" data-code="STD_ZB_YES_NO" placeholder="是否预授信额度" :colspan="12"></yu-xform-item>
...
export default {
  ...
  data: function () {
    return {
      ...
      lmtAmt: '',
      lmtAmtLabel: '授信金额',
      origiLmtAccSubAmtLabel: '原额度分项金额',
      origiLmtAccSubAmtHidden: true,
      serno: '',
      lmtTerm: '',
      lmtGraperTerm: ''
  ...
  mounted () {
    ...
    // 根据分项流水号查询 授信类型
    this.queryLmtTypeBySubSerno(this.getFactory().contextData.subSerno);
    this.getLmtTerm();
  },
  methods: {
    ...
    // 根据分项流水号查询 授信类型
    queryLmtTypeBySubSerno (value) {
      var _this = this;
      _this.$xutils.request({
        // 同步请求
        async: false,
        url: this.$backend.cmisBiz + '/api/lmtappsub/selectlmtappdatabysubserno',
        data: _this.formdata.subSerno,
        success: (response, status, xhr) => {
          if (response.code == '0') {
            if (response.data.lmtType == '08') {
              _this.origiLmtAccSubAmtHidden = false;
              _this.lmtAmtLabel = '调剂后授信金额';
              _this.origiLmtAccSubAmtLabel = '调剂前授信金额';
            }
          } else {
            _this.$xutils.showMsgBox('提示', response.message);
          }
