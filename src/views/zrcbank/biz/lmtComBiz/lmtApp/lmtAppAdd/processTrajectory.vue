...
export default {
  ...
  mounted () {
    var _this = this;    
    let contextObj = _this.getFactory().contextData;
    let routeObj = _this.$route.meta.params;
    if (_this.pageParams) {
      _this.serno = _this.pageParams.serno;
    } else if (routeObj.serno) {
      _this.serno = routeObj.serno;
    } else if (routeObj.pvpSerno) {
      _this.serno = routeObj.pvpSerno;
    } else if (routeObj.grpSerno) {
      _this.serno = routeObj.grpSerno;
    } else if (contextObj.biz_serno) {
      _this.serno = contextObj.biz_serno;
    } else if (contextObj.pvpSerno) {
      // 出账申请流水号
      _this.serno = contextObj.pvpSerno;
    } else if (contextObj.grpSerno) {
      // 集团授信申请流水号
      _this.serno = contextObj.grpSerno;
    } else if (contextObj.biz_serno) {
      // 集团授信申请流水号
      _this.serno = contextObj.biz_serno;
    }
    ...
    if (!_this.serno || _this.serno === '') {
      _this.serno = this.getFactory().contextData.instanceInfo.bizId;
    }
    this.AfterInit();
  },
  methods: {
