...
<template>
  ...
  <div class="tab-form">
    <yu-tabs v-model="tabName">
      <yu-tab-pane label="变更申请信息" name="base">
        <yu-panel title="批复基本信息" :hideFilter="false" :collapseHide="false">
          <yu-xform ref="refForm" label-width="120px" v-model="formdata" class="yu-form-gap">
            <yu-xform-group>
              ...
              <yu-xform-item label="终审机构" disabled ctype="select" name="finalApprBrType" data-code="STD_FINAL_APPR_BR_TYPE"></yu-xform-item>
              <yu-xform-item label="审批结论" disabled ctype="select" name="replyResult" data-code="STD_ZB_APPR_STATUS"></yu-xform-item>
              <yu-xform-item label="批复状态" disabled ctype="select" name="replyStatus" data-code="STD_XD_REPLY_STATUS"></yu-xform-item>
              <yu-xform-item label="责任人" disabled ctype="input" name="managerId" hidden></yu-xform-item>
              <yu-xform-item label="责任人" disabled ctype="input" name="managerIdName"></yu-xform-item>
              <yu-xform-item label="责任机构" disabled ctype="input" name="managerBrId" hidden></yu-xform-item>
        ...
        <yu-panel title="授信批复分项明细" :hideFilter="false" :collapseHide="false">
          <yu-xtable :data="replySubList" style="width: 100%;margin-bottom: 20px;" row-key="pkId" border
            ...
            <yu-xtable-column prop="lmtChgDrawNo" label="授信分项额度编号" sortable width="180">
              <template slot-scope="scope">
                  <span style="color: blue;" @click="openDialog(scope.row)">{{ scope.row.lmtChgDrawNo }}</span>
              </template>
            </yu-xtable-column>
        ...
        <yu-panel title="变更情况说明" :hideFilter="false" :collapseHide="false">
         <yu-xform v-model="chgdata">
          <yu-xform-group :column="2">
            ...
            <yu-xform-item label="变更事项说明" ctype="textarea" name="replyChgContentMemo" :disabled="newDefined" colspan="24"></yu-xform-item>
            <yu-xform-item label="登记人" disabled ctype="input" name="inputId"></yu-xform-item>
            <yu-xform-item label="登记机构" disabled ctype="input" name="inputBrId"></yu-xform-item>
          </yu-xform-group>
        </yu-xform>
        </yu-panel>

        <yu-form-buttons align="center">
          <yu-button type="primary" @click="updateReplyChg" :disabled="newDefined">暂存</yu-button>
          <yu-button type="primary" @click="updateReplyChg1" :disabled="newDefined">保存</yu-button>
          <yu-button type="primary" @click="docommit" :disabled="newDefined">提交</yu-button>
          <yu-button type="primary" @click="goBack">返回</yu-button>
        </yu-form-buttons>
      </yu-tab-pane>
      <yu-tab-pane label="审批历史" name="asset">
        <yu-panel title="审批历史">
        <template>
            <div>
    ...
    <yu-dialog title="" :visible.sync="dialogTableVisible" :min-height="800" center="true">
      <div>
        <yu-xform ref="refForm1" label-width="120px" :form-type="formType" v-model="formdataSub" :rules="formRules" :form-change="triggerEditChangeEvent">
          <yu-xform-group :column="2">
        ...
        <div>
          <yu-toolbar>
            <yu-button type="primary" @click="addSubPrdFn" v-show="showFn">新增</yu-button>
            <yu-button type="primary" @click="modifySubPrdFn" v-show="showFn">修改</yu-button>
            <yu-button type="primary" @click="viewSubPrd">查看</yu-button>
            <yu-button type="primary" @click="deleteSubPrd" v-show="showFn">删除</yu-button>
          </yu-toolbar>
...
export default {
  ...
  data: function () {
    return {
      ...
      baseParams: {condition: JSON.stringify({serno: this.$route.params.serno || this.bizPageData.instanceInfo.bizId}) },
      pagedata: false,
      nodeHistory: [],
      activeNames: [],
      commentinfo: '',
  ...
  methods: {
    ...
    getReplyChg: function (serno) {
      var _this = this;
      yufp.service.request({
        method: 'POST',
        data: serno,
        url: backend.cmisBiz + '/api/lmtreplychg/selectbyserno',
        callback: function (code, message, response) {
          yufp.extend(_this.formdata, response.data);
        }
      });
    },
    ...
    openDialog: function (a) {
      a['lmtDrawNo'] = a.lmtChgDrawNo;
      this.$dialog.open(
        '授信品种信息',
        'zrcbank/biz/lmtComBiz/lmtReplyAcc/lmtAppSubPrdDetailInfo',
        900,
        600,
        a
      );
