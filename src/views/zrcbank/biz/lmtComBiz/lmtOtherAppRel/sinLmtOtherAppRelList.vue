      ...
      <yu-tab-pane label="其他申请事项审批表" name="rateAppTab">
        <yu-panel title="其他申请事项审批列表" :hideFilter="false" :collapseHide="false" >
          <yu-toolbar>
            ...
            <yu-button  type="primary"  @click="immit"  :hidden=" op == 'VIEW'">引入</yu-button>
            <yu-button  type="primary"  @click="disimmit" :hidden=" op == 'VIEW'">取消引入</yu-button>
          </yu-toolbar>
        <yu-xtable ref="refTable" condition-key="condition" row-number :data-url="dataUrl" :pageable="false" :default-load="false" :base-params="baseParams" selection-type="radio" request-type="POST">
      ...
      <yu-xdialog title="其他事项新增向导" :visible.sync="dialogVisibleGuide" width="650px">
        <yu-xform ref="refForm" label-width="140px" v-model="formdata" >
          <yu-xform-group column="1">
            ...
            <yu-xform-item label="其他事项申报类型" rules="required" ctype="select" placeholder="其他事项申报类型" name="otherAppType" data-code="STD_LMT_OTHER_APP_TYPE" exclude-key="05,06,07,08" :datacode-filter="datacodeFilterFn" colspan="22"></yu-xform-item>
          </yu-xform-group>
...
export default {
  ...
  mounted () {
    let _this = this;
    _this.dataUrl = backend.cmisBiz + '/api/otheritemapp/selectotheritemappdatabyserno';
    // 页面转转
    debugger;
    if (_this.pageParams) {
      _this.serno = _this.pageParams.serno;
      _this.op = _this.pageParams.op;
      // _this.dataUrl = backend.cmisBiz + '/api/otheritemapp/getotheritemapp';
      ...
      if (!_this.op || _this.op == '') {
        if (_this.$route.meta.params.type && (_this.$route.meta.params.type == 'DONE' || _this.$route.meta.params.type == 'HIS')) {
          _this.op = 'VIEW';
        }
      }
    }
  ...
  methods: {
    ...
    datacodeFilterFn: function (opts, datacode, name) {
      return opts.filter(function (op) {
        if (op.key === '01' || op.key === '02' || op.key === '03' || op.key === '04' || op.key === '09' || op.key === '10' || op.key === '11') {
          return true;
        }
        return false;
      });
    ...
    addFn: function () {
      ...
      yufp.service.request({
        ...
        callback: function (code, message, response) {
          ...
          _this.formdata.cusType = response.data.cusType;
          _this.formdata.lmtSerno = response.data.serno;
        }
      });
    ...
    immit: function () {
      ...
      yufp.service.request({
        method: 'POST',
        data: params,
        url: backend.cmisBiz + '/api/lmtapp/changeselectListByPageData',
        callback: function (code, message, response) {
          if (response.code == 0) {
            _this.$refs.refTable.remoteData();
          } else {
    ...
    disimmit: function () {
      ...
      var params = {
        serno: row.serno,
        lmtSerno: _this.serno

      };
      yufp.service.request({
        method: 'POST',
        data: params,
        url: backend.cmisBiz + '/api/lmtapp/changedeleteListByPageData',
        callback: function (code, message, response) {
          if (response.code == 0) {
            _this.$refs.refTable.remoteData();
          } else {
    ...
    nextFn () {
      ...
      if (jsoPar.otherAppType == '05' ||
      jsoPar.otherAppType == '06' ||
      jsoPar.otherAppType == '07' ||
      jsoPar.otherAppType == '08'
      ) {
        _this.$message('当前选取其他事项申报类型不可以跟随授信一起发起处理!');
        return;
      }
      if (jsoPar.otherAppType == '01') {
        if (jsoPar.cusId) {
          _this.$request({
            async: false,
            method: 'GET',
            url: `${backend.cmisCus}/api/cusbase/${jsoPar.cusId}`,
            data: jsoPar.cusId
          }).then((data) => {
            let cusCataLog = data.data.data.cusCatalog;
            // 企业
            if (cusCataLog == '2') {
              _this.$request({
                async: false,
                method: 'POST',
                url: `${backend.cmisCus}/api/cuscorp/${jsoPar.cusId}`,
                data: jsoPar.cusId
              }).then((data) => {
                jsoPar.tradeClass = data.data.data.tradeClass;
                jsoPar.corpScale = data.data.data.corpScale;
              });
            } else {
              // 个人
              let cusIndivUnit = {};
              cusIndivUnit.cusId = jsoPar.cusId;
              _this.$request({
                async: false,
                method: 'POST',
                ...
                // 取单位所属行业
                jsoPar.tradeClass = data.data.data.indivComTrade;
                jsoPar.corpScale = data.data.data.indivComTyp;
              });
            }
          });
        }
          ...
          _this.$request({
            ...
            if (cusCataLog == '2') {
              _this.$request({
                ...
                jsoPar.trade = data.data.data.tradeClass;
                jsoPar.corpQlty = data.data.data.corpQlty;
              });
            } else {
              // 个人
              let cusIndivUnit = {};
              ...
              _this.$request({
                ...
                // 取单位所属行业
                jsoPar.trade = data.data.data.indivComTrade;
              });
            }
          });
        }
      }
      jsoPar.lmtSerno = _this.serno;
      _this.$confirm('是否确认新增?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/lmtapp/insertOtherApproveByLmtSerno',
              data: jsoPar,
              callback: function (code, message, response) {
                if (response.code == '0') {
                  _this.dialogVisibleGuide = false;
                  response.data.lmtSerno = _this.serno;
    ...
    jumpOtherAppUpdatePage (data) {
      ...
      if (otherAppType == '01') {
        ...
        yufp.service.request({
          async: false,
          method: 'POST',
          url: backend.cmisBiz + '/api/otherforrateapp/selectbymodel',
          data: {condition: JSON.stringify({serno: data.serno, oprType: '01'})},
          callback: function (code, message, response) {
            yufp.clone(response.data[0], data);
          }
        ...
        yufp.service.request({
          async: false,
          method: 'POST',
          url: backend.cmisBiz + '/api/otherrecordaccpsignplanapp/selectbymodel',
          data: {condition: JSON.stringify({oprType: '01', serno: data.serno})},
          callback: function (code, message, response) {
            yufp.clone(response.data[0], data);
        ...
        yufp.service.request({
          async: false,
          method: 'POST',
          url: backend.cmisBiz + '/api/othergrtvalueidentyapp/selectByModel',
          data: {condition: JSON.stringify({serno: data.serno, oprType: '01'})},
          callback: function (code, message, response) {
            yufp.clone(response.data[0], data);
            console.log('data====>', data);
