...
<template>
  ...
  -->
  <div>
    <yu-panel title="同业客户额度调整" panel-type="simple">
...
export default {
  ...
  data: function () {
    return {
      ...
      dataParam: { condition: JSON.stringify({ cusType: '3'}) },
      dataUrl: backend.cmisLmt + '/api/apprstrmtablechgapp/queryStrChgAppBySerno',
      dataResult: backend.cmisLmt + '/api/apprstrmtableinfo/queryStrChgAppBySerno',
      formdata: {},
      formdata2: {},
      rule: [
        {
  ...
  mounted () {
    this.Param = { condition: JSON.stringify({ oprType: '01', inputId: this.loginCode, cusType: '3' }) };
    this.dataParam = { condition: JSON.stringify({ oprType: '01', cusType: '3' }) };
  },
  methods: {
    ...
    nextFn: function () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisLmt + '/api/apprstrmtablechgapp/initapprstrmtablechgappinfo',
        data: nextobj,
        callback: function (code, message, response) {
          if (code == 0) {
            ...
            model.lmtType = '07';
            model.routeKey = 'apprStrMtableChgApp' + response.data.appSerno + 'ADD';
            model.formdata = obj;
            ...
            _this.$nextTick(function () {
              this.$router.addTab({
                // 路由名称
                name: 'zrcbank/lmt/peerStrMtableChgApp/peerStrMtableChgAppDetail',
            ...
            _this.$message({
              duration: 4000,
              message: '错误提示：' + response.data.rtnCode + response.data.rtnMsg,
              type: 'warning'
            });
            return;
          }
    ...
    infoFn: function () {
      ...
      var obj = _this.$refs.refTable.selections[0];
      var routeKey = 'apprStrMtableChgApp' + selectionsAry[0].appSerno + 'DETAIL';
      model.apprSerno = selectionsAry[0].apprSerno;
      model.appSerno = selectionsAry[0].appSerno;
      model.routeKey = routeKey;
      model.formdata = obj;
      ...
      _this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisLmt + '/api/apprstrmtablechgapp/logicalDelete/' + selections[0].pkId,
              callback: function (code, message, response) {
    ...
    backToAdd () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisLmt + '/api/apprstrmtablechgapp/valilmtchgappinway/' + apprSerno,
        callback: function (code, message, response) {
          if (code == 0) {
            if (response.data == 0) {
              // 传字段
