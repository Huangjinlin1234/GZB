    ...
    <yu-tabs v-model="tabName">
      <yu-tab-pane label="额度信息" name="base">
        <yu-panel title="额度信息" panel-type="simple">
          <yu-xform ref="refAsmcaForm" label-width="120px" v-model="formdata" :disabled="formDisabled" class="yu-form-gap">
            <yu-xform-group>
              <yu-xform-item label="申请流水号" name="appSerno" ctype="input" disabled></yu-xform-item>
      ...
      <yu-tab-pane label="额度明细" name="ApprCoopSubChgApp">
        <yu-panel title="批复额度分项基础信息调整申请" panel-type="simple">
          ...
          <yu-xtable ref="ApprLmtSubChgAppTable" row-number row-key="pkId" :data-url="urls.ApprLmtSubChgAppUrl" :base-params="indirectParam" :default-load="false" request-type="POST">
        ...
        <yu-xdialog :title="dialogTitle" :visible.sync="dialogVisible" width="1200px">
          <yu-xform ref="refForm" label-width="120px" v-model="formdata2" :disabled="dialogformDisabled">
            ...
            <div class="yu-grpButton">
              ...
              <yu-button  @click="dialogcancelFn" type="primary">取消</yu-button>
            </div>
          </yu-xform>
        </yu-xdialog>
      </yu-tab-pane>
...
yufp.lookup.reg('STD_ZB_LMT_TYPE,STD_ZB_YES_NO,STD_ZB_BIZ_ATTR,STD_CONT_TYPE,STD_ZB_BIZ_STATUS,STD_ZB_APPR_ST,STD_PARTNER_TYPE,STD_ZB_ASSURE_MEANS');
  ...
  methods: {
    ...
    tabcancelFn: function () {
      ...
      if (_this.$route.meta.params) {
        ...
        yufp.router.removeTab('/zrcbank/lmt/peerStrMtableChgApp/peerStrMtableChgAppDetail/' + routeKey);
      } else {
