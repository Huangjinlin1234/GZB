...
<template>
  ...
  -->
  <div>
    <yu-panel title="合作方客户额度调整" panel-type="simple">
      ...
      <yu-xtable ref="refTable" condition-key="condition" row-number :data-url="dataUrl" :base-params="Param" :default-load="false" request-type="POST">
        ...
        <yu-xtable-column label="合作方编号" prop="cusId" width="120"></yu-xtable-column>
        <yu-xtable-column label="合作方名称" prop="cusName" width="120"></yu-xtable-column>
        <yu-xtable-column label="合作方类型" prop="copType" width="120" data-code="STD_PARTNER_TYPE"></yu-xtable-column>
        <yu-xtable-column label="授信总额" prop="lmtAmt" width=""></yu-xtable-column>
    ...
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible" width="1100px" height="600px">
      <yu-panel title="额度数据选择">
        ...
        <yu-xtable ref="selectTable" condition-key="condition" row-number :data-url="dataResult" :base-params="dataParam" request-type="POST">
          ...
          <yu-xtable-column label="已用额度" prop="totalUsedAmt" width="120"></yu-xtable-column>
          <yu-xtable-column label="可用额度" prop="totalnousedamt" width="120">
            <template slot-scope="scope">
              <span>{{ formatterNum(scope.row.totalAvlAmt - scope.row.totalUsedAmt) }}</span>
            </template>
          </yu-xtable-column>
...
export default {
  ...
  data: function () {
    return {
      ...
      dataParam: {},
      dataUrl:
        backend.cmisLmt + '/api/apprcoopchgapp/queryApprCoopChgAppInfoBySerno',
      dataResult: backend.cmisLmt + '/api/apprcoopinfo/queryApprCoopBySerno',
      formdata: {},
      formdata2: {},
      rule: [
        {
  ...
  mounted () {
    this.Param = {
      condition: JSON.stringify({ oprType: '01', inputId: this.loginCode })
    };
  },
  methods: {
    /**
    ...
    nextFn: function () {
      ...
      yufp.service.request({
        method: 'POST',
        url: backend.cmisLmt + '/api/apprcoopchgapp/initApprCoopChgAppInfo',
        data: nextobj,
        callback: function (code, message, response) {
          if (code == 0) {
            ...
            model.lmtType = '03';
            model.routeKey = 'apprCoopChgApp' + response.data.appSerno + 'ADD';
            model.formdata = obj;
            ...
            _this.$nextTick(function () {
              this.$router.addTab({
                // 路由名称
                name: 'zrcbank/lmt/apprCoopChgApp/apprCoopChgAppDetail',
    ...
    modifyFn: function () {
      ...
      var routeKey = 'apprCoopChgApp' + selectionsAry[0].appSerno + 'EDIT';
      model.pkId = selectionsAry[0].pkId;
      model.apprSerno = selectionsAry[0].apprSerno;
    ...
    infoFn: function () {
      ...
      var obj = _this.$refs.refTable.selections[0];
      var routeKey = 'apprCoopChgApp' + selectionsAry[0].appSerno + 'DETAIL';
      model.appSerno = selectionsAry[0].appSerno;
      model.routeKey = routeKey;
      model.formdata = obj;
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              ...
              url:
                backend.cmisLmt +
                '/api/apprcoopchgapp/logicalDelete/' +
                selections[0].pkId,
              callback: function (code, message, response) {
    ...
    backToAdd () {
      ...
      yufp.service.request({
        ...
        url:
          backend.cmisLmt +
          '/api/apprcoopchgapp/valilmtchgappinway/' +
          apprSerno,
        callback: function (code, message, response) {
          if (code == 0) {
            if (response.data == 0) {
