    ...
    <yu-tabs v-model="tabName">
      ...
      <yu-tab-pane label="额度明细" name="ApprCoopSubChgApp">
        <yu-panel title="批复额度分项基础信息调整申请" panel-type="simple">
          <yu-button-drop>
            <yu-button v-show="saveBtnShow" @click="modifyFn">修改</yu-button>
            <yu-button @click="infoFn">详情</yu-button>
            <yu-button v-show="saveBtnShow" @click="addLcrcaFn">新增占用合同信息</yu-button>
          </yu-button-drop>
        ...
        <yu-xdialog :title="dialogTitle" :visible.sync="dialogVisible" width="1200px">
          <yu-xform ref="refForm" label-width="120px" v-model="formdata2" :disabled="dialogformDisabled">
            ...
            <div class="yu-grpButton">
              <yu-button v-show="dialogsaveBtnShow" icon="check" type="primary" @click="dialogsaveFn">保存</yu-button>
              <yu-button icon="yx-undo2" type="primary" @click="dialogcancelFn">取消</yu-button>
            </div>
          </yu-xform>
        </yu-xdialog>
      </yu-tab-pane>
...
yufp.lookup.reg('STD_ZB_LMT_TYPE,STD_ZB_YES_NO,STD_ZB_BIZ_ATTR,STD_CONT_TYPE,STD_ZB_BIZ_STATUS,STD_ZB_APPR_ST,STD_PARTNER_TYPE,STD_PRD_TYPE_PROP');
  ...
  methods: {
    ...
    tabcancelFn: function () {
      ...
      if (_this.$route.meta.params) {
        ...
        yufp.router.removeTab('/zrcbank/lmt/cusStrMtableChgApp/cusStrMtableChgAppDetail/' + routeKey);
      } else {
