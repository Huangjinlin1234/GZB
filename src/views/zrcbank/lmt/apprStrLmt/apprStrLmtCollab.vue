    ...
    <yu-panel title="输入查询条件" panel-type="simple">
      <yu-xform related-table-name="refTable" form-type="search" v-model="searchFormdata" label-width="120px">
        <yu-xform-group :column="2">
          <yu-xform-item label="合作方编号" placeholder="合作方编号" name="cusId" ctype="input"></yu-xform-item>
          <yu-xform-item label="合作方名称" placeholder="合作方名称" name="cusName" ctype="input" fuzzy-query="both"></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
    </yu-panel>
    <yu-panel title="合作方客户额度视图列表" panel-type="simple">
      <yu-button-drop>
        <yu-button @click="infoFn" type="primary">查看</yu-button>
      </yu-button-drop>
      <yu-xtable ref="refTable" condition-key="condition" row-number :data-url="dataUrl" :base-params="Param" selection-type="radio" :default-load="false" requestType="POST">
...
yufp.lookup.reg('CERT_TYPE,CARD_NO,STD_PARTNER_TYPE');
...
export default {
  ...
  data: function () {
    return {
      searchFormdata: {},
      dataUrl: backend.cmisLmt + '/api/apprcoopinfo/queryListByInstuCde',
      formdata: {},
      rule: [
        {
          required: true,
  ...
  mounted () {
    ...
    this.Param = { condition: JSON.stringify({ instuCde: this.instuCde }) };
  },
  methods: {
    /**
     * 详情
    ...
    infoFn: function () {
      ...
      model.copType = selectionsAry[0].copType;
      ...
      _this.$nextTick(function () {
        this.$router.addTab({
          // 路由名称
          name: 'zrcbank/lmt/apprStrLmt/apprStrLmtDetailCollab/apprStrLmtDetailCollab',
