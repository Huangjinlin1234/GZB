...
<template>
  ...
  -->
  <div>
    <yu-panel title="产品户买入返售（信用债）白名单管理">
    ...
    <yu-xdialog :title="viewTitle[viewType]" :visible.sync="dialogVisible" width="650px">
      <yu-xform ref="refForm" label-width="120px" v-model="formdata" :disabled="formDisabled">
        <yu-xform-group>
          ...
          <yu-xform-item label="白名单额度类型" name="lmtWhiteType" :hidden="true" ctype="select" data-code="STD_ZB_LMT_WHITE_TYPE" disabled></yu-xform-item>
          <yu-xform-item label="客户编号" name="cusId" ctype="input" :rules="rule[0]" icon="search" @click="onChange" disabled></yu-xform-item>
          <yu-xform-item label="总行行号" name="aorgNo" ctype="input" disabled></yu-xform-item>
          <yu-xform-item label="客户名称" name="cusName" ctype="input" disabled></yu-xform-item>
          <!-- <yu-xform-item label="机构类型" name="cusType" ctype="select" data-code="STD_ZB_INTB_TYPE"></yu-xform-item> -->
          ...
          <yu-xform-item label="单个产品户买入返售限额（万元）" name="singleResaleQuota" :formatter="formatter" :digit="4" type="num" unit="万"></yu-xform-item>
...
yufp.lookup.reg('STD_ZB_LMT_WHITE_TYPE,STD_ZB_INTB_TYPE,STD_ZB_OPR_TYPE,STD_ZB_INSTU_CDE');
...
export default {
  ...
  data: function () {
    return {
      ...
      exportTemplateUrl: backend.cmisLmt + '/api/lmtwhiteinfo/exportwhiteinfo02',
      excelImportUrl: backend.cmisLmt + '/api/lmtwhiteinfo/importwhiteinfo02',
      excelExportUrl: backend.cmisLmt + '/api/lmtwhiteinfo/exportlmtwhiteinfo02',
      Param: { condition: JSON.stringify({ oprType: '01', lmtWhiteType: '02'}) },
      formdata: {},
      rule: [
        {
          required: true,
  ...
  methods: {
      ...
      this.$dialog.open(
        ...
        params => {
          ...
          yufp.service.request({
            method: 'POST',
            url: backend.cmisLmt + '/api/lmtwhiteinfo/checkSameOrgResaleCusIdIsExist',
    ...
    saveFn: function () {
      ...
      model.singleResaleQuota = this.mul(_this.formdata.singleResaleQuota, 10000);
    ...
    addFn: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdata.lmtWhiteType = '02';
    ...
    modifyFn: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdata.singleResaleQuota = _this.div(_this.formdata.singleResaleQuota, 10000);
      });
    },
    ...
    deleteFn: function () {
      ...
      if (selections[0].sigUseAmt != null && selections[0].sigUseAmt != 0) {
        _this.$message({
