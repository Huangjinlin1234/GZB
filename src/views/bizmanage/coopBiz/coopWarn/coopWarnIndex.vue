<template>
  ...
  <yu-panel title="预警信息列表" :collapseHide="false">
    <template slot="right">
      <yu-button-drop>
        <yu-button type="primary" ref="btn_$query" @click="viewFn()">查看</yu-button>
      </yu-button-drop>
    </template>
    <yu-xtable ref="refTable" request-type="POST" condition-key="condition" :pageable="true" style="margin-top:10px" row-number :data-url="url" :base-params="riskParam" :default-load="false">
...
export default {
  ...
  created () {
    ...
    this.riskParam = { condition: { custId:this.$route.meta.params ? this.$route.meta.params.partnerNo : this.$route.params.partnerNo}};
  },
  mounted () {
