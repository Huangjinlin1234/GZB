    ...
    <div id="d1_B_B_A_BillCardContent">
      <yu-xform ref="refForm" label-width="200px" :form-type="operate" :hidden-rule="false" v-model="formdata" :rules="formRules">
        ...
        <yu-xform-group :column="2">
          <yu-xform-item label="总合作额度(元)" ctype="yu-num" number-formatter="0,000.00" maxlength="14" name="totlCoopLmtAmt" rules="required" placeholder="总合作额度(元)" @blur="calculateCoopLmtAmt"></yu-xform-item>
          ...
          <yu-xform-item label="合作期限(月)" ctype="yu-num" name="coopTerm" rules="required" maxlength="5" placeholder="合作期限(月)"  :precision="0" :min="1" ></yu-xform-item>
        </yu-xform-group>
        <yu-xform-group :column="2">
        ...
        <yu-xform-group :column="2">
          <yu-xform-item label="是否白名单控制" ctype="select" name="isWhiteListCtrl" rules="required" data-code="STD_ZB_YES_NO" placeholder="是否白名单控制" ></yu-xform-item>
        </yu-xform-group>
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="其他相关说明" ctype="textarea" name="otherCorreDesc" v-model="formdata.otherCorreDesc" rules="required" :autosize="{ minRows: 3}" maxlength="500" :colspan="24" placeholder="其他相关说明" ></yu-xform-item>
        </yu-xform-group>
        </yu-panel>
      </yu-xform>
    </div>
...
export default {
  ...
  watch: {
    ...
    'pageParam.coopPlanNo': function (value) {
      if (value) {
        var _this = this;
        _this.$xutils.request({
          type: 'POST',
          url: _this.$backend.cmisLmt + '/api/lmt4inner/cmislmt0039',
          data: JSON.stringify({apprSerno: value}),
  ...
  methods: {
    calculateCoopLmtAmt: function () {
      if (this.formdata.oprType == 1 && this.formdata.totlCoopLmtAmt) {
        const totlCoopLmtAmtUsable = this.formdata.totlCoopLmtAmtUsable || 0;
        this.formdata.totlCoopLmtAmt = this.formdata.totlCoopLmtAmt - totlCoopLmtAmtUsable;
        this.formdata.totlCoopLmtAmtUsed = this.formdata.totlCoopLmtAmt - totlCoopLmtAmtUsable;
        this.formdata.totlCoopLmtAmtUsable = totlCoopLmtAmtUsable;
    ...
    checkTotlCoopLmtAmtUsed: function () {
      ...
      if (_this.pageParam.oprType == 2) {
        _this.$xutils.request({
          ...
          data: JSON.stringify({accNo: _this.pageParam.coopPlanNo, instuCde: userInfo.instu.code}),
          success: (response, status, xhr) => {
            if (response.code == 0) {
              // 接口掉不通,模拟假数据
              _this.formdata.totlCoopLmtAmtUsable = 0;
              const totlCoopLmtAmtUsable = _this.formdata.totlCoopLmtAmtUsable || 0;
              _this.formdata.totlCoopLmtAmtUsed = _this.formdata.totlCoopLmtAmt - totlCoopLmtAmtUsable;
            } else {
