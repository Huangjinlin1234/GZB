...
export default {
  ...
  mounted () {
    ...
    if (this.pageParams && this.pageParams.serno) {
    ...
    // 通用流程进入
    if (this.getFactory().contextData && this.getFactory().contextData.instanceInfo) {
    ...
    // opendialog 通用模板工厂
    if (this.getFactory().contextData && this.getFactory().contextData.serno) {
      serno = this.getFactory().contextData.serno;
    }

    // 通过绑定的children 传过来的数据
    if(this.children && this.children.serno){
