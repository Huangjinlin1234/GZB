...
<script>
import d1Billlist from './ctrLoanExtSearch_d1_BillList.vue'
export default {
  ...
  methods:{
      ...
      if (checkFlg) {
        ...
        this.$xutils.showConfirmBox('提示', '是否作撤销议号为:' + params.ext_ctr_no + '的协议？', 350, 150, confirmFlag => {
          if (confirmFlag) {
            ...
            params.logout_date = this.$xutils.dateFormat('yyyy-MM-dd', new Date());
            ...
            if (jsoRt) {
              ...
              this.$xutils.showMsgBox('提示', '撤销失败!');
            }
          }
        });
      } else {
    ...
    doPrint() {
      ...
      if (ext_ctr_status !== '002') {
        this.$xutils.showMsgBox('提示', '只有生效的协议才能打印!');
        return;
      }
