...
<script>
import d1Billcard from './iqpIdtAdjAppUpdate_d1_BillCard.vue'
export default {
  ...
  methods:{
    ...
    AfterInit() {
      ...
      //获取授信起始日期
      let lmtStarDatr = this.d1_BillCard.getItemValue('lmt_star_date');

      if (lmtStarDatr == '') {
        lmtStarDatr = '2020-09-11';
      }

      const year = lmtStarDatr.substring(0, 4);
      const month = lmtStarDatr.substring(5, 7);
      const day = lmtStarDatr.substring(8, 10);
      ...
      const lmtDocIdate = y + '-' + mon + '-' + day;
      this.d1_BillCard.setItemValue('lmt_doc_idate', lmtDocIdate);
    ...
    // 授信信息查看
    viewLmt() {
      const lmtCtrNo = this.d1_BillCard.getItemValue('lmt_ctr_no');

      if (!(lmtCtrNo && lmtCtrNo != '')) {
        this.$xutils.showMsgBox('提示', '授信协议编号为空,请确认！ ');
        return;
      }

      const lmtInfo = this.findLmtInfo(lmtCtrNo);

      //判定对公对私
      this.queryLmt(lmtCtrNo);
    },

    //授信信息
    findLmtInfo(lmtCtrNo) {
      let returnData = {};
      ...
      this.$xutils.request({
        ...
        type: 'get',
        url: this.$backend.cmisLmt + '/api/lmtctr/' + lmtCtrNo,
    ...
    queryLmt(lmtCtrNo) {
      ...
      this.$dialog.open(
        '',
        'bizmanage/lmtBiz/lmtCtrBiz/lmtCtrDetailsIndex',
        -1,
        -1,
        params,
        null,
