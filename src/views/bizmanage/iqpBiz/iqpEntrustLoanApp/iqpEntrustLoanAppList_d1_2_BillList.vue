<template>
  <div>
    <yu-panel title="委托贷款合同申请历史" :hideFilter="false" :collapseHide="false">
      <yu-xform ref="refForm" form-type="search" v-model="searchFormdata" label-width="120px" :custom-search-fn="customSearch">
        <yu-xform-group :column="3">
          ...
          <yu-xform-item label="审批状态" ctype="select" data-code="STD_ZB_APPR_STATUS" exclude-key="111,000,992,991,993" :datacode-filter="datacodeFilterFn" placeholder="审批状态" name="apprStatus"></yu-xform-item>
        </yu-xform-group>
        <yu-button-drop>
          <yu-button ref="btn_doViewHis" type="primary" v-if="checkCtrl('hisview')" @click="customClick('doViewHis')">查看</yu-button>
        </yu-button-drop>
      </yu-xform>
      <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl" :base-params="{ apprStatus: '997 or apprStatus' }" :default-load="true" request-type="POST">
        <yu-xtable-column label="合同编号" prop="contNo" hide-column></yu-xtable-column>
        <yu-xtable-column label="主键" prop="pkId" hidden></yu-xtable-column>
        ...
        <yu-xtable-column label="登记机构" prop="inputBrIdName" hide-column></yu-xtable-column>
        <yu-xtable-column label="登记机构" prop="inputBrIdName" hide-column></yu-xtable-column>
...
<script>
yufp.lookup.reg('STD_ZB_CUR_TYP,STD_ZB_APPR_STATUS,STD_ZB_GUAR_WAY');
...
export default {
  ...
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: this.$backend.cmisBiz + '/api/iqpentrustloanapp/donesignlist',
      baseParams: {},
      deleteUrl: this.$backend.cmisBiz + '/api/iqpentrustloanapp/delete/'
    };
  },
  methods: {
    // 字典项过滤
