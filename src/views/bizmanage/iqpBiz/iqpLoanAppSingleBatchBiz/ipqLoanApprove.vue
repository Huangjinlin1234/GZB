<template>
  ...
  <yu-tabs v-model="activeName" @tab-click="handleClick" :tab-position="'left'">
    <yu-tab-pane label="业务申请信息" name="first">
      <yu-xform ref="refForm" label-width="200px" v-model="formdata" more-fields-flag="true" :disabled="op" >
        ...
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="申请金额" ctype="yu-currency" name="appAmt"  :disabled="op"  @change="iRChange" :rules="numberRules" ></yu-xform-item>
          ...
          <yu-xform-item label="贷款形式" ctype="select" name="loanModel" data-code="STD_ZB_LOAN_TYPE" :disabled="true"></yu-xform-item>
      ...
      <yu-panel title="利率信息" is-collapse panel-type="normal">
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="利率调整日" ctype="select" name="loanRateAdjDay" data-code="STD_ZB_CHANGE_IR2" :disabled="op"  :hidden="!rateFlag"></yu-xform-item>
          <yu-xform-item label="利率调整方式" ctype="select" data-code="STD_ZB_IR_ADJ_MODE3" name="irAdjustType"  disabled @change="irAdjustTypeChangeFn"></yu-xform-item>
          <yu-xform-item label="LPR定价区间" ctype="select" name="lprRateIntval" data-code="STD_ZB_LPR_LIMIT" rules="required" :disabled="op"></yu-xform-item>
          ...
          <yu-xform-item label="LPR浮动点(BP)" ctype="yu-num" name="rateFloatPoint"  @blur="iRChangeTerm2" placeholder="LPR浮动点(BP)" :rules="baseFormRules[0]" :disabled="rateFlag2" ></yu-xform-item>
      ...
      <yu-panel title="还款方式" panel-type="normal" is-collapse>
        <yu-xform-group :column="2">
          <yu-xform-item label="结息方式" ctype="select" name="eiMode"  data-code="STD_ZB_EI_MODE" :disabled="op" :rules="baseFormRules[0]"></yu-xform-item>
          <yu-xform-item label="还款方式" ctype="select" name="repayMode"  data-code="STD_REPAY_TYPE" :disabled="op" :rules="baseFormRules[0]"></yu-xform-item>
      ...
      <yu-panel title="担保信息" panel-type="normal" is-collapse>
        <yu-xform-group :column="2">
          <yu-xform-item label="担保方式" ctype="select" name="guarWay"  data-code="STD_ZB_ASSURE_MEANS" :disabled="op" :rules="baseFormRules[0]"></yu-xform-item>
      ...
      <yu-panel title="登记信息" panel-type="normal" is-collapse>
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="责任机构" ctype="input" name="inputBrId" :disabled="true"></yu-xform-item>
           <yu-xform-item label="业务类型" ctype="input" name="bizType" :disabled="true" hidden></yu-xform-item>
        </yu-xform-group>
      </yu-panel>
    </yu-xform>
  </yu-tab-pane>
  <yu-tab-pane label="影像扫描" name="six">
        <iqpImage :page-params="pageParams"></iqpImage>
  </yu-tab-pane>
  </yu-tabs>
...
<script>
yufp.lookup.reg(
  'USER_STATUS,USER_CLASSIFY,DEPARTMENT,GENDER,IDENT_TYPE,BRANCH,PWD_TYPE,STD_ZB_CONT_TYPE2,STD_ZB_LOAN_TYPE,STD_CONT_MODE'
);
import iqpImage from '@/views/bizmanage/iqpBiz/iqpLoanAppSingleBatchBiz/iqpImageApprove.vue';
export default {
  components: { iqpImage},
  name: 'StandardForm',
  props: {
  ...
  data: function () {
    return {
      ...
      pwdType: 'default',
      hiddenFlg: true,
      pageParams: {iqpSerno: this.bizPageData.instanceInfo.bizId}
  ...
  mounted: function () {
    ...
    this.formdata.author = '1000000000';
    this.formdata.curtLprRate = '0.05';
  },
  methods: {
    // 公积金控制
    fundChange (value) {
    ...
    iRChange (value) {
      var curtLprRate = this.formdata.curtLprRate;
      if (curtLprRate != null && value != null) {
        var monthRepayAmt = curtLprRate * value;
        this.formdata.monthRepayAmt = monthRepayAmt;
      }
      // 本笔月还款额合计=本笔月还款额+本笔公积金贷款月还款额
      var pundLoanMonRepayAmt = this.formdata.pundLoanMonRepayAmt;
      if (pundLoanMonRepayAmt != null && curtLprRate != null && value != null) {
        var monthRepaySum = parseFloat(curtLprRate * value) + parseFloat(pundLoanMonRepayAmt);
