<template>
  <!--
    ...
    @description 银承合同申请历史
  -->
  <div>
    <yu-panel title="银承合同申请历史" :hideFilter="false" :collapseHide="false">
      <yu-xform ref="refForm" form-type="search" v-model="searchFormdata" label-width="120px" :custom-search-fn="customSearch">
        <yu-xform-group :column="3">
          ...
          <yu-xform-item label="审批状态" ctype="select" data-code="STD_ZB_APPR_STATUS" placeholder="审批状态" name="approveStatus" exclude-key="111,000,992,991,993" :datacode-filter="datacodeFilterFn"></yu-xform-item>
        </yu-xform-group>
      </yu-xform>
      <yu-toolbar>
...
<script>
// 注册字典项
yufp.lookup.reg('STD_ZB_CUR_TYP,STD_ZB_GUAR_WAY,STD_PRD_TYPE_PROP');
...
export default {
  ...
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: this.$backend.cmisBiz + '/api/iqpaccpapp/donesignlist',
      baseParams: {},
      deleteUrl: this.$backend.cmisBiz + '/api/iqpaccpapp/delete/'
    };
  },
  methods: {
    // 字典项过滤
    datacodeFilterFn: function (opts, datacode, name) {
      return opts.filter(function (op) {
        if (op.key === '996' || op.key === '997' || op.key === '998') {
          return true;
        }
        return false;
      });
