    ...
    <div id="d1_BillCardContent">
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent">
        <yu-xform-group :column="1">
          ...
          <yu-xform-item label="产品编号" ctype="yu-dg-prd-pop" name="prdId" rules="required" placeholder="产品编号" @select-fn="commonSelectFn" :mapping="{'prdId':'prdId','prdName':'prdName'}" :param="{'prdType':'03'}" @change="isOnlineCvrg"></yu-xform-item>
          <yu-xform-item label="产品名称" ctype="input" placeholder="产品名称" name="prdName" rules="required" disabled></yu-xform-item>
          <yu-xform-item label="担保方式" ctype="select" name="guarMode" rules="required" data-code="STD_ZB_GUAR_WAY" placeholder="担保方式" @change="isAllBail" :disabled="isDisabled" :datacode-filter="datacodeFilterFn1"></yu-xform-item>
          <yu-xform-item label="合同类型" ctype="select" name="contType" rules="required" data-code="STD_CONT_TYPE" placeholder="合同类型"  :disabled="isDisabled" :datacode-filter="datacodeFilterFn"></yu-xform-item>
          ...
          <yu-xform-item label="授信额度编号" ctype="input" name="lmtAccNo" :rules="{ required: isLmtAccNoRequired, message: '字段不能为空' }" placeholder="授信额度编号" icon="search" :triger-click="false" :on-icon-click="handleIconClick"></yu-xform-item>
          <yu-xform-item label="批复编号" ctype="input" placeholder="批复编号" name="replyNo" hidden></yu-xform-item>
...
yufp.lookup.reg('STD_ZB_GUAR_WAY,STD_CONT_TYPE,STD_ZB_YES_NO');
export default{
  name: 'D1BillCard',
  mixins: [mixinForm],
  data: function () {
  ...
  methods: {
    ...
    isOnlineCvrg (value) {
      if (value == '610001') {
        this.formdata.guarMode = '00';
        this.formdata.contType = '2';
        this.isDisabled = true;
      } else {
        this.formdata.guarMode = '';
        this.formdata.contType = '';
      }
    },
    ...
    commonDgContSelect () {
      ...
      var data = { cusId: cusId, lmtAccNo: lmtAccNo, busiType: '08'};
    ...
    handleIconClick: function () {
      ...
      var data = { cusId: cusId, instuCde: instuCde, isPreCrd: '0', suitGuarWay: guarMode, prdId: prdId};
  ...
  watch: {
    'formdata.lmtAccNo': function (val) {
      if (val) {
        this.customParams = {
          condition: {contStatus: '200', lmtAccNo: val, busiType: '08'}
        };
      }
    }
  }
