<template>
  <yu-tabs type="card">
    <yu-tab-pane label="开证合同申请">
      <d1-1-billlist ref="d1_1_BillList"></d1-1-billlist>
    </yu-tab-pane>
    <yu-tab-pane label="开证合同申请历史">
      <d1-2-billlist ref="d1_2_BillList"></d1-2-billlist>
    </yu-tab-pane>
  </yu-tabs>
</template>
<script>
import d11Billlist from './iqpTfLocAppList_d1_1_BillList.vue';
import d12Billlist from './iqpTfLocAppList_d1_2_BillList.vue';
export default {
  ...
  methods: {
    ...
    // 新增
    doAdd () {
      this.$dialog.open('开证合同申请', 'bizmanage/iqpBiz/iqpTfLocApp/iqpTfLocAppAddIndex', 800, 700, null, null);
    },
        ...
        this.$xutils.showConfirmBox('提示', '是否确认删除?\r\n请谨慎操作!', 350, 150, isOK => {
          if (isOK) {
            this.$xutils.request({
              ...
              url: this.$backend.cmisBiz + '/api/iqptflocapp/logicdelete',
    ...
    goToDetailFn (row) {
      ...
      row.topOutsystemCode = 'GJYWHTJB;GJYWDY;GJYWZY;GJYWBZDB;GJYWXYZ;GJYWDCCZ;GJYWSQSQT;';
      row.imageParams = {
        contid: row.contNo,
      ...
      // var _this = this;
      // this.$dialog.open('开证合同申请', 'cfgmanage/productconfig/templetfactory/tempetfactorypreviewIndex', 1500, 800, row, () => {
