<template>
  <div>
    <yu-panel title="普通贷款合同申请列表" :hideFilter="false" :collapseHide="false">
      ...
      <yu-button-drop>
        <yu-button ref="btn_onAdd" type="primary" v-if="checkCtrl('add')" @click="customClick('onAdd')">新增</yu-button>
        <yu-button ref="btn_onUpdate" type="primary" v-if="checkCtrl('edit')" @click="customClick('onUpdate')">修改</yu-button>
        <yu-button ref="btn_onDelete" type="primary" v-if="checkCtrl('delete')" @click="customClick('onDelete')">删除</yu-button>
        <yu-button ref="btn_onCheck" type="primary" v-if="checkCtrl('view')" @click="customClick('onCheck')">查看</yu-button>
      </yu-button-drop>
...
yufp.lookup.reg('STD_ZB_CUR_TYP,STD_ZB_APP_ST,STD_ZB_GUAR_WAY,STD_PRD_TYPE_PROP');
export default{
  name: 'D11BillList',
  mixins: [mixinList],
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: this.$backend.cmisBiz + '/api/iqploanapp/selectIqpLoanAppListData',
      baseParams: {condition: {bizType: '02', belgLine: '03'}, sort: 'iqpSerno desc'},
