<template>
  <div>
    <yu-panel title="普通贷款合同申请历史列表" :hideFilter="false" :collapseHide="false">
      ...
      <yu-button-drop>
        <yu-button ref="btn_onCheck2" type="primary" v-if="checkCtrl('hisview')" @click="customClick('onCheck2')">查看</yu-button>
      </yu-button-drop>
...
yufp.lookup.reg('STD_ZB_CUR_TYP,STD_ZB_APPR_STATUS,STD_ZB_GUAR_WAY,STD_PRD_TYPE_PROP');
export default{
  name: 'D12BillList',
  mixins: [mixinList],
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: this.$backend.cmisBiz + '/api/iqploanapp/selectIqpLoanAppHisListData',
  ...
  methods: {
    ...
    datacodeFilterFn: function (opts, datacode, name) {
      ...
      for (let op = 0; op < opts.length; op++) {
        let opt = opts[op];
        if (opt.key === '000' || opt.key === '992' || opt.key === '111') {
          result[count] = opts[op];
          count++;
        }
      }
      return result;
