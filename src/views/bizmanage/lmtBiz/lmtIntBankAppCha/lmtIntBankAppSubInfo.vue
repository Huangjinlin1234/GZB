<template>
  <div>
    <yu-panel title="授信分项信息" panel-type="simple">
      ...
      <yu-xtable ref="refTable" condition-key="condition" row-number selection-type="radio" :data-url="dataUrl" :base-params="Param" request-type="POST" style="width: 100%;margin-top:10px;">
        <yu-xtable-column prop="subSerno" label="授信分项流水号" width="250"></yu-xtable-column>
        <yu-xtable-column prop="lmtBizTypeName" label="授信分项额度名称"></yu-xtable-column>
        <yu-xtable-column prop="lmtAmt" label="授信额度（万元）" width="150">
          <template slot-scope="scope">
            <span>{{ numFn(scope.row.lmtAmt) }}</span>
          </template>
        </yu-xtable-column>
        <yu-xtable-column prop="curType" label="币种" width="150" data-code="STD_ZB_CUR_TYP"> </yu-xtable-column>
        <yu-xtable-column prop="isRevolv" label="是否循环" width="100" data-code="STD_ZB_YES_NO"> </yu-xtable-column>
        <yu-xtable-column prop="term" label="期限" width="150"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
    <!-- 新增弹层 -->
    <yu-xdialog :title="'适用授信品种' + viewTitle[viewType]" :visible.sync="dialogVisible" width="800px" height="400px">
      <yu-xform label-width="120px" ref="refForm" :disabled="formDisabled" :form-type="formType" v-model="formdata">
        ...
        <yu-xform-group :column="2">
          <yu-xform-item label="授信品种编号" ctype="yu-xlmtsub-tree" rules="required" name="lmtBizType" :disabled="editDisabled" width="480" height="480" :parms="{limitStrNo:'03'}" @select-fn="commonSelectFn"
            :show-text="false" :mapping="{'id':'lmtBizType','label':'lmtBizTypeName'}"></yu-xform-item>
          ...
          <yu-xform-item label="授信额度(万元)" ctype="yu-num" placeholder="授信额度(万元)" number-formatter="0,000" name="lmtAmt" @change="changeLmtAmt" :rules="rule[0]"></yu-xform-item>
          <yu-xform-item label="额度期限(月)" ctype="input" placeholder="额度期限(月)" name="term" @change="changeTerm" :rules="rule[0]"></yu-xform-item>
          ...
          <yu-xform-item :hidden="showStatus" label="货币基金总授信额度(万元)" ctype="yu-num" name="lmtMfAmt" :rules="rule[0]" number-formatter="0,000" placeholder="货币基金总授信额度"></yu-xform-item>
          <yu-xform-item :hidden="showStatus" label="单只货币基金授信额度(万元)" ctype="yu-num" name="lmtSingleMfAmt" :rules="rule[0]" number-formatter="0,000" placeholder="单只货币基金授信额度"></yu-xform-item>
        </yu-xform-group>
...
import {numFn, numDe, numDM} from "@/utils/unitchange";
...
export default {
  ...
  created() {
    console.log("===============11111", this.children);
    if (this.children) {
  ...
  methods: {
    ...
    getLmtAmtTerm() {
      ...
      _this
        ...
        .then((data) => {
          let obj = data.data[0];
          this.lmtTerm = {
            origiLmtAmt: obj.origiLmtAmt,
            lmtAmt: obj.lmtAmt,
            origiLmtTerm: obj.origiLmtTerm,
            lmtTerm: obj.lmtTerm,
          };
        });
      // });
    ...
    changeLmtAmt(val) {
      let num = parseFloat(numDe(val) * 1 * 10000).toFixed();
      num = parseInt(num);
      let lmtTerm = this.lmtTerm;
      let origiLmtAmt = lmtTerm.origiLmtAmt;
      let lmtAmt = lmtTerm.lmtAmt;
      let total;
      let save = parseInt(lmtAmt) - parseInt(this.lmtAmt);
      if (this.editDisabled) {
        //修改
        total = save + num;
      } else {
        //新增
        total = parseInt(lmtAmt) + num;
      }
      if (total > origiLmtAmt) {
        this.$message({
          message: "分项授信额度总和不能超过原授信额度",
          type: "warning",
        });
        this.formdata.lmtAmt = 0;
      }
    },
    changeTerm(val) {
      let lmtTerm = this.lmtTerm;
      let origiLmtTerm = lmtTerm.origiLmtTerm;
      if (this.editDisabled) {
        //修改
        let term = this.origiLmtAccSubTerm; // 自身的期限
        if (val > term) {
          this.$message({
            message: "额度期限不能超过自身额度期限",
            type: "warning",
          });
          this.formdata.term = 0;
        }
      } else {
        if (val > origiLmtTerm) {
          //新增
          this.$message({
            message: "额度期限不能超过原授信额度期限",
            type: "warning",
          });
          this.formdata.term = 0;
        }
    ...
    addFn: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdata.subSerno = this.$xutils.getSEQWithParamFromServer(
          "LMT_SUB_SERNO"
        );
    ...
    modifyFn: function () {
      ...
      _this.$nextTick(function () {
        ...
        this.origiLmtAccSubTerm = obj.origiLmtAccSubTerm; //修改时 与自身期限作比较
        this.lmtAmt = obj.lmtAmt; //修改时 与自身额度作比较
        obj = numDM(obj, "D");
