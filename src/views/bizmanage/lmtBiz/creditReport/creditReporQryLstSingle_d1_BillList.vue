      ...
      <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl" :base-params="{'qryCls':'0'}" :default-load="true">
        ...
        <yu-xtable-column label="操作" prop="operate" width="100">
          <template slot-scope="scope">
            <yu-button @click="clickDataRow(scope.row)" type="text" size="small">发起申请</yu-button>
          </template>
        </yu-xtable-column>
      </yu-xtable>
    </yu-panel>
...
import dialogBillcard from './creditReporQryLstSingle_dialog_BillCard'
export default{
  name: "d1_BillList",
  ...
  methods: {
    clickDataRow(rowJsonData) {
      debugger
      this.$dialog.open('发起申请', '', -1, -1, rowJsonData);
    }
  }
}
</script>
