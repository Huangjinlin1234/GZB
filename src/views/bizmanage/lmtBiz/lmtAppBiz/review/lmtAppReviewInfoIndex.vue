<template>
  <div>
    ...
    <d1-13-billcard ref="d1_13_BillCard" v-show="isCardShow2"></d1-13-billcard>
  </div>
</template>
<script>
import d1Billcard from './lmtAppReviewInfo_d1_BillCard.vue';
import d113Billcard from './lmtAppReviewInfo_d1_13_BillCard.vue';
/**
* title 单一客户复审基本信息
* author 朱滋润
* date 2021-04-02
  ...
  methods: {
    ...
    AfterInit () {
      ...
      if (this.getFactory().contextData.op == 'VIEW') {
        ...
        this.d1_BillCard = this.$refs.d1_13_BillCard; // 创建卡片
    ...
    calculate () {
      this.$dialog.open(
        ...
        null,
        data => {
          this.d1_BillCard.setBillCardItemValue('eval_high_curfund_lmt_amt', data, data);
        }
      );
    },
    ...
    viewOriginAcc () {
      const originSerno = this.d1_BillCard.getBillCardItemValue('origi_lmt_reply_serno');
