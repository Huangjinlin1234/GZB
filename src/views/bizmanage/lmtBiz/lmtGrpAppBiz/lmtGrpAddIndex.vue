...
<script>
import d1Billcard from './lmtGrpAdd_d1_BillCard.vue';
export default {
  ...
  methods: {
    ...
    // 获取授信类型
    getLmtTypeByCusId (cusId) {
      this.$xutils.request({
        // 同步请求
        async: false,

        url: this.$backend.cmisBiz + '/api/lmtgrpapp/getlmttypebycusid/' + cusId,
    ...
    sendSavePost (params, a) {
      ...
      this.$xutils.showMsgBox('提示', '新增成功', null, null, () => {
        this.$dialog.close(this.dialogId);
        this.$dialog.open('', 'bizmanage/lmtBiz/lmtGrpAppBiz/lmtGroupAppIndex', -1, -1, params, null, true, true);
      });
    }
  }
};
