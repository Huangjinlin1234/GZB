<template>
  <yu-tabs type="card">
    <yu-tab-pane label="同业客户授信申报">
      <yu-panel title="同业授信申请表" panel-type="simple">
        ...
        <yu-xtable ref="refInfoTable" style="margin-top:5px" row-number condition-key="condition" selection-type="radio"
                   @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dataInfoUrl"
    ...
    <yu-tab-pane label="同业客户授信历史">
      <yu-panel title="同业客户授信历史" panel-type="simple">
        ...
        <yu-button-drop>
          ...
          <yu-button ref="btn_doReco" type="primary" @click="onReconcide" v-if="checkCtrl('repeat')">发起复议</yu-button>
        </yu-button-drop>
        <yu-xtable ref="refBackTable" style="margin-top:5px" row-number condition-key="condition" selection-type="radio"
                   :pageable="true" :data-url="dataBackUrl"
    ...
    <!-- 新增弹层 -->
      <yu-xdialog title="额度申请向导" :visible.sync="dialogVisible" width="800px" height="450px">
        <yu-xform label-width="120px" ref="addForm" :form-type="formType" v-model="formdata">
          <yu-xform-group :column="1">
            <yu-xform-item label="客户名称" ctype="input" name="cusName" placeholder="客户名称" rules="required" @click="goCustomer" @click.stop icon="search" disabled>
            </yu-xform-item>
          </yu-xform-group>
          <yu-xform-group :column="2">
            ...
            <yu-xform-item label="实际控制人" ctype="input" placeholder="实际控制人" name="acuCtrlorCusId" disabled></yu-xform-item>
...
export default {
  ...
  data () {
    return {
      ...
      infoParam: { condition: { oprType: '01', approveStatusList: '000,111,990,991,992,993', lmtTypeList: '01,03,05' } },
      backParam: { condition: { oprType: '01', approveStatusList: '996,997,998', lmtTypeList: '01,03,05' } },
  ...
  mounted: function(){
    // 开启监听表格监听事件
    yufp.globalEventBus.$on('refreshLmtIntbankTable', this.refreshTable);
  },
  destroyed: function () {
    yufp.globalEventBus.$off('refreshLmtIntbankTable');
  },
  methods: {
    /**
     * 参照公共的确认事件
      ...
      yufp.service.request({
        ...
        callback (code, message, response) {
          if (code == '0') {
            ...
            _this.addTab(response.data.map, 'Info', 'EDIT', '修改额度申报详情');
          } else {
    ...
    doUpdate () {
      ...
      if (jsoPar[0].lmtType == '05') {
        this.addTab(jsoPar[0], 'Back', 'EDIT', '修改额度申报详情');
      } else {
    ...
    onReconcide: function () {
      var _this = this;
      var selections = _this.$refs.refBackTable.selections;
      ...
      _this.$confirm('确认发起复议', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/lmtintbankapp/onreconside',
              data: selections[0],
              callback: function (code, message, response) {
                if (response.code == '0') {
                  ...
                  let bean = response.data.lmtIntbankApp;
                  _this.addTab(bean, 'Back', 'EDIT', '修改额度申报详情');
                }
              }
            });
          }
    ...
    goCustomer () {
      ...
      _this.$dialog.open(
        ...
        (retVal) => {
          ...
          yufp.service.request({
            method: 'POST',
            url: backend.cmisBiz + '/api/lmtintbankapp/valilmttype',
            data: {condition: JSON.stringify({cusId: _this.formdata.cusId, oprType: '01'})},
            callback: function (code, message, response) {
              if (response.code == '0') {
                if (response.data.lmtType == '03') {
                  _this.formdata.lmtType = '03';
                }
              }
            }
          });
          ...
          _this.formdata.intbankOrgType = retVal.intbankType;
