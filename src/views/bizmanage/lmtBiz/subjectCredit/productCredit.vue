<template>
  <div>
    ...
    <yu-tabs type="card">
      <yu-tab-pane label="产品授信申报">
        <yu-panel title="产品授信申请列表" panel-type="simple">
          <yu-xform v-model="searchFormdata" related-table-name="refTable" form-type="search" :remove-empty="true" label-width="120px">
            <yu-xform-group :column="3">
              <yu-xform-item label="客户名称" ctype="input" placeholder="客户名称" name="cusName" maxlength="80"></yu-xform-item>
              <yu-xform-item label="客户编号" ctype="input" placeholder="客户编号" name="cusId" maxlength="20"></yu-xform-item>
              <yu-xform-item label="产品类型" ctype="select" placeholder="产品类型" name="lmtBizType" data-code="STD_ZB_PRD_BIZ_TYPE" :datacode-filter="datacodeFilterFn1" exclude-key="4009,16010101,16020101,4011,4012"></yu-xform-item>
          ...
          <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" :pageable="true" :data-url="dataUrl1" :base-params="Param1" requestType="POST" :default-load="true">
            ...
            <yu-xtable-column label="利率" width="100">
              <template slot-scope="scope">
      ...
      <yu-tab-pane label="产品授信申报历史">
        <yu-panel title="产品授信申请历史列表" panel-type="simple">
    ...
    <yu-xdialog title="额度申请向导" :visible.sync="show_dialog" width="800px" height="400px">
      <yu-xform label-width="120px" ref="addForm" :form-type="formType" v-model="formdata">
        ...
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="业务类型" ctype="select" placeholder="业务类型" name="lmtBizType" :options="lmtTypeOptions" rules="required" :disabled="disabledType" @change="changeLmtTypeOptions"></yu-xform-item>
...
yufp.lookup.reg(
  'STD_ZB_PRD_BIZ_TYPE,STD_SX_LMT_TYPE,STD_ZB_APPR_STATUS,STD_ZB_APPR_STATUS'
);
...
export default {
  ...
  data () {
    return {
      ...
      dataUrl1: this.$backend.cmisBiz + '/api/lmtsiginvestapp/selectByModelProduct',
      Param1: {
      ...
      dataUrl2: this.$backend.cmisBiz + '/api/lmtsiginvestapp/selectByModelProductHis',
      Param2: {
        condition: JSON.stringify({
          listtype: '02',
          oprType: '01',
  ...
  methods: {
    ...
    datacodeFilterFn1: function (opts, datacode, name) {
      return opts.filter(function (op) {
        // 4009,16010101,16020101,4011,4012
        if ('4009,16010101,16020101,4011,4012,'.indexOf(op.key + ',') != -1) {
          return false;
        }
        return true;
      });
    ...
    goCustomer () {
      ...
      _this.$dialog.open(
        ...
        (retVal) => {
          ...
          _this.lmtTypeExcludeCode = '4009,16010101,16020101,4011,4012';
          // }
          let relData = [];
