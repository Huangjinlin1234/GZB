  ...
  <yu-xform ref="refForm" label-width="200px" v-model="formdata" related-table-name="refTable" :rules="formRules">
    <yu-panel title="担保人/增信人信息" panel-type="simple">
      <yu-button-drop style="margin-bottom:10px;" v-show="saveBtnShow">
        <yu-button ref="btn_doAdd" type="primary" @click="doAdd_01">新增</yu-button>
        <yu-button ref="btn_doDelete" type="primary" @click="doDelete_01">删除</yu-button>
      </yu-button-drop>
      <yu-xtable ref="refTable_01" condition-key="condition" selection-type="radio" row-number :data-url="dataUrl" :base-params="Param_01" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
        <yu-xtable-column label="客户编号" prop="grtCusId" width="180"></yu-xtable-column>
        ...
        <yu-xtable-column label="担保金额（万元）" prop="guarAmt" :formatter="Currency" width="140">
          <template slot-scope="scope">
            <span>{{ numFn(scope.row.guarAmt) }}</span>
          </template>
        </yu-xtable-column>
    ...
    <yu-panel title="抵质押品信息" panel-type="simple">
      <yu-button-drop style="margin-bottom:10px;" v-show="saveBtnShow">
        <yu-button ref="btn_doAdd" type="primary" @click="doAdd_02">新增</yu-button>
        <yu-button ref="btn_doDelete" type="primary" @click="doDelete_02">删除</yu-button>
      </yu-button-drop>
      <yu-xtable ref="refTable_02" condition-key="condition" selection-type="radio" row-number :data-url="dataUrl" :base-params="Param_02" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
        ...
        <yu-xtable-column label="担保类型" prop="guarType" width="120" data-code="STD_ZB_GUAR_WAY"></yu-xtable-column>
        <yu-xtable-column label="抵质押物类型" prop="pldimnType" data-code="STD_GUIDE_TYPE" width="120"></yu-xtable-column>
    ...
    <yu-xdialog title="担保人/增信人信息" :visible.sync="show_dialog_01" width="800px" height="400px">
      <yu-xform label-width="120px" ref="addForm_01" :form-type="formType" v-model="formdata_01" :rules="rules">
        ...
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="担保金额（万元）" ctype="yu-num" number-formatter="0,000" autofocus name="guarAmt" placeholder="担保金额" rules="required"></yu-xform-item>
    ...
    <yu-xdialog title="抵质押品信息" :visible.sync="show_dialog_02" width="800px" height="400px">
      <yu-xform label-width="120px" ref="addForm_02" :form-type="formType" v-model="formdata_02" :rules="rules">
        ...
        <yu-xform-group :column="2">
          ...
          <yu-xform-item label="担保类型" ctype="select" placeholder="担保类型" name="guarType" data-code="STD_ZB_GUAR_WAY" :datacode-filter="guarTypeDatacodeFilterFn" rules="required"></yu-xform-item>
          <yu-xform-item label="抵质押物类型" ctype="select" placeholder="抵质押物类型" name="pldimnType" data-code="STD_GUIDE_TYPE" rules="required"></yu-xform-item>
...
yufp.lookup.reg('STD_ZB_YES_NO,STD_ZB_GUAR_WAY,STD_GUIDE_TYPE');
export default {
  props: {
    children: Object
  },
  ...
  created () {
    yufp.lookup.bind('STD_ZB_GUAR_WAY', function (data) {
  ...
  methods: {
    guarTypeDatacodeFilterFn: function (opts, datacode, name) {
      return opts.filter(function (op) {
        if (op.key === '10' || op.key === '20') {
          return true;
        }
        return false;
      });
    ...
    isValidGuarChange_01: function (val) {
      if (this.formdata_01.isValidGuar == '1') {
        ...
        var message =
          '（一）我国中央政府、中央人民银行、政策性银行、公共部门实体和商业银行；' +
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/lmtapprelguar/deleteLogicByPkId',
              data: {
    ...
    optSave_01: function () {
      ...
      model.guarAmt = parseFloat(model.guarAmt * 10000).toFixed();
      model.guarType = '30';
