<template>
  <yu-xform ref="refForm" label-width="200px" v-model="formdata" related-table-name="refTable" :rules="rules">
    <yu-panel title="基本信息" panel-type="simple">
      ...
      <yu-xform-group :column="2">
        ...
        <yu-xform-item label="投资类型" placeholder="投资类型" name="nstdInvestType" ctype="select" data-code="STD_ZB_NSTD_INVEST_TYPE" :disabled="!saveBtnShow"></yu-xform-item>
      ...
      <yu-xform-group :column="1">
        <yu-xform-item label="募集资金用途" placeholder="募集资金用途" name="raiseAmtPurp" ctype="input" maxlength="2000" :disabled="!saveBtnShow"></yu-xform-item>
        ...
        <yu-xform-item label="项目介绍/交易结构" placeholder="交易结构" name="tranStr" ctype="textarea" maxlength="4000" colspan="15" :autosize="{ minRows: 10}" :disabled="!saveBtnShow"></yu-xform-item>
      </yu-xform-group>
    </yu-panel>
    ...
    <yu-panel title="授信方案" panel-type="simple">
      <yu-xform-group :column="2">
        <yu-xform-item label="授信金额（万元）" placeholder="授信金额" name="lmtAmt" ctype="yu-num" number-formatter="0,000"  @blur="getLargeLmtInfo"  :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="审批权限" placeholder="审批权限" name="apprMode" ctype="select" data-code="STD_APPR_MODE" disabled></yu-xform-item>
        <yu-xform-item label="利率（%）" placeholder="利率（%）" name="rate" ctype="yu-num" sign="%" :multiple="100" @blur="getLargeLmtInfo" :disabled="!saveBtnShow"></yu-xform-item>
        ...
        <yu-xform-item label="承销方式" placeholder="承销方式" name="underwritingMode" ctype="select" @change="onUnderwritingModeChange" data-code="STD_ZB_UNDERWRITING_MODE" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="我行承销金额（万元）" placeholder="我行承销金额（万元）" name="bankUnderwritingAmt" ctype="yu-num" number-formatter="0,000" :disabled="optDisabled1"></yu-xform-item>
      </yu-xform-group>
    </yu-panel>
...
yufp.lookup.reg(
  'STD_SX_LMT_TYPE,STD_ZB_PRD_BIZ_TYPE,STD_ZX_CUR_TYPE,STD_ZB_DEBT_INNER_LEVEL,STD_ZB_TRANS_MARKET,STD_ZB_YES_NO,' +
    'STD_ZB_CUR_TYP,STD_ZB_DEBT_EVAL_RESULT,STD_ZB_LARGE_MAIN_MANAGER_TYPE,STD_ZB_BASIC_ASSET_NORMAL_TYPE,' +
  'STD_ZB_NORM_INVEST_TYPE,STD_ZB_NSTD_INVEST_TYPE,STD_ZB_UNDERWRITING_MODE,STD_DEBT_EVAL_ORG,STD_APPR_MODE'
);
...
export default {
  ...
  methods: {
    ...
    //我行承销金额：若承销方式选择了【他行承销】，则我行承销金额默认为0且不可更改
    onUnderwritingModeChange: function (val) {
      if (val == 20 && this.children.op == 'EDIT') {
        this.formdata.bankUnderwritingAmt = 0;
        this.optDisabled1 = true;
      } else {
        this.optDisabled1 = false;
      }
    },
