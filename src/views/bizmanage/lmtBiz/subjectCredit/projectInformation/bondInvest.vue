  ...
  <yu-xform ref="refForm" label-width="200px" v-model="formdata" related-table-name="refTable" :rules="rules">
    <yu-panel title="基本信息" panel-type="simple">
      ...
      <yu-xform-group :column="2">
        ...
        <yu-xform-item label="是否有担保/增信" placeholder="是否有担保/增信" name="isExistGuarCreditect" ctype="select" data-code="STD_ZB_YES_NO" @change="onExistGuarCreditectChange" rules="required" :disabled="!saveBtnShow">
        </yu-xform-item>
        ...
        <!-- 债券分类（投资类型） -->
        <yu-xform-item label="债券分类" placeholder="债券分类" name="investType" ctype="select" data-code="STD_ZB_NORM_INVEST_TYPE" :datacode-filter="datacodeFilterFn" :disabled="!saveBtnShow"></yu-xform-item>
      ...
      <yu-xform-group :column="1">
        ...
        <yu-xform-item label="项目介绍/交易结构" placeholder="交易结构" name="tranStr" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" :disabled="!saveBtnShow"></yu-xform-item>
      </yu-xform-group>
    </yu-panel>
...
yufp.lookup.reg(
  "STD_SX_LMT_TYPE,STD_ZB_PRD_BIZ_TYPE,STD_ZX_CUR_TYPE,STD_ZB_DEBT_INNER_LEVEL,STD_ZB_TRANS_MARKET,STD_ZB_YES_NO," +
    "STD_ZB_CUR_TYP_PAR,STD_ZB_CUR_TYP,STD_ZB_DEBT_EVAL_RESULT,STD_ZB_LARGE_MAIN_MANAGER_TYPE,STD_ZB_BASIC_ASSET_NORMAL_TYPE,STD_ZB_NORM_INVEST_TYPE,STD_DEBT_EVAL_ORG"
);
...
import {numDM, projectInformation, getFile, upImage, DeleteImage} from "@/utils/unitchange";
export default {
  ...
  methods: {
    // 债项评级  评级机构和债项评级日期：若债项评级选择无，则此字段置空非必输
