<template>
  <div>
    <yu-panel title="底层资产清单(大额风险暴露）" panel-type="simple">
      ...
      <yu-panel title="单笔投资授信批复查询列表" panel-type="simple">
      <yu-button-drop :show-length="8">
        ...
        <yufp-excel-export  v-if="checkCtrl('download')" :export-url="exportTemplateUrl" title="模板下载"></yufp-excel-export>
        ...
        <yufp-excel-export  v-if="checkCtrl('export')" :export-url="excelExportUrl" title="导出" :export-param="searchFormdata"></yufp-excel-export>
      </yu-button-drop>
...
export default{
  ...
  data: function () {
    return {
      dataUrl: this.$backend.cmisBiz + "/api/siginvestdetailsinfo/selectByModelForInfoDetail",
  ...
  created() {
    ...
    this.Param.condition = JSON.stringify({ oprType: '01', status: '01', proNo: this.children.proNo })
  },
  methods: {
