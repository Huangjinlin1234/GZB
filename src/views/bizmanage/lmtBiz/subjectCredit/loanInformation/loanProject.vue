  ...
  <yu-xform ref="refForm" label-width="200px" v-model="formdata" related-table-name="refTable">
    <yu-panel title="基本信息" panel-type="simple">
      <yu-xform-group :column="2">
        <yu-xform-item label="入池基础资产标准" placeholder="入池基础资产标准" name="poolBasicAssetNormal" maxlength="60" ctype="input" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="初始起算日" placeholder="初始起算日" name="firstStartDate" ctype="datepicker" :disabled="!saveBtnShow"></yu-xform-item>
      </yu-xform-group>
      <yu-xform-group :column="2">
        <yu-xform-item label="基础资产类型" placeholder="基础资产类型" name="basicAssetType" ctype="select" data-code="STD_ZB_BOTT_TYPE" disabled :rules="rule[0]"></yu-xform-item>
      </yu-xform-group>
      <yu-xform-group :column="2">
        <yu-xform-item label="贷款笔数" placeholder="贷款笔数" name="loanQnt" ctype="input" maxlength="40" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="借款人数量" placeholder="借款人数量" name="debitQnt" ctype="input" maxlength="200" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="资产池总本金余额（万元）" placeholder="资产池总本金余额" name="poolTotalAmt" ctype="yu-num" number-formatter="0,000" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="资产池合同金额（万元）" placeholder="资产池合同金额" name="poolContAmt" precision="0" ctype="yu-num" number-formatter="0,000" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="单个借款人平均本金余额（万元）" placeholder="单个借款人平均本金余额" name="sigDebitAvgAmt" ctype="yu-num" number-formatter="0,000" precision="0" :disabled="!saveBtnShow" ></yu-xform-item>
      </yu-xform-group>
      <yu-xform-group :column="2">
        <yu-xform-item label="加权平均利率" placeholder="加权平均利率" name="loanWeightAvgRate" ctype="yu-num" sign="%" :multiple="100"
          :disabled="!saveBtnShow" ></yu-xform-item>
        <yu-xform-item label="最晚一笔到期贷款期限" placeholder="最晚一笔到期贷款期限" name="lastSigLoanEndTerm" ctype="datepicker" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="前3大借款人未偿还本金余额占比" placeholder="未偿还本金余额占比" name="lastThrUnrepayCapPerc" ctype="yu-num" sign="%" :multiple="100" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="前5大借款人未偿还本金余额占比" placeholder="未偿还本金余额占比" name="lastFifUnrepayCapPerc" ctype="yu-num" sign="%" :multiple="100" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="是否穿透至底层资产" placeholder="是否穿透至底层资产" name="isPassBasicAsset" :rules="rule[0]" @change="isPassBasicAssetChange" ctype="select" data-code="STD_ZB_YES_NO" :disabled="!saveBtnShow"></yu-xform-item>
    ...
    <yu-panel title="产品各级别投资情况" panel-type="simple">
      ...
      <yu-panel title="B.不同单户余额投资余额分布" panel-type="simple">
        ...
        <yu-xtable ref="refBTable" condition-key="condition" show-summary :summary-method="getSummaries" selection-type="radio" row-number :data-url="dataBUrl" :base-params="ParamB" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
      ...
      <yu-panel title="C.入池投资利率分布" panel-type="simple">
        ...
        <yu-xtable ref="refCTable" condition-key="condition" show-summary :summary-method="getSummaries" selection-type="radio" row-number :data-url="dataCUrl" :base-params="ParamC" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
      ...
      <yu-panel title="D.入池投资剩余期限分布" panel-type="simple">
        ...
        <yu-xtable ref="refDTable" condition-key="condition" show-summary :summary-method="getSummaries" selection-type="radio" row-number :data-url="dataDUrl" :base-params="ParamD" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
      ...
      <yu-panel title="E.入池借款人行业分布" panel-type="simple">
        ...
        <yu-xtable ref="refETable" condition-key="condition" show-summary :summary-method="getSummaries" selection-type="radio" row-number :data-url="dataEUrl" :base-params="ParamE" requestType="POST" style="width: 100%; height: 100%; margin-top: 5px">
...
export default {
  ...
  created () {
    ...
    this.dataAUrl = backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/selectByModel';
    this.dataBUrl = backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/selectByModel';
    this.dataCUrl = backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/selectByModel';
    this.dataDUrl = backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/selectByModel';
    this.dataEUrl = backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/selectByModel';
    this.ParamA = { condition: JSON.stringify({oprType: '01', serno: serno, bottomAnalyClsValve: '10'}) };
    this.ParamB = { condition: JSON.stringify({oprType: '01', serno: serno, bottomAnalyClsValve: '20'}) };
    this.ParamC = { condition: JSON.stringify({oprType: '01', serno: serno, bottomAnalyClsValve: '30'}) };
    this.ParamD = { condition: JSON.stringify({oprType: '01', serno: serno, bottomAnalyClsValve: '40'}) };
    this.ParamE = { condition: JSON.stringify({oprType: '01', serno: serno, bottomAnalyClsValve: '50'}) };
  ...
  methods: {
    ...
    getDetails (serno, cusId) {
      ...
      _this
        ...
        .then((data) => {
          if (data.code == '0') {
            let obj = data.data;
            obj.poolTotalAmt = parseFloat(parseFloat(obj.poolTotalAmt / 10000).toFixed());
            obj.poolContAmt = parseFloat(parseFloat(obj.poolContAmt / 10000).toFixed());
            obj.sigDebitAvgAmt = parseFloat(parseFloat(obj.sigDebitAvgAmt / 10000).toFixed());
    ...
    addFnA: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdataA.pkId = this.$xutils.getSEQWithParamFromServer('PK_VALUE');
        _this.formdataA.inputId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
        _this.formdataA.inputDate = this.$xutils.getDefaultformulaData('$CURRDATE');
        _this.formdataA.inputBrId = this.$xutils.getDefaultformulaData('$LoginOrgCode');
        _this.formdataA.inputIdName = this.$xutils.getDefaultformulaData('$LoginUserName');
        _this.formdataA.inputBrIdName = this.$xutils.getDefaultformulaData('$LoginOrgName');
      });
    },
    /**
     * 删除
      ...
      _this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        callback: function (action) {
          if (action === 'confirm') {
            yufp.service.request({
              method: 'POST',
              url: backend.cmisBiz + '/api/lmtsiginvestbasiccptanly/logicalDelete',
    ...
    addFnB: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdataB.pkId = this.$xutils.getSEQWithParamFromServer('PK_VALUE');
        _this.formdataB.inputId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
        _this.formdataB.inputDate = this.$xutils.getDefaultformulaData('$CURRDATE');
        _this.formdataB.inputBrId = this.$xutils.getDefaultformulaData('$LoginOrgCode');
        _this.formdataB.inputIdName = this.$xutils.getDefaultformulaData('$LoginUserName');
        _this.formdataB.inputBrIdName = this.$xutils.getDefaultformulaData('$LoginOrgName');
      });
    },
    /**
     * 删除
    ...
    addFnC: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdataC.pkId = this.$xutils.getSEQWithParamFromServer('PK_VALUE');
        _this.formdataC.inputId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
        _this.formdataC.inputDate = this.$xutils.getDefaultformulaData('$CURRDATE');
        _this.formdataC.inputBrId = this.$xutils.getDefaultformulaData('$LoginOrgCode');
        _this.formdataC.inputIdName = this.$xutils.getDefaultformulaData('$LoginUserName');
        _this.formdataC.inputBrIdName = this.$xutils.getDefaultformulaData('$LoginOrgName');
      });
    },
    /**
     * 删除
    ...
    addFnD: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdataD.pkId = this.$xutils.getSEQWithParamFromServer('PK_VALUE');
        _this.formdataD.inputId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
        _this.formdataD.inputDate = this.$xutils.getDefaultformulaData('$CURRDATE');
        _this.formdataD.inputBrId = this.$xutils.getDefaultformulaData('$LoginOrgCode');
        _this.formdataD.inputIdName = this.$xutils.getDefaultformulaData('$LoginUserName');
        _this.formdataD.inputBrIdName = this.$xutils.getDefaultformulaData('$LoginOrgName');
      });
    },
    /**
     * 删除
    ...
    addFnE: function () {
      ...
      _this.$nextTick(function () {
        ...
        _this.formdataE.pkId = this.$xutils.getSEQWithParamFromServer('PK_VALUE');
        _this.formdataE.inputId = this.$xutils.getDefaultformulaData('$LoginLoginCode');
        _this.formdataE.inputDate = this.$xutils.getDefaultformulaData('$CURRDATE');
        _this.formdataE.inputBrId = this.$xutils.getDefaultformulaData('$LoginOrgCode');
        _this.formdataE.inputIdName = this.$xutils.getDefaultformulaData('$LoginUserName');
        _this.formdataE.inputBrIdName = this.$xutils.getDefaultformulaData('$LoginOrgName');
      });
    },
    /**
     * 删除
    ...
    saveFn: function () {
      ...
      model.updDate = this.$xutils.getDefaultformulaData('$CURRDATE');
      model.updateTime = this.$xutils.getDefaultformulaData('$CURRTIME');
      model.poolTotalAmt = parseFloat(parseFloat(model.poolTotalAmt * 10000).toFixed());
      model.poolContAmt = parseFloat(parseFloat(model.poolContAmt * 10000).toFixed());
      model.sigDebitAvgAmt = parseFloat(parseFloat(model.sigDebitAvgAmt * 10000).toFixed());
      model.basicAssetTotalAmt = parseFloat(parseFloat(model.basicAssetTotalAmt * 10000).toFixed());

      var updateurl =
        backend.cmisBiz + '/api/lmtsiginvestbasicinfo/updateZtqkfx';
      yufp.service.request({
        method: 'POST',
        url: updateurl,
        data: model,
