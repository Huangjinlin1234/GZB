<template>
  <yu-xform label-width="210px" v-model="formdata" :rules="rules" ref="refForm">
    <yu-panel title="复议内容" panel-type="simple">
      <yu-xform-group :column="1">
        <yu-xform-item label="上期申请授信情况及总行审批意见" placeholder="上期申请授信情况及总行审批意见" name="lastLmtCondition" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" maxlength="4000" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="本次申请复议内容" placeholder="本次申请复议内容" name="lmtRediContent" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" maxlength="4000" :disabled="!saveBtnShow"></yu-xform-item>
      </yu-xform-group>
    </yu-panel>
    <yu-panel title="复议理由" panel-type="simple">
      <yu-xform-group :column="1">
        <yu-xform-item label="进一步陈述坚持要求发放该笔融资的原因" placeholder="进一步陈述坚持要求发放该笔融资的原因" name="keepFinaReason" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" maxlength="4000" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="风险防范措施" placeholder="风险防范措施" name="riskGuardMeasu" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" maxlength="4000" :disabled="!saveBtnShow"></yu-xform-item>
        <yu-xform-item label="其他理由" placeholder="其他理由" name="otherResn" ctype="textarea" colspan="15" :autosize="{ minRows: 10}" maxlength="4000" :disabled="!saveBtnShow"></yu-xform-item>
      </yu-xform-group>
...
export default {
  ...
  mounted() {
    ...
    this.serno = this.children.serno;
  ...
  methods: {
    lookReconsider() {
      ...
      let key = "report";
      params.src = _this.$backend.frptRptService + "zjty-fysq30.cpt&op=view&lmtSerno=" +
        params.lmtSerno;
      _this.$router.addTab({
        // 路由名称
        name: name,
