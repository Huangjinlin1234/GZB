<template>
  <div>
    <yu-panel title="机构撤并管理" panel-type="simple">
      ...
      <yu-xtable ref="refTable" row-number condition-key="condition" request-type="post" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data="tableData" >
        ...
        <yu-xtable-column label="合并机构号" prop="repealOldOrg" data-code="STD_PARTNER_TYPE"></yu-xtable-column>
        <yu-xtable-column label="合并机构名称" prop="operPageName"  width="150"></yu-xtable-column>
        <yu-xtable-column label="撤并日期" prop="broadcastDate"></yu-xtable-column>
        <yu-xtable-column label="广播日期（申请日期）" prop="createTime"></yu-xtable-column>
        <yu-xtable-column label="处理状态" prop="createTime"></yu-xtable-column>
        <yu-xtable-column label="上一次处理状态" prop="createTime"></yu-xtable-column>
      </yu-xtable>
    </yu-panel>
  </div>
</template>
...
export default {
  ...
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: 'ADD',
      dataUrl: backend.console + '/api/s/org/repeal',
      baseParams: {},
