<template>
  <div class="ctrCvrgContList_d1_2_BillList">
    <yu-panel title="历史合同" :hideFilter="false" :collapseHide="false">
      <yu-xform ref="refForm" form-type="search" v-model="searchFormdata" label-width="120px" :custom-search-fn="customSearch">
        <yu-xform-group :column="3">
          <yu-xform-item label="合同编号" ctype="input" placeholder="合同编号" name="contNo"></yu-xform-item>
          <yu-xform-item label="合同类型" ctype="select" data-code="STD_ZB_CONT_TYPE" placeholder="合同类型" name="contType"></yu-xform-item>
      ...
      <yu-toolbar>
        <yu-button ref="btn_onPrintHis" @click="customClick('onPrintHis')" type="primary" v-if="checkCtrl('hisprint')">打印</yu-button>
        <yu-button ref="btn_onViewHis" @click="customClick('onViewHis')" type="primary" v-if="checkCtrl('hisview')">查看</yu-button>
        <yu-button ref="btn_onLogoutHis" @click="customClick('onLogoutHis')" type="primary" v-if="checkCtrl('hislogout')">注销</yu-button>
      </yu-toolbar>
      <yu-xtable ref="refTable" row-number condition-key="condition" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dataUrl2"
        :base-params="{'contStatus !':'100'}" :default-load="true" request-type="POST">
        <yu-xtable-column label="合同编号" prop="contNo"></yu-xtable-column>
        <yu-xtable-column label="合同类型" prop="contType" data-code="STD_ZB_CONT_TYPE"></yu-xtable-column>
...
export default {
  ...
  data: function () {
    return {
      ...
      dialogVisible: false,
      formType: "ADD",
      dataUrl2: this.$backend.cmisBiz + "/api/ctrcvrgcont/donesignlist",
      baseParams: {},
...
<style>
.ctrCvrgContList_d1_2_BillList .yu-buttons {
  margin-bottom: 10px;
}
</style>