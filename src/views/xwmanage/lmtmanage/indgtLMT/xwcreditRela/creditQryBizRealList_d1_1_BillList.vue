  ...
  <div>
    <yu-panel title="人行征信查询申请列表" :collapseHide="false">
      <template slot="right">
        <yu-button-drop style="padding: 0">
          <!-- <yu-button ref="btn_doInsect" v-if="!isGrp" @click="customClick('doInsect')">新增</yu-button> -->
          <yu-button type="primary" ref="btn_doImport" v-if="!isGrp" @click="doImport">引入</yu-button>
          <yu-button type="primary" ref="btn_doUpdate" v-if="!isGrp" @click="customClick('doUpdate')">修改</yu-button>
          <yu-button type="primary" ref="btn_doView" v-if="!isManagerId" @click="customClick('doView')">查看</yu-button>
          <yu-button type="primary" ref="btn_doDelete" v-if="!isGrp" @click="customClick('doDelete')">删除</yu-button>
    ...
    <div>
      <yu-xdialog title="客户选择" :visible.sync="dialogTableVisible" width="1500px" height="800px" center="true">
        ...
        <yu-xtable ref="dialogtable" row-number condition-key="condition" :baseParams="baseParamsCred" selection-type="radio" @row-click="onRowClick" @row-dblclick="onRowDBClick" @cell-click="onCellClick" :pageable="true" :data-url="dialogDataUrl" :default-load="false" request-type="POST">
...
export default{
  ...
  data: function () {
    return {
      ...
      // dialogDataUrl: backend.cmisBiz + '/api/creditreportqrylst/checkcredreport/',
  ...
  mounted () {
    ...
    let bizScene = 'CRE18'; // 授信调查
    if (this.$route.meta.params) { // 授信调查
      if (this.$route.meta.params.model_group_no === 'CMG000262') {
    ...
    setTimeout(() => {
      ...
      if (data.length == 0 && this.$route.meta.params.model_group_no !== 'CMG000153') { // 非集团授信场景
        let _this = this;
        let model = {};
  ...
  methods: {
    ...
    doImport () {
      // var _this = this;
      // var params = this.$refs.refTable.selections;
      // if (params.length === 0) {
      //   this.$xutils.showMsgBox('提示', '必须选择一条记录进行操作！');
      //   return;
      // }
      // _this.dialogTableVisible = true;
      // var selectArr = params[0];
      // yufp.clone(selectArr, _this.selectData);

      // var param = {
      //   condition: JSON.stringify(selectArr)
      // };
      // this.$nextTick(function () {
      //   this.$refs.dialogtable.remoteData(param);
      // });
    ...
    selectReturn () {
      ...
      selectionArr[0].scene = '01'; // 01:授信管理 02:合同签订 03:出账放款 04:贷后检查
      yufp.service.request({
        method: 'POST',
