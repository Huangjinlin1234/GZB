...
<script>
import d1Billcard from './noRepayGreatBaseInfo_d1_BillCard.vue';
...
export default {
  ...
  methods: {
    ...
    // 【保存】按钮的逻辑
    save () {
      const saveFlag = this.jbxx.validateBillCardValue();
      if (!saveFlag) {
        this.$message({ message: '尚有必输项未填入', type: 'success' });
        return;
      }
      this.jbxx.formdata.activityFlag = this.jbxx.comdata.activityFlag;
      this.jbxx.formdata.managementBelongFlag = this.jbxx.comdata.managementBelongFlag;
      this.jbxx.formdata.debtFlag = this.jbxx.comdata.debtFlag;
      this.jbxx.formdata.guarFlag = this.jbxx.comdata.guarFlag;
      this.jbxx.formdata.accidentFlag = this.jbxx.comdata.accidentFlag;
      this.jbxx.formdata.guaranteeFlag = this.jbxx.comdata.guaranteeFlag;
      const data = {
        surveyReportBasicInfo: this.jbxx.formdata,
        surveyReportComInfo: this.jbxx.facfomdata
      };
      const rsPars = {};
      this.$xutils.request({
        // 同步请求
    ...
    creat () {
      ...
      _this.$request({
        ...
        if (response.code == '0') {
          ...
          _this.jbxx.formdata.spouseCusId = response.data.cusId;
          _this.jbxx.formdata.spouseCertCode = response.data.certCode;
          _this.jbxx.formdata.spouseName = response.data.cusName;
