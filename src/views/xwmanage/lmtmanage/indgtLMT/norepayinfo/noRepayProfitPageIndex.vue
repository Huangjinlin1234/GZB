...
<script>
import d1Billlist from './noRepayProfitPage_d1_BillList.vue';
...
export default {
  ...
  methods: {
    /* 无还本续贷损益表*/
    afterInit () {
      this.list = this.$refs.d1_BillList;
    },

    /* 保存按钮*/
    doNextStep () {
      let d1_BillCard = this.$refs.list.$refs.dialog_BillCard;
      ...
      this.$xutils.showMsgBox('提示', '保存成功', 500, 200, () => {}); // 弹出提示
      // YuXP.closeDialog();
    },
    ...
    doselectCob () {
      let d1_BillCard = this.$refs.list.$refs.dialog_BillCard;
      // 校验
      if (d1_BillCard.getBillCardItemValue('cob_name') == null ||
      ...
      this.$dialog.open(
        ...
        params => {
          ...
          // d1_B_A_BillList.queryDataByCondition("survey_serno='"+jsoOpenPars.survey_serno+"'")
        }
      );
