  ...
  methods: {
    afterInit () {
      ...
      yufp.service.request({
        ...
        data: {
          // bizId: 'XD202105260000036'
          bizId: this.getPvpSerno()
    ...
    getPvpSerno () {
      ...
      if (this.bizPageData != null) {
        ...
        // 单页面 跳审批的时候 是这个
        // 审批走单一页面
        pvpSerno = this.bizPageData.instanceInfo.bizId;
      } else if (this.$route.params.hasOwnProperty('pvpSerno')) {
        // 走路由
        pvpSerno = this.$route.meta.params.pvpSerno;
      } else {
        // 走审批模版工厂
        pvpSerno = this.getFactory().bizPageData.instanceInfo.bizId;
      }
      //try {
      // 走路由
        //pvpSerno = this.$route.meta.params.pvpSerno;
