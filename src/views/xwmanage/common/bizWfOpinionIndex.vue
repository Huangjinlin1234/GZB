...
export default {
  ...
  mounted () {
    // 从模板工厂进来通用  this.getFactory().bizPageData.instanceInfo.bizId;
    // this.surveySerno = this.$route.meta.params.surveySerno || this.getFactory().contextData.pvpSerno || this.getFactory().contextData.bizSerno || this.getFactory().contextData.iqpSerno || this.getFactory().bizPageData.instanceInfo.bizId;
    try {
      // 正常页面进入 审批
      // 走新增tab页签进来的
      this.surveySerno = this.$route.params.hasOwnProperty('surveySerno') ? this.$route.meta.params.surveySerno : this.getFactory().bizPageData.instanceInfo.bizId ||
       this.getFactory().contextData.pvpSerno || this.getFactory().contextData.bizSerno || this.getFactory().contextData.iqpSerno;
    } catch (e) {
      // 走审批模版工厂
