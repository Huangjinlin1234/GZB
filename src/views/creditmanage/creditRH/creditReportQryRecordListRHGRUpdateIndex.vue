...
<script>
import d1Billcard from './creditReportQryRecordListRHGRUpdate_d1_BillCard.vue';
...
export default {
  ...
  methods: {
    afterInit () {
      ...
      // this.d1BillCard.queryDataByCondition({'crqlSerno' : this.getFactory().contextData.crqlSerno});
    ...
    onSave () {
      ...
      if (this.d1BillCard.getItemValue('certType') === 'D') { // 港澳通行证校验
        if (!isRealHM(this.d1BillCard.getItemValue('certCode'))) {
