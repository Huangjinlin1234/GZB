    ...
    <div id="d1_BillCardContent">
      <yu-xform ref="refForm" label-width="150px" :form-type="formType" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent">
        <yu-xform-group :column="2">
          <yu-xform-item label="征信查询类别" ctype="select" rules="required" name="qryCls" :options="dicOptions.qryClsOptions" placeholder="征信查询类别" @change="ChangeQryCls"></yu-xform-item>
          <yu-xform-item label="与主借款人关系" ctype="select" rules="required" name="borrowRel" :options="dicOptions.borrowRelOptions" placeholder="与主借款人关系" @change="ChangeBorrowRel"></yu-xform-item>
          ...
          <yu-xform-item label="征信查询对象名称" ctype="input" rules="required" placeholder="征信查询对象名称" name="cusName" disabled></yu-xform-item>
          <yu-xform-item label="查询对象证件类型" ctype="select" rules="required" name="certType" :options="dicOptions.certTypeOptions" placeholder="查询对象证件类型" @change="ChangeCertType"></yu-xform-item>
...
import { validator, isRealHM, isOrgCode } from '@/utils/validate';
export default{
  name: 'D1BillCard',
  mixins: [mixinForm],
  props: {
