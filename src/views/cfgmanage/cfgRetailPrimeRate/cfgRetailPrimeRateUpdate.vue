    ...
    <div>
        ...
        <yu-xform ref="refForm" v-model="formdata">
            <yu-xform-group :column="2">
                <yu-xform-item label="产品编号" ctype="input" name="prdId" rules="required" placeholder="产品编号" disabled></yu-xform-item>
        ...
        <yu-xdialog title="机构选取" :visible.sync="orgDialogVisible" width="800px">
            <yu-xform ref="searchForm" related-table-name="orgTable" v-model="searchFormdata"
              ...
              <yu-xform-group :column="3">
                <yu-xform-item name="orgCode" label="机构代码" placeholder="机构代码" fuzzy-quer="both"></yu-xform-item>
                <yu-xform-item name="orgName" label="机构名称" placeholder="机构名称" fuzzy-quer="both"></yu-xform-item>
              </yu-xform-group>
            </yu-xform>
...
export default {
  ...
  methods: {
    ...
    onSaveData () {
      ...
      this.$request({
        url: _this.$backend.cmisCfg + '/api/cfgretailprimerate/update/',
        method: 'post',
        data: jsoPar
      }).then((response) => {
        if (response.code === '0') {
