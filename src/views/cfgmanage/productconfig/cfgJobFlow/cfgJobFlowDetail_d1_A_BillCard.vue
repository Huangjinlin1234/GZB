      ...
      <yu-xform ref="refForm" label-width="120px" :form-type="formType" v-model="formdata" :rules="formRules" :form-change="triggerEditChangeEvent">
        <yu-panel title="作业流配置信息" panel-type="normal">
          <yu-xform-group :column="3">
            ...
            <yu-xform-item label="业务规则方案编号" ctype="yu-xbins-rule1" name="planId" placeholder="业务规则方案编号" @select-fn="commonSelectFn" :mapping="{ planId: 'planId' }" :disabled="disabled"></yu-xform-item>
            <yu-xform-item label="工作流编号" ctype="yu-xflow-define" name="wfiSignId" rules="required" placeholder="工作流编号" @select-fn="commonSelectFn" :mapping="{ flowId: 'wfiSignId', flowName: 'wfiSignName' }" width="480" height="480" :disabled="disabled"></yu-xform-item>
          </yu-xform-group>
          <yu-xform-group :column="3">
            <yu-xform-item label="工作流名称" ctype="input" placeholder="工作流名称" name="wfiSignName" disabled></yu-xform-item>
            <yu-xform-item label="审批查看模板组" ctype="yu-xmodel-factory" name="apprViewUrl" rules="required" placeholder="审批查看模板组" @select-fn="commonSelectFn" :mapping="{ modelGroupNo: 'apprViewUrl' }" :disabled="disabled"></yu-xform-item>
            <yu-xform-item label="审批调整模板组" ctype="yu-xmodel-factory" name="apprAdjustTpl" placeholder="审批调整模板组" @select-fn="commonSelectFn" :mapping="{ modelGroupNo: 'apprAdjustTpl' }" :disabled="disabled"></yu-xform-item>
            <yu-xform-item label="备注" ctype="textarea" name="remark" placeholder="备注" :colspan="24" :disabled="disabled"></yu-xform-item>
          </yu-xform-group>
        </yu-panel>
...
export default {
  ...
  watch: {
    "formdata.wfiSignId": function(newVal) {
      let condition = { jobFlowId: this.formdata.jobFlowId, wfiSignId: newVal };
      this.$parent.getDialogDataFunc(condition);
    }
  },
  methods: {
