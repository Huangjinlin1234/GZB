...
export default {
  ...
  methods: {
    ...
    doNextStep () {
      ...
      var serno = this.$xutils.getSEQWithParamFromServer('IQP_SERNO');
      var correNo = this.$xutils.dateFormat('yyyyMMddhhmmss', new Date());
      jsoPar.serno = serno;
      jsoPar.correNo = correNo;

      this.d1_BillCard.serno = serno;
      this.d1_BillCard.correNo = correNo;
      this.d1_BillCard.formdata.appType = '01';
      this.d1_BillCard.formdata.oprType = '01';
      this.d1_BillCard.formdata.approveStatus = '000';
      this.d1_BillCard.saveBillCardData();
      this.$dialog.open('关联客户认定', 'cusmanage/cusRelevance/cusGuideApp2Index', '-1', '-1', jsoPar, data => {
        if (data) {
