...
export default {
  ...
  methods: {
    ...
    // 新增申请
    onAdd () {
      this.$dialog.open('', 'cusmanage/cusRelevance/cusGuideAppIndex', 800, 600, {}, () => {
        this.d1_BillList.queryDataByCondition();
      });
    },
    ...
    onUpdate () {
      ...
      if ((approveStatus && !(approveStatus == '000' || approveStatus == '992')) || loginCode != params.inputId) {
        _this.$xutils.showMsgBox('提示', '仅【待发起】及【打回】且经办人是自己的数据可以修改!');
        return;
      }

      this.$dialog.open(
        '关联客户修改界面',
        'cusmanage/cusRelevance/cusGuideAppUpdateIndex',
        -1,
        -1,
        params,
        () => {
