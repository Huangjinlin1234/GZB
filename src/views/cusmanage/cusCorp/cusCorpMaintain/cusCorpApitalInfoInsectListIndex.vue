...
<script>
import d1Billcard from './cusCorpApitalInfoInsectList_d1_BillCard.vue';
export default {
  ...
  methods: {
    AfterInit () {
      ...
      if(this.pageParams.op == 'ADD'){
        ...
        this.d1_BillCard.setItemValue('cusIdRel', this.pageParams.cusIdRel);
    ...
    onSave () {
      ...
      if(new Date(this.d1_BillCard.getItemValue('invDate')) > new Date(openday)){
        this.$message({
          message: '出资时间不允许超过当前日期',
          type: 'warning'
        });
        return;
      }
