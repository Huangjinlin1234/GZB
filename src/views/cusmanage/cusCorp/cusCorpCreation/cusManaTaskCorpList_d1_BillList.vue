<template>
  <div>
  <yu-panel title="对公客户日常任务" :hideFilter="false" :collapseHide="false">
      <yu-xform ref="refForm" form-type="search" v-model="searchFormdata" label-width="120px" :custom-search-fn="customSearch">
        <yu-xform-group :column="3">
          <yu-xform-item label="业务类型" ctype="select" data-code="STD_ZB_CUS_BIZ_TYPE" placeholder="业务类型" name="bizType" exclude-key="A04,A05,A06,A07,A08,A09,A10,A11,A12,A13,B01,B02,B03,B04,B05,B06,B07,B08,B09,C01,C02"></yu-xform-item>
...
export default {
  ...
  data: function () {
    return {
      ...
      baseParams: { condition: { taskStatusList: '1,2', bizType:'A01,A02,A03', inputId: this.$xutils.getDefaultformulaData("$LoginLoginCode")} },
  ...
  methods: {
    ...
    route (row) {
      ...
      if (biz_type == 'A02') {
        ...
        let key = 'tempCusComCreate' + json.cusId;
        json.key = `/${name}/${key}`;
        this.$router.addTab({
          // 路由名称
          name: name,
        ...
        let key = 'fromalCusCom' + json.cusId;
        json.key = `/${name}/${key}`;
        this.$router.addTab({
          // 路由名称
          name: name,
        ...
        // this.$dialog.open("公司客户创建（集中作业）", "cfgmanage/productconfig/templetfactory/tempetfactorypreviewIndex", -1, -1, json, () => {});
      }
    },
    //作废
    cancleClick (row) {
